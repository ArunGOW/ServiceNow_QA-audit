...
// Fetch incidents
const fetchIncidents = async (skipVal = 0, pageNum = 1) => {
  try {
    setLoading(true);

    const res = await axios.post(
      "http://127.0.0.1:8000/api/users/get/incidents",
      { skip: skipVal, limit, user_sid: user?.user_sid }
    );

    setIncidents(res.data.incidents || []); 
    setTotalCount(res.data.total || 0);   // <-- API should return total count
    setPage(pageNum);
  } catch (err) {
...
const handlePageChange = (newPage) => {
  ...
  setSkip(newSkip);
  fetchIncidents(newSkip, newPage);
};
...
<div className="d-flex justify-content-center align-items-center mt-3 gap-3">
  ...
  <span className="fw-semibold">
    Page {page} / {Math.ceil(totalCount / limit) || 1}
  </span>
