...
const [allIncidents, setAllIncidents] = useState([]); // store all 200
const [incidents, setIncidents] = useState([]);       // sliced 10
const [skip, setSkip] = useState(0);
const limit = 10;
...
useEffect(() => {
  if (allIncidents.length > 0) {
    ...
    setIncidents(allIncidents.slice(start, end));
  }
}, [allIncidents, skip]);

// Pagination handlers
const handleNext = () => {
  if (skip + limit < allIncidents.length) {
    setSkip(skip + limit);
  }
};
...
const handlePrevious = () => {
  ...
  const fetchIncidents = async () => {
    ...
    try {
      ...
      const url = "http://127.0.0.1:8000/api/users/get/incidents";
      console.log("📡 Fetching incidents from:", url);
  ...
  // Slice incidents based on skip & limit
  useEffect(() => {
    const sliced = allIncidents.slice(skip, skip + limit);
    setIncidents(sliced);
  }, [allIncidents, skip]);
