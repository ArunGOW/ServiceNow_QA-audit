...
const Dashboard = () => {
  ...
  // fetch incidents API
  const fetchIncidents = async (skipValue = 0) => {
    if (!user?.user_sid) return;

    try {
      setLoading(true);

      const url = `http://localhost:8000/api/users/get/incidents?skip=${skipValue}&limit=${limit}`;
      console.log("📡 Fetching incidents from:", url);
  ...
  // handlers
  const handleNext = () => {
    if (skip + limit < totalPages * limit) {
      setSkip(skip + limit);
    }
  };
