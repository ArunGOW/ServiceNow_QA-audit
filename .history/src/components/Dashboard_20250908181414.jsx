  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await axios.post(`http://localhost:8000/api/users/get/incidents?skip=${skip}&limit=${limit}`);
        setPendingIncidents(res.data.pending || []);
        setProcessedIncidents(res.data.processed || []);
      } catch (error) {
  ...
  return (
    <Layout>
      ...
      <div className="d-flex justify-content-center mt-3">
        <PaginationControls
          currentPage={currentPage}
          totalPages={20} // 🔹 ideally from backend total count
