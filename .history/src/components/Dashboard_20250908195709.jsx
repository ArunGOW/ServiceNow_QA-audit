...
//                 <thead style={{ background: "linear-gradient(90deg, #0d6efd, #6610f2)", color: "white" }}>
//                   <tr>
//                     <th>INCIDENT NO</th>
//                     <th>SHORT DESCRIPTION</th>
//                     <th>STATUS</th>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!user?.user_sid) {
        console.warn("No user_sid found, skipping fetch.");
        return;
      }
      ...
      try {
        ...
        const allIncidents = res.data?.incidents || [];
          ...
          // Decide pending vs processed
        const pending = allIncidents.filter(
          (i) =>
            !i.audit_status || 
            i.audit_status?.toLowerCase() === "pending" ||
            i.resolution_status?.toLowerCase() === "escalated"
        );

         const processed = allIncidents.filter(
          (i) =>
            i.audit_status?.toLowerCase() === "completed" ||
            i.resolution_status?.toLowerCase() === "resolved"
        );
     ...
     <Layout>
      ...
      <div className="d-flex justify-content-center mb-4">
        <ButtonGroup>
          ...
          <Button
            variant={activeTab === "processed" ? "primary" : "outline-primary"}
