...
//         alert("Your Google account email is not verified.");
//         return;
//       }
...
//         const { user_sid, user_type, session_token } = res.data;

//         // Store in localStorage (or cookies)
//         localStorage.setItem("user_sid", user_sid);
//         localStorage.setItem("user_type", user_type);
//         localStorage.setItem("session_token", session_token);

//         console.log("✅ Login successful:", res.data);
//         alert(`Welcome ${decoded.name}! You are logged in as ${user_type}`);
...
const Login = () => {
  const handleLoginSuccess = async (credentialResponse) => {
    try {
      ...
      // 1️⃣ Fetch your user list from backend
      const { data: users } = await axios.get("http://localhost:8000/api/users/get/list_users");
      console.log("📋 User List:", users);

      // 2️⃣ Check if Google user email exists in your system
      ...
      // 3️⃣ If found, send token to backend login endpoint
      const response = await axios.post("http://localhost:8000/api/users/login-google", {
