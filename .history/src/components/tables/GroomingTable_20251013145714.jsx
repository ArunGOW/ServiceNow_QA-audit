...
const GroomingTable = ({ incidents, loading, fetchIncidents }) => {
  const { token } = useAuth();
  const [showModal, setShowModal] = useState(false);
  ...
  const formatDate = (dateStr) => {
  if (!dateStr) return ""; // handles null, undefined, or empty
  const date = new Date(dateStr);

  // Check if date is valid
  if (isNaN(date.getTime())) return ""; // prevent "Invalid time value"
  return date.toISOString().split("T")[0];
};
  ...
  // Handle update grooming details
  const handleUpdate = async () => {
    try {
      const response = await axios.put(
        `http://localhost:8000/api/incidents/${formData.incident_sid}/update_grooming`,
        formData,
        {
          headers: {
      ...
      console.error("❌ Error updating grooming details:", error);
      toast.error("❌ Failed to update grooming details!", {
        position: "top-center",
        autoClose: 3000,
        transition: Slide,
      });
  ...
  if (loading) {
    return <p className="text-center mt-3 fw-bold">Loading incidents...</p>;
  }

  return (
    <div className="container mt-4">
      <h4 className="fw-bold text-center mb-3 text-primary">
        Grooming Pending Incidents
      </h4>
