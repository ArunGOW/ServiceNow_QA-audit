...
const GroomingTable = ({ incidents, loading, refresh }) => {
  ...
  const handleCommentToggle = (comment) => {
    setSelectedComments((prev) => {
      ...
      setTotalScore(
        updated.reduce((sum, item) => sum + (item.point_value || 0), 0)
      );
  ...
  const handleOpenModal = (incident) => {
    ...
    setFormData({
      ...
      audit_date:
        incident.audit_date || new Date().toISOString().split("T")[0],
    });

    if (incident.qa_comment) {
      const selectedNames = incident.qa_comment
        .split(", ")
        .map((name) => name.trim());
  ...
  const handleUpdate = async () => {
    try {
      ...
      toast.success("Incident updated successfully!");
      logger.info("Incident update response:", res.data);
  ...
  const handleAssign = async () => {
    if (!selectedUser) {
      ...
      logger.warn("Assignment failed: No user selected.");
      return;
    }

    if (selectedIncidents.length === 0) {
      toast.error("Please select at least one incident.");
      logger.warn("Assignment failed: No incidents selected.");
      return;
    }

    if (!loggedUserSid) {
      toast.error("Logged-in user SID missing.");
      logger.error("Assignment error: Missing loggedUserSid");
      return;
    }
            ...
            {incidents.length > 0 ? (
              incidents.map((incident) => (
                <tr key={incident.sid}>
                  ...
                  <td className="short-desc-cell">
                    <div title={incident.short_description}>
                      {incident.short_description || "N/A"}
                    </div>
                  </td>
            ...
            <Form>
              ...
              <div className="mb-4 p-3 border rounded shadow-sm bg-light">
                ...
                <Row className="mb-3">
                  <Col md={4}>
                    <div>
                      <span className="fw-bold">Incident Number: </span>
                      {formData.incident_number || "N/A"}
                    </div>
                  </Col>
                  ...
                  <Col md={8}>
                    <div>
                      ...
                      {formData.short_description || "N/A"}
                    </div>
                  </Col>
                </Row>
                ...
                <Row>
                  <Col md={4}>
                    <div>
                      <span className="fw-bold">Handled By: </span>
                      {formData.assigned_analyst || "N/A"}
                    </div>
                  </Col>

                  <Col md={4}>
                    <div>
                      <span className="fw-bold">Handled On: </span>
                      {formatDate(formData.incident_date) || "N/A"}
                    </div>
                  </Col>
                  ...
                  <Col md={4}>
                    <div>
                      ...
                      {formData.resolution_status || "N/A"}
                    </div>
                  </Col>
                </Row>
              </div>
              ...
              <div className="mb-4 p-3 border rounded shadow-sm bg-light">
                ...
                <Row className="mb-3">
                  <Col md={4}>
                    <span className="fw-bold">QA Status:</span>{" "}
                    <span
                      className={`badge rounded-pill px-3 py-2 text-white ${
                        formData.qa_status === "Pass"
                          ? "bg-success"
                          : formData.qa_status === "Fail"
                          ? "bg-danger"
                          : "bg-secondary"
                  ...
                  <Col md={4}>
                    <span className="fw-bold">QA Done On:</span>{" "}
                    {formatDate(formData.audit_date) || "N/A"}
                  </Col>

                  <Col md={4}>
                    <span className="fw-bold">QA Agent:</span>{" "}
                    {formData.qc_analyst || user?.full_name || "N/A"}
                  </Col>
                </Row>

                <Row>
                  <Col md={8}>
                    <span className="fw-bold">QA Comments:</span>
                    <div>
                      {formData?.scores &&
                      Object.keys(formData.scores).length > 0 ? (
                        Object.entries(formData.scores).map(
                          ([key, value], idx) => (
