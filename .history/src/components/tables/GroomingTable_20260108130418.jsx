...
const handleUpdate = async () => {
  ...
  // 2. Ensure qc_analyst is captured from logged-in user
  const currentAnalyst = user?.full_name || user?.name || "Unknown Analyst";

  // 3. Build the NESTED scores object
  const scoresObj = {};

  selectedComments.forEach((item) => {
    // Convert Category Name to snake_case (e.g., "Communication Clarity" -> "communication_clarity")
    const categoryKey = item.parent_name 
      ? item.parent_name.toLowerCase().replace(/\s+/g, "_") 
      : "uncategorized";

    // Convert Checkpoint Name to snake_case (e.g., "Clear Language" -> "clear_language")
    const subKey = item.check_point_name 
      ? item.check_point_name.toLowerCase().replace(/\s+/g, "_") 
      : "unnamed_point";

    const points = Number(item.point_value) || 0;

    // Initialize category if it doesn't exist
    if (!scoresObj[categoryKey]) {
    ...
    // Add values
    scoresObj[categoryKey].total += points;
    scoresObj[categoryKey].sub_scores[subKey] = points;
  });

  // 4. Add the total_score_achieved to the scores object
  scoresObj.total_score_achieved = totalScore;
  ...
  const payload = [
    {
      ...
      assigned_analyst: formData.assigned_analyst || "",
      qc_analyst: currentAnalyst, // ✅ This will now use the logged-in user
      is_audited: true,
      audit_status: "completed",
      notes: formData.notes, // ✅ Mandatory notes
      audit_date: new Date().toISOString(),
      
      // ✅ This is the nested structure you requested
      scores: scoresObj, 
  ...
  try {
    logger.info("🚀 Sending Final Payload:", payload);
    const res = await api.post("/users/update/incident-status", payload);
