...
const GroomingTable = ({ incidents, loading, refresh }) => {
  ...
  const initialFormData = {
    ...incident,
    incident_sid: incident.sid,
    qc_analyst: incident.qc_analyst || user?.full_name || "",
    audit_date: incident.audit_date || new Date().toISOString().split("T")[0],
    notes: incident.notes || "",
  };
  setFormData(initialFormData);

  // If the incident already has scores in the new nested format
  if (incident.scores && typeof incident.scores === 'object') {
    const preSelected = [];
    
    // Iterate through categories (e.g., communication_clarity)
    Object.keys(incident.scores).forEach(catKey => {
      if (catKey === "total_score_achieved") return;

      const category = incident.scores[catKey];
      if (category.sub_scores) {
        // Look at each sub_score
        Object.keys(category.sub_scores).forEach(subKey => {
          const scoreValue = category.sub_scores[subKey];
          
          // Only add to selected if it has a value > 0
          if (scoreValue > 0) {
            // Find the original point object from our master list to maintain object reference
            const originalPoint = qualityPoints.find(qp => 
              qp.check_point_name.toLowerCase().replace(/\s+/g, "_") === subKey
            );
            if (originalPoint) preSelected.push(originalPoint);
          }
        });
      }
    });
    ...
    setSelectedComments(preSelected);
    setTotalScore(incident.scores.total_score_achieved || 0);
  } else {
    setSelectedComments([]);
    setTotalScore(0);
  }
...
//       try {
//   const res = await api.post("/users/user/fail-edit", payload);

//   toast.success("Updated successfully!");

//   // ⬅️ Add this
//   if (typeof refresh === "function") {
//     refresh();        // reload incidents list
//   }
...
//       // 🔄 refresh parent table only once
...
//   const handleUpdate = async () => {
//      // 1. Mandatory Notes Validation
//   if (!formData.notes || formData.notes.trim() === "") {
//     logger.warn("Validation failed: Notes field is empty");
//     return toast.error("Please enter Audit Notes before saving!");
//   }

//   // 2. Ensure qc_analyst is captured (Fix for the empty field)
//   const currentAnalyst = user?.full_name || user?.name || formData.qc_analyst || "System User";

//   // 3. Transform scores into the nested structure you want
//   const nestedScores = selectedComments.reduce((acc, item) => {
//     // Standardize category key (lowercase and underscores)
//     const categoryKey = (item.category || item.parent_name || "Uncategorized")
//       .toLowerCase()
//       .replace(/\s+/g, "_");
...
//     // Standardize sub-score key
//     const subKey = (item.check_point_name || "unnamed_point")
//       .toLowerCase()
//       .replace(/\s+/g, "_");

//     const points = Number(item.point_value) || 0;

//     // Add to category total and map sub_score
//     acc[categoryKey].total += points;
//     acc[categoryKey].sub_scores[subKey] = points;

//     return acc;
//   }, {});

//   // Add the final total sum to the scores object
//   nestedScores.total_score_achieved = totalScore;
//   try {
//     const payload = [
//       {
//         incident_sid: formData.incident_sid,
...
//       },
//     ];
//     logger.info("Sending payload with analyst:", currentAnalyst);
//     logger.info("🚀 Outgoing Payload:", payload);
...
// const handleUpdate = async () => {
//   // 1. Mandatory Notes Check
//   if (!formData.notes || formData.notes.trim() === "") {
//     logger.warn("Validation failed: Notes field is empty");
//     return toast.error("Please enter Audit Notes before saving!");
//   }

//   const currentAnalyst = user?.full_name || user?.name || "Unknown Analyst";

//   // 2. Build the NESTED scores object with all categories
//   const scoresObj = {};

//   // Helper function to convert strings to snake_case
//   const toSnakeCase = (str) => 
//     str ? str.toLowerCase().trim().replace(/\s+/g, "_").replace(/[^\w]/g, "") : "unknown";

//   // Get unique categories from your qualityPoints master list
//   const categories = [...new Set(qualityPoints.map(p => p.parent_name || "Uncategorized"))];

//   categories.forEach(categoryName => {
//     const categoryKey = toSnakeCase(categoryName);
    
//     // Initialize the structure
//     scoresObj[categoryKey] = {
//       total: 0,
//       sub_scores: {}
//     };

//     // Find all checkpoints belonging to this category from the master list
//     const pointsInCategory = qualityPoints.filter(p => (p.parent_name || "Uncategorized") === categoryName);

//     pointsInCategory.forEach(point => {
//       const subKey = toSnakeCase(point.check_point_name);
      
//       // Check if this specific checkpoint was actually selected/scored
//       const isSelected = selectedComments.find(c => c.sid === point.sid);
//       const pointsValue = isSelected ? (Number(isSelected.point_value) || 0) : 0;

//       // Assign to sub_scores and increment category total
//       scoresObj[categoryKey].sub_scores[subKey] = pointsValue;
//       scoresObj[categoryKey].total += pointsValue;
//     });
//   });

//   // 3. Add the total_score_achieved (Sum of all category totals)
//   scoresObj.total_score_achieved = Object.values(scoresObj).reduce((acc, cat) => acc + (cat.total || 0), 0);
...
//       incident_date: formatDate(formData.incident_date),
//       short_description: formData.short_description || "",
//       resolution_status: formData.resolution_status || "escalated",
//       assigned_analyst: formData.assigned_analyst || "",
//       qc_analyst: currentAnalyst,
//       is_audited: true,
//       audit_status: "completed",
...
//   try {
//     logger.info("🚀 Sending Final Nested Payload:", payload);
//     const res = await api.post("/users/update/incident-status", payload);
...
const handleUpdate = async () => {
          ...
          {formData && (
            <Form>
              ...
              <div className="mb-4 p-3 border rounded shadow-sm bg-light">
                ...
                <Row className="mt-3">
    <Col md={12}>
      <span className="fw-bold mb-2 d-block">Score Breakdown:</span>
      <div className="qa-comments-list p-2 border rounded bg-white">
        {formData?.scores ? (
          Object.entries(formData.scores).map(([catKey, catData]) => {
            // Skip the total score key and categories with 0 total if you want to keep it clean
            if (catKey === "total_score_achieved" || catData.total === 0) return null;
            
            return (
              <div key={catKey} className="mb-2 pb-2 border-bottom last-child-border-0">
                <div className="d-flex justify-content-between">
                  <strong className="text-primary">{catKey.replace(/_/g, ' ').toUpperCase()}</strong>
                  <span className="badge bg-secondary">{catData.total} pts</span>
                </div>
                <ul className="small mb-0 mt-1">
                  {Object.entries(catData.sub_scores).map(([subKey, score]) => (
                    score > 0 && (
                      <li key={subKey}>
                        {subKey.replace(/_/g, ' ')}: <span className="fw-bold">{score}</span>
                      </li>
                    )
                  ))}
                </ul>
          ...
          <span className="text-muted italic">No scoring data available.</span>
        )}
      </div>
    </Col>
  </Row>
  ...
  <Row className="mt-3">
    <Col className="text-end">
      <div className="info-field">
        <span className="fw-bold me-2">Total Score Achieved:</span>
        <span className="badge bg-primary rounded-pill px-4 py-2 fs-5">
          {totalScore || 0}
        </span>
      </div>
    </Col>
