...
const GroomingTable = ({ incidents, loading, refresh }) => {
  ...
  const getCleanQAName = () => {
  // Priority 1: Use the logged-in user's name from context
  // Priority 2: Fallback to formData if user object isn't loaded
  const nameToProcess = user?.full_name || formData.qc_analyst || "system";
  
  // .split(" ")[0] handles "Arun Kumar" -> "Arun"
  // .toLowerCase() handles "Arun" -> "arun"
  return nameToProcess.split(" ")[0].toLowerCase();
};
  ...
  const handleUpdate = async () => {
   ...
   // Formatting the name here
  const formattedQCName = getFirstName(user?.full_name);
  try {
    const payload = [
      {
