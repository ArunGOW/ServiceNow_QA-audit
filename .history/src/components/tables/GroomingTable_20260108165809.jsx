...
//   };
    ...
    const payload = [
  {
    ...
    incident_number:
      formData.incident_number ?? selectedIncident.incident_number,
    ...
    short_description:
      formData.short_description ?? selectedIncident.short_description,

    resolution_status:
      formData.resolution_status ?? selectedIncident.resolution_status,

    resolution_shared:
      formData.resolution_shared ?? selectedIncident.resolution_shared,

    updates_link:
      formData.updates_link ?? selectedIncident.updates_link,

    notes:
      formData.notes ?? selectedIncident.notes,

    escalated_to_team:
      formData.escalated_to_team ?? selectedIncident.escalated_to_team,

    assigned_analyst:
      formData.assigned_analyst ?? selectedIncident.assigned_analyst,

    // 👇 grooming should capture current QC analyst
    qc_analyst: currentAnalyst,

    is_audited:
      formData.is_audited ?? selectedIncident.is_audited ?? true,

    audit_status:
      formData.audit_status ?? selectedIncident.audit_status ?? "done",
    ...
    qa_comment: [
      ...new Set(selectedComments.map(c => c.parent_name)),
    ].join(", "),

    rca_done:
      formData.rca_done ?? selectedIncident.rca_done ?? false,

    grooming_needed:
      formData.grooming_needed ?? selectedIncident.grooming_needed ?? false,

    grooming_done:
      true, // 👈 grooming table always true

    kba_resolution_needed:
      formData.kba_resolution_needed ??
      selectedIncident.kba_resolution_needed ??
      false,

    kba_resolution_updated:
      formData.kba_resolution_updated ??
      selectedIncident.kba_resolution_updated ??
      false,

    // 👇 SAME scores structure as pending
