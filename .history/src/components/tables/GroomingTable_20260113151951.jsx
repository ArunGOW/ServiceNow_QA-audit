...
<div className="premium-container p-4">
  ...
  <div className="header-glass mb-4 p-4 d-flex justify-content-between align-items-center">
    <div>
      <h4 className="fw-bold mb-0 text-gradient text-uppercase">Pending Grooming</h4>
      <p className="text-muted small mb-0">Review and finalize quality control audits</p>
    </div>
    <div className="stats-badge d-none d-md-flex gap-3">
  ...
  <div className="table-card shadow-sm">
    {loading ? (
      ...
      <Table hover responsive className="premium-table mb-0">
        <thead>
          <tr>
             <th className="ps-3 py-3" style={{ width: "110px", minWidth: "110px" }}>
  <div className="d-flex align-items-center gap-2">
    <Form.Check
      type="checkbox"
      className="custom-checkbox-premium compact-check"
      checked={incidents.length > 0 && selectedIncidents.length === incidents.length}
      onChange={handleSelectAll}
    />
    <span className="header-all-text fw-bold">ALL</span>
  </div>
</th>
            <th>Incident No</th>
            <th>Short Description</th>
        ...
        <tbody>
          {incidents.length > 0 ? (
            incidents.map((incident, index) => (
              <tr key={incident.sid || index} className="align-middle">
                <td className="ps-4">
                  <Form.Check
                    type="checkbox"
                ...
                <td>
                  <Button
                    variant="link"
                    className="text-decoration-none fw-bold incident-link"
                    onClick={() => handleOpenModal(incident)}
                  >
                    #{incident.incident_number}
                  </Button>
                ...
                <td>
                  <div className="text-truncate-2" title={incident.short_description}>
                    {incident.short_description || "N/A"}
                  </div>
                </td>
                ...
                <td>
                  <span className="analyst-name">
                    {incident.assigned_analyst?.toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase()) || "N/A"}
                  </span>
                </td>
                <td>{formatDate(incident.updated_at)}</td>
                <td className="pe-4 fw-semibold text-secondary">
                   {incident.qc_analyst?.toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase()) || "NA"}
                </td>
              </tr>
            ))
          ) : (
            <tr>
              <td colSpan="7" className="py-5 text-center text-muted">No grooming incidents found.</td>
            </tr>
          )}
        </tbody>
      </Table>
  ...
  <Modal show={showModal} onHide={handleCloseModal} size="lg" centered className="premium-modal">
    <Modal.Header closeButton className="bg-dark text-white p-4">
      <Modal.Title className="fw-bold">
        <i className="bi bi-shield-check me-2"></i> Audit Review Details
      </Modal.Title>
    </Modal.Header>
      ...
      {formData && (
        <Form>
          ...
          <div className="modal-section mb-4">
            <h6 className="section-title"><i className="bi bi-info-circle me-2"></i>Incident Information</h6>
            <div className="info-grid">
                <div className="info-box"><strong>Number:</strong> {formData.incident_number}</div>
                <div className="info-box"><strong>Date:</strong> {formatDate(formData.incident_date)}</div>
                <div className="info-box"><strong>Status:</strong> <span className="text-primary">{formData.resolution_status}</span></div>
                <div className="info-box full-width"><strong>Description:</strong> {formData.short_description}</div>
            </div>
          </div>

          {/* Section 2: QA Score Card */}
          <div className="modal-section mb-4">
             <div className="d-flex justify-content-between align-items-center mb-3">
                <h6 className="section-title mb-0"><i className="bi bi-trophy me-2"></i>QA Score Summary</h6>
                <div className={`qa-badge ${formData.qa_status?.toLowerCase()}`}>
                    {formData.qa_status || "Pending"}
                </div>
             </div>
             
             <div className="comments-wrapper p-3 bg-white rounded border">
                {formData?.scores && Object.entries(formData.scores).map(([key, value], idx) => (
                    <div key={idx} className="comment-pill d-flex justify-content-between small mb-2 border-bottom pb-1">
                        <span><strong>{key}:</strong> {value.description}</span>
                        <span className="badge bg-secondary text-white">{value.point_value}</span>
                    </div>
                ))}
                <div className="text-end mt-2 fw-bold text-dark fs-5">
                    Total Score: <span className="text-primary">{totalScore || 0}</span>
                </div>
             </div>
          </div>

          {/* Section 3: Grooming Action (MANDATORY) */}
          <div className="modal-section action-section p-3 rounded">
            <h6 className="section-title text-white"><i className="bi bi-pencil-square me-2"></i>Grooming Action</h6>
            <Row>
                <Col md={12} className="mb-3">
                    <div className="p-3 bg-white rounded shadow-sm">
                        <Form.Check 
                            type="checkbox"
                            id="grooming-done-check"
                            name="grooming_done"
                            className="fw-bold text-success fs-5 mb-2"
                            checked={!!formData.grooming_done}
                            onChange={handleChange}
                        ...
                        <Form.Control 
                            ...
                            rows={3}
                            name="notes"
                            placeholder="Type mandatory audit observations here..."
    ...
    <Modal.Footer className="bg-white border-top-0 p-3">
      <Button variant="outline-dark" className="px-4" onClick={handleCloseModal}>Cancel</Button>
      <Button variant="primary" className="premium-btn px-5" onClick={handleUpdate}>Save Audit Changes</Button>
    </Modal.Footer>
  </Modal>
  <ToastContainer />
</div>
