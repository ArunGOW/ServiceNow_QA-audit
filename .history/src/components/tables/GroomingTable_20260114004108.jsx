...
const GroomingTable = ({ incidents, loading, refresh }) => {
  ...
  useEffect(() => {
    if (selectedComments && selectedComments.length > 0) {
      ...
      const total = Object.values(formData.scores).reduce((sum, val) => sum + (Number(val.point_value) || 0), 0);
      setTotalScore(total);
    } else {
      setTotalScore(0);
    }
  ...
  const handleUpdate = async () => {
    ...
    selectedComments.forEach((item) => {
      const categoryKey = item.parent_name?.toLowerCase().replace(/\s+/g, '_');
      const subKey = item.sub_check_point_name?.toLowerCase().replace(/\s+/g, '_');
      if (categoryKey && scoresPayload[categoryKey]) {
        scoresPayload[categoryKey].sub_scores[subKey] = item.max_points;
        scoresPayload[categoryKey].total += (item.max_points || 0);
      }
    });
  ...
  const handleSelectAll = () => {
    setSelectedIncidents(selectedIncidents.length === incidents.length ? [] : incidents.map((i) => i.sid));
  };
    ...
    <Card className="premium-card border-0 shadow-sm">
      <Card.Header className="bg-white py-3 d-flex justify-content-between align-items-center border-bottom">
        ...
        <Badge bg="soft-primary" className="text-primary px-3 py-2 rounded-pill">
          {incidents.length} Tasks Pending
        </Badge>
      </Card.Header>
              ...
              <tbody>
                {incidents.map((incident) => (
                  <tr key={incident.sid}>
                    ...
                    <td className="text-truncate" style={{ maxWidth: '200px' }}>{incident.short_description}</td>
                    ...
                    <td className="pe-4 text-end">
                      <Button variant="outline-primary" size="sm" onClick={() => handleOpenModal(incident)}>
                        Review
                      </Button>
                    </td>
                  </tr>
      ...
      <Modal show={showModal} onHide={handleCloseModal} size="lg" centered className="premium-modal">
        <Modal.Header closeButton className="border-0">
          <Modal.Title className="fw-bold">Grooming Assessment</Modal.Title>
        </Modal.Header>
        <Modal.Body className="px-4">
          <div className="info-grid mb-4">
             <Row>
                <Col md={6}><small className="text-muted d-block">Incident</small><strong>{formData.incident_number}</strong></Col>
                <Col md={6}><small className="text-muted d-block">Status</small><Badge bg="info">{formData.resolution_status}</Badge></Col>
             </Row>
          </div>
          
          <Form.Group className="mb-3">
            <Form.Label className="fw-bold">Grooming Notes</Form.Label>
            <Form.Control 
              as="textarea" rows={3} 
              name="notes"
              value={formData.notes || ""}
