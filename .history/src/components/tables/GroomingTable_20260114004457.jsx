...
<div className="premium-table-container shadow-sm border rounded-4 bg-white p-4">
  {/* Header Section */}
  <div className="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h5 className="fw-bold mb-1 text-dark">Pending Grooming</h5>
      <p className="text-muted small mb-0">Review and finalize quality check incidents</p>
    </div>
    
    {/* Refined Assignment Controls */}
    <div className="d-flex gap-2 align-items-center bg-light p-2 rounded-3 border">
      ...
      >
        ...
        {users.map((u) => (
          <option key={u.sid} value={u.sid}>{u.full_name}</option>
        ))}
      </Form.Select>
      <Button
        size="sm"
        variant="primary"
        className="rounded-2 px-3 fw-bold shadow-sm"
        disabled={selectedIncidents.length === 0 || !selectedUser}
        onClick={handleAssign}
      >
        Assign Tasks
  ...
  {loading ? (
    <div className="text-center py-5">
      <Spinner animation="border" variant="primary" />
      <p className="mt-2 text-muted">Fetching incidents...</p>
    </div>
  ) : (
    <div className="table-responsive">
      <Table hover className="align-middle custom-premium-table">
        <thead>
          <tr>
          ...
          {incidents.length > 0 ? (
            incidents.map((incident, index) => (
              <tr key={incident.sid || index}>
                ...
                <td>
                  <div className="text-truncate text-secondary" style={{ maxWidth: "250px" }} title={incident.short_description}>
                    {incident.short_description || "—"}
                  </div>
                </td>
                <td>
                  <div className="d-flex align-items-center">
                    <div className="avatar-circle me-2">{incident.assigned_analyst?.charAt(0)}</div>
                    <span className="small fw-medium text-dark">{incident.assigned_analyst || "N/A"}</span>
                  </div>
                </td>
                <td><span className="badge bg-soft-info text-info">{incident.qc_analyst || "Pending"}</span></td>
                <td className="text-end pe-4">
                   <Button variant="light" size="sm" onClick={() => handleOpenModal(incident)} className="border">Review</Button>
                </td>
              </tr>
            ))
          ) : (
  ...
  <Modal show={showModal} onHide={handleCloseModal} size="lg" centered className="premium-modal">
    <Modal.Header closeButton className="border-0 pb-0">
      <Modal.Title className="fw-bold">Incident Review <small className="text-muted fw-normal ms-2">#{formData.incident_number}</small></Modal.Title>
    </Modal.Header>
    <Modal.Body className="pt-3">
      <div className="review-grid">
        {/* Section: Overview */}
        <div className="info-card mb-4">
          <label className="section-label">Incident Overview</label>
          <Row className="g-3">
            <Col md={8} className="border-end">
              <p className="small text-muted mb-1">Description</p>
              <p className="fw-medium">{formData.short_description}</p>
            </Col>
            <Col md={4}>
               <p className="small text-muted mb-1">Status</p>
               <span className="badge bg-soft-primary text-primary">{formData.resolution_status}</span>
            </Col>
          </Row>
        </div>

        {/* Section: Grooming Actions */}
        <div className="action-card bg-light p-3 rounded-3 border">
          <label className="section-label mb-3">Grooming Requirements</label>
          <Row className="g-3">
            <Col md={4}>
              <Form.Group className="premium-switch">
                <Form.Label className="small fw-bold mb-0">Grooming Done</Form.Label>
                <Form.Check 
                  type="switch" 
                  name="grooming_done"
                  checked={!!formData.grooming_done} 
                  onChange={handleChange}
                  label={formData.grooming_done ? "Completed" : "Pending"}
                />
              </Form.Group>
            </Col>
            <Col md={8}>
               <Form.Group>
                 <Form.Label className="small fw-bold">Instructions</Form.Label>
                 <Form.Control 
                   className="form-control-sm border-0 shadow-xs" 
            ...
            <Col md={12}>
              <Form.Group>
                <Form.Label className="small fw-bold">Audit Observations <span className="text-danger">*</span></Form.Label>
                <Form.Control 
                  as="textarea" rows={3} 
    ...
    <Modal.Footer className="border-0 pt-0">
      <Button variant="outline-secondary" className="px-4" onClick={handleCloseModal}>Cancel</Button>
      <Button variant="primary" className="px-4 fw-bold" onClick={handleUpdate}>Submit Grooming</Button>
    </Modal.Footer>
  </Modal>
</div>
