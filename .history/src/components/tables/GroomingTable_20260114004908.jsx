...
//     </div>
 ...
 <div className="bg-white rounded-4 shadow-sm border p-4">
  ...
  <div className="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h5 className="fw-bold mb-1" style={{ color: "#2d3436" }}>Pending Grooming</h5>
      <p className="text-muted small mb-0">Manage and review pending incident assessments</p>
    </div>
    ...
    <div className="d-flex gap-2 align-items-center bg-light p-2 rounded-3 border">
      <Form.Select
        ...
        style={{ width: "200px", outline: "none", boxShadow: "none" }}
        value={selectedUser || ""}
        onChange={(e) => setSelectedUser(e.target.value)}
      >
        <option value="">-- Select User --</option>
  ...
  {loading ? (
    <div className="text-center py-5">
      <Spinner animation="border" variant="primary" />
      <p className="text-muted mt-2">Loading records...</p>
    </div>
  ) : (
    <div className="table-responsive">
      <Table hover className="align-middle premium-table">
            ...
            <th style={{ width: "120px" }} className="text-center">
              <div className="d-flex flex-column align-items-center">
                <Form.Check
                  ...
                  onChange={handleSelectAll}
                />
                <span style={{ fontSize: "10px", marginTop: "2px" }} className="text-muted text-uppercase fw-bold">Select All</span>
              </div>
            </th>
            <th className="fw-bold text-muted small text-uppercase">Incident No</th>
            <th className="fw-bold text-muted small text-uppercase">Short Description</th>
            <th className="fw-bold text-muted small text-uppercase">Handled On</th>
            <th className="fw-bold text-muted small text-uppercase">Handled By</th>
            <th className="fw-bold text-muted small text-uppercase">QC Date</th>
            <th className="fw-bold text-muted small text-uppercase">QC Analyst</th>
          </tr>
        </thead>
        <tbody>
          {incidents.length > 0 ? (
            incidents.map((incident, index) => (
              <tr key={incident.sid || index} className="hover-row">
                ...
                <td className="small text-muted">{incident.incident_date || "NA"}</td>
                <td>
                  <span className="badge bg-light text-dark border fw-medium px-2 py-1">
                    {incident.assigned_analyst || "N/A"}
                  </span>
                </td>
                <td className="small text-muted">{formatDate(incident.updated_at) || "N/A"}</td>
                <td>
                   <div className="d-flex align-items-center">
                      <div className="avatar-sm me-2">{incident.qc_analyst?.charAt(0) || "U"}</div>
                      <span className="small fw-medium">{incident.qc_analyst || "NA"}</span>
                   </div>
                </td>
              </tr>
            ))
            ...
            <tr>
              <td colSpan="7" className="text-center py-5 text-muted">No grooming incidents found.</td>
            </tr>
          )}
        </tbody>
      </Table>
  ...
  {/* ✅ Modal - KEEPING YOUR OLD STYLE EXACTLY */}
  <Modal show={showModal} onHide={handleCloseModal} size="lg" centered>
    <Modal.Header closeButton className="bg-primary text-white">
      <Modal.Title className="fw-bold d-flex justify-content-center align-items-center ">Incident Details</Modal.Title>
    </Modal.Header>
        ...
        <Form>
          ...
          <div className="mb-4 p-3 border rounded shadow-sm bg-light">
            ...
            <Row className="mb-2">
              <Col md={4}>
                <div className="info-field">
                  <span className="fw-bold me-2">QA Status:</span>
                  <span className={`badge rounded-pill px-3 py-2 fs-6 text-white ${formData.qa_status === "Pass" ? "bg-success" : formData.qa_status === "Fail" ? "bg-danger" : "bg-secondary"}`}>
                    {formData.qa_status || "N/A"}
                  </span>
                </div>
              </Col>
              ...
              <Col md={4}>
                <div className="info-field">
                  <span className="fw-bold me-2">QA Agent:</span>
                  <span>{formData.qc_analyst || user?.full_name || "N/A"}</span>
                </div>
              </Col>
            </Row>
            <Row className="mt-3 align-items-start">
              ...
              <Col md={4} className="text-end">
                <div className="info-field fw-semibold">
                  <span className="fw-bold me-2">Total Score:</span>
                  <span className="badge bg-primary rounded-pill px-3 py-2 fs-6">{totalScore || 0}</span>
                </div>
              </Col>
            </Row>
          </div>
          ...
          <div className="mb-4 p-3 border rounded shadow-sm bg-light">
            ...
            <Row className="mb-3">
              <Col md={4}>
                <Form.Group>
                  <Form.Label className="fw-bold">Grooming Done <span className="text-danger">*</span></Form.Label>
                  <Form.Check type="checkbox" name="grooming_done" checked={!!formData.grooming_done} onChange={handleChange} label="Mark as Done" isInvalid={!formData.grooming_done} />
                </Form.Group>
              </Col>
              <Col md={8}>
                <Form.Group>
                  <Form.Label className="fw-bold">Grooming Instructions</Form.Label>
                  <Form.Control type="text" name="grooming_instructions" value={formData.grooming_instructions || ""} onChange={handleChange} />
                </Form.Group>
              </Col>
            </Row>
            <Row>
              <Col md={12}>
                <Form.Group>
                  <Form.Label className="fw-bold">Audit Notes / Observations <span className="text-danger">*</span></Form.Label>
                  <Form.Control as="textarea" rows={3} name="notes" placeholder="Enter detailed feedback (Mandatory)..." value={formData.notes || ""} onChange={handleChange} isInvalid={formData.notes === ""} />
                </Form.Group>
              </Col>
            </Row>
          </div>
