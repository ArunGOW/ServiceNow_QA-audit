...
//     </div>
 ...
 <div className="bg-white rounded-4 shadow-sm border overflow-hidden">
  ...
  <div className="premium-banner d-flex justify-content-between align-items-center px-4 py-3">
    <div className="d-flex align-items-center">
      <div className="banner-icon-wrapper me-3">
        <i className="bi bi-clipboard-check-fill text-white"></i>
      </div>
      <div>
        <h5 className="fw-bold mb-0 text-white" style={{ letterSpacing: '0.5px' }}>Pending Grooming</h5>
        <span className="badge bg-white bg-opacity-25 text-white small" style={{ fontSize: '0.7rem' }}>
    ...
    <div className="d-flex gap-2 align-items-center bg-white bg-opacity-10 p-1 rounded-3 border border-white border-opacity-25">
      <Form.Select
        size="sm"
        className="border-0 bg-transparent text-white fw-medium custom-select-white"
        style={{ width: "160px", fontSize: '0.85rem' }}
        value={selectedUser || ""}
        onChange={(e) => setSelectedUser(e.target.value)}
      >
        <option value="" className="text-dark">Assign To...</option>
        {users.map((user) => (
          <option key={user.sid} value={user.sid} className="text-dark">
            {user.full_name}
          </option>
        ))}
      </Form.Select>
      <Button
        size="sm"
        variant="light"
        className="px-3 fw-bold text-primary shadow-sm py-1"
        disabled={selectedIncidents.length === 0 || !selectedUser}
        onClick={handleAssign}
        style={{ fontSize: '0.8rem', borderRadius: '5px' }}
      >
        Assign
      </Button>
    </div>
  ...
  {loading ? (
    ...
    <div className="table-responsive">
      <Table hover className="align-middle mb-0 compact-table">
        <thead>
          <tr>
            {/* ✅ Reduced Checkbox Space */}
            <th style={{ width: "40px" }} className="ps-3 pe-0">
              <Form.Check
                type="checkbox"
            ...
            <th className="text-muted small fw-bold">INCIDENT NO</th>
            <th className="text-muted small fw-bold">DESCRIPTION</th>
            <th className="text-muted small fw-bold">HANDLED ON</th>
            <th className="text-muted small fw-bold">HANDLED BY</th>
            <th className="text-muted small fw-bold">QC DATE</th>
            <th className="text-muted small fw-bold">QC ANALYST</th>
          </tr>
        </thead>
        <tbody>
          {incidents.length > 0 ? (
            incidents.map((incident, index) => (
              <tr key={incident.sid || index}>
                ...
                <td className="fw-bold py-2">
                  <span 
                    className="text-primary cursor-pointer hover-link"
                ...
                <td className="py-2">
                  <div className="text-truncate text-secondary" style={{ maxWidth: "200px", fontSize: "0.8rem" }}>
                    {incident.short_description || "N/A"}
                  </div>
                </td>
                <td className="small text-muted py-2" style={{ fontSize: '0.75rem' }}>
                  {incident.incident_date || "NA"}
                </td>
                <td className="py-2">
                  <span className="compact-analyst-tag">
                    {incident.assigned_analyst || "N/A"}
                  </span>
                </td>
                <td className="small text-muted py-2" style={{ fontSize: '0.75rem' }}>
                  {formatDate(incident.updated_at) || "N/A"}
                </td>
                <td className="py-2">
                   <div className="d-flex align-items-center">
                      <div className="avatar-xs me-2">{incident.qc_analyst?.charAt(0) || "U"}</div>
                      <span className="fw-medium" style={{ fontSize: '0.8rem' }}>{incident.qc_analyst || "NA"}</span>
                   </div>
                </td>
              </tr>
            ))
          ) : (
            <tr><td colSpan="7" className="text-center py-4 text-muted">No grooming incidents found.</td></tr>
          )}
        </tbody>
      </Table>
    </div>
  ...
  {/* Modal remains exactly as your original style below */}
</div>
