...
//     </div>
 ...
 <div className="bg-white rounded-4 shadow-sm border overflow-hidden">
  ...
  {loading ? (
    ...
    <div className="table-responsive">
      <Table hover className="align-middle mb-0 refined-table">
        ...
        {/* ✅ Modal */}
      <Modal show={showModal} onHide={handleCloseModal} size="xl" centered scrollable={false}>
  <Modal.Header closeButton className="bg-primary text-white py-2">
    <Modal.Title className="fw-bold w-100 text-center fs-5">Incident Assessment Details</Modal.Title>
  </Modal.Header>
  
  <Modal.Body className="bg-light px-4 py-3" style={{ maxHeight: '85vh', overflowY: 'auto' }}>
    {formData && (
      <Form>
        <Row className="g-3">
          {/* LEFT COLUMN: Incident Info & Grooming Action */}
          <Col md={6}>
            <div className="card shadow-sm border-0 h-100">
              <div className="card-body p-3">
                <h6 className="fw-bold border-bottom pb-2 mb-3 text-primary">Incident Information</h6>
                <div className="mb-2"><span className="fw-bold small text-muted text-uppercase">Incident:</span> <span className="ms-2 fw-medium text-dark">{formData.incident_number || "N/A"}</span></div>
                <div className="mb-2"><span className="fw-bold small text-muted text-uppercase">Description:</span> <p className="ms-0 mt-1 small text-secondary lh-sm">{formData.short_description || "N/A"}</p></div>
                
                <Row className="mt-3 g-2">
                  <Col xs={6}><div className="small"><span className="fw-bold d-block text-muted">Handled By:</span> {formData.assigned_analyst || "N/A"}</div></Col>
                  <Col xs={6}><div className="small"><span className="fw-bold d-block text-muted">Handled On:</span> {formatDate(formData.incident_date) || "N/A"}</div></Col>
                </Row>

                <h6 className="fw-bold border-bottom pb-2 mt-4 mb-3 text-primary">Grooming Actions</h6>
                <Form.Group className="mb-3">
                  <Form.Label className="fw-bold small">Audit Notes <span className="text-danger">*</span></Form.Label>
                  <Form.Control as="textarea" rows={4} name="notes" required isInvalid={formData.notes === ""} placeholder="Enter detailed feedback..." value={formData.notes || ""} onChange={handleChange} style={{ fontSize: '0.9rem' }} />
                </Form.Group>
                
                <Form.Group>
                  <Form.Check type="checkbox" name="grooming_done" id="groom-check" checked={!!formData.grooming_done} onChange={handleChange} label={<span className="fw-bold">Mark Grooming as Completed</span>} isInvalid={!formData.grooming_done} className="p-2 border rounded bg-white" />
                </Form.Group>
              </div>
            </div>
          </Col>

          {/* RIGHT COLUMN: QA Details (The "Big" Metrics) */}
          <Col md={6}>
            <div className="card shadow-sm border-0 h-100">
              <div className="card-body p-3">
                <h6 className="fw-bold border-bottom pb-2 mb-3 text-primary">QA Assessment Result</h6>
                
                {/* BIG STATUS AND SCORE SECTION */}
                <div className="d-flex justify-content-between align-items-center mb-4 p-3 rounded bg-white border">
                  <div className="text-center flex-grow-1 border-end">
                    <span className="fw-bold small text-muted d-block mb-2">QA STATUS</span>
                    <span className={`badge rounded-pill px-4 py-3 fs-4 shadow-sm text-uppercase ${formData.qa_status === "Pass" ? "bg-success" : "bg-danger"}`}>
                      {formData.qa_status || "FAIL"}
                    </span>
                  </div>
                  <div className="text-center flex-grow-1">
                    <span className="fw-bold small text-muted d-block mb-2">TOTAL SCORE</span>
                    <span className="text-primary fw-bold display-5 d-block" style={{ lineHeight: 1 }}>
                      {formData?.scores?.total_score_achieved ?? 0}
                    </span>
                  </div>
                </div>

                <div className="qa-comments-container p-2 rounded bg-light" style={{ maxHeight: '250px', overflowY: 'auto' }}>
                  <span className="fw-bold small text-muted mb-2 d-block">CATEGORY SCORES:</span>
                  {formData?.scores ? (
                    Object.entries(formData.scores).map(([key, value], idx) => {
                      if (key === "total_score_achieved") return null;
                      return (
                        <div key={idx} className="d-flex justify-content-between align-items-center mb-2 p-2 bg-white rounded border-start border-primary border-4 shadow-xs">
                          <span className="text-capitalize small fw-medium text-dark">{key.replace(/_/g, " ")}</span>
                          <span className="badge bg-soft-primary text-primary fw-bold">Score: {value.total || 0}</span>
                        </div>
                      );
                    })
                  ) : <div className="text-center text-muted small py-3">No scores available</div>}
                </div>
                
                <div className="mt-3 pt-2 border-top">
                   <div className="small d-flex justify-content-between">
                      <span className="text-muted">QA Agent: <strong>{formData.qc_analyst || "N/A"}</strong></span>
                      <span className="text-muted">Done On: <strong>{formatDate(formData.audit_date) || "N/A"}</strong></span>
                   </div>
                </div>
              </div>
            </div>
  ...
  <Modal.Footer className="py-2">
    <Button variant="outline-dark" onClick={handleCloseModal} className="px-4">Cancel</Button>
    <Button variant="success" onClick={handleUpdate} className="px-5 fw-bold shadow-sm">Save Assessment</Button>
  </Modal.Footer>
</Modal>
      <ToastContainer />
</div>
