...
//     </div>
 ...
 <div className="bg-white rounded-4 shadow-sm border overflow-hidden">
  ...
  <Modal.Header closeButton className="bg-primary text-white py-2">
    <Modal.Title className="fw-bold w-100 text-center fs-5">Incident Details</Modal.Title>
  </Modal.Header>
  <Modal.Body className="bg-light px-4">
    {formData && (
      <Form>
        {/* Incident Information - Clean & Compact */}
        <div className="mb-3 p-3 border-0 rounded shadow-sm bg-white">
          <h6 className="fw-bold mb-3 text-primary small text-uppercase" style={{ letterSpacing: '1px' }}>
            Incident Information
          </h6>
          <Row className="mb-2">
            <Col md={4}>
              <div className="info-label small text-muted fw-bold">INCIDENT NUMBER</div>
              <div className="fw-bold text-dark">{formData.incident_number || "N/A"}</div>
            </Col>
            <Col md={8}>
              <div className="info-label small text-muted fw-bold">DESCRIPTION</div>
              <div className="text-secondary small line-clamp-2">{formData.short_description || "N/A"}</div>
            </Col>
          </Row>
          <Row className="mt-3">
            <Col md={4}>
              <div className="info-label small text-muted fw-bold">HANDLED BY</div>
              <div className="text-dark">
                {formData.assigned_analyst 
                  ? formData.assigned_analyst.charAt(0).toUpperCase() + formData.assigned_analyst.slice(1).toLowerCase() 
                  : "N/A"}
              </div>
            </Col>
            <Col md={4}>
              <div className="info-label small text-muted fw-bold">HANDLED ON</div>
              <div className="text-dark">{formatDate(formData.incident_date) || "N/A"}</div>
            </Col>
            <Col md={4}>
              <div className="info-label small text-muted fw-bold">CURRENT STATUS</div>
              <span className="badge bg-secondary-subtle text-secondary border px-2">{formData.resolution_status || "N/A"}</span>
            </Col>
          </Row>
        </div>

        {/* QA Details - High Impact Status & Score */}
        <div className="mb-3 p-3 border-0 rounded shadow-sm bg-white">
          <h6 className="fw-bold mb-3 text-primary small text-uppercase" style={{ letterSpacing: '1px' }}>
            QA Assessment
          </h6>
          
          {/* Status & Total Score Row */}
          <div className="d-flex align-items-center justify-content-between bg-light p-3 rounded mb-4 border">
            <div className="d-flex flex-column align-items-center px-4 border-end">
              <span className="fw-bold small text-muted mb-2">QA STATUS</span>
            ...
            <div className="d-flex flex-column align-items-center px-4">
              <span className="fw-bold small text-muted mb-1">TOTAL SCORE</span>
              <span className="display-4 fw-bold text-primary" style={{ lineHeight: '1' }}>
                {formData?.scores?.total_score_achieved ?? 0}
              </span>
            </div>

            <div className="ms-auto text-end border-start ps-4">
              <div className="small mb-1"><span className="fw-bold text-muted">QA Agent:</span> {formData.qc_analyst || "N/A"}</div>
              <div className="small"><span className="fw-bold text-muted">Date:</span> {formatDate(formData.audit_date) || "N/A"}</div>
            </div>
          </div>

          <div className="info-field d-flex flex-column">
            <span className="fw-bold text-muted small mb-2">CATEGORY BREAKDOWN:</span>
            {formData?.scores ? (
              <div className="qa-comments-list-vertical">
                {Object.entries(formData.scores).map(([key, value], idx) => {
                  if (key === "total_score_achieved") return null;
                  return (
                    <div key={idx} className="d-flex justify-content-between align-items-center py-2 px-3 mb-2 bg-light rounded border-start border-primary border-3">
                      <span className="text-capitalize fw-medium text-dark">{key.replace(/_/g, " ")}</span>
                      <span className="badge bg-white text-primary border fw-bold px-3">Score: {value.total || 0}</span>
                    </div>
                  );
                })}
              </div>
            ) : (
              <div className="text-center py-2 text-muted italic">No category data available</div>
            )}
          </div>
        </div>

        {/* Grooming Details - Input Section */}
        <div className="mb-3 p-3 border-0 rounded shadow-sm bg-white">
          <h6 className="fw-bold mb-3 text-primary small text-uppercase" style={{ letterSpacing: '1px' }}>
            Grooming Actions
          </h6>
          <Row className="mb-3 g-3">
            <Col md={12}>
              <Form.Group>
                <Form.Label className="fw-bold small text-muted">AUDIT NOTES / OBSERVATIONS <span className="text-danger">*</span></Form.Label>
                <Form.Control
                  as="textarea"
                  rows={2}
                  name="notes"
                  required
                  isInvalid={formData.notes === ""}
            ...
            <Col md={8}>
              <Form.Group>
                <Form.Label className="fw-bold small text-muted">GROOMING INSTRUCTIONS</Form.Label>
                <Form.Control
                  type="text"
                  name="grooming_instructions"
                  value={formData.grooming_instructions || ""}
            ...
            <Col md={4} className="d-flex align-items-end pb-1">
              <Form.Check
                ...
                checked={!!formData.grooming_done}
                onChange={handleChange}
                label={<span className="fw-bold small">MARK AS DONE</span>}
                className="custom-check-premium"
              />
            </Col>
          </Row>
        </div>
  ...
  <Modal.Footer className="bg-white border-top-0">
    <Button variant="outline-dark" onClick={handleCloseModal} className="px-4 border-0">Close</Button>
    <Button variant="success" onClick={handleUpdate} className="px-5 fw-bold shadow-sm rounded-pill">Save Changes</Button>
  </Modal.Footer>
</Modal>
      <ToastContainer />
</div>
