...
import "../../styles/GroomingTable.css";
 ...
 const payload = [
  ...
  qc_analyst: user?.full_name || user?.name,
    ...
    scores: scoresPayload,
     sla_breached: formData.sla_breached ?? selectedIncident.sla_breached ?? false,
...
//     </div>
  ...
  <div className="premium-lite-banner d-flex justify-content-between align-items-center px-4 py-2 border-bottom">
    ...
    <div className="modern-count-badge py-1 px-3">
      <span className="badge-text" style={{ fontSize: '0.65rem' }}>TOTAL QUEUE</span>
      <span className="badge-number ms-2" style={{ fontSize: '0.8rem' }}>{incidents.length}</span>
    </div>
  </div>
  ...
  {loading ? (
    ...
    <div className="table-responsive">
      {/* Added 'compact-table' class for 10-row optimization */}
          ...
          {incidents.length > 0 ? (
            incidents.map((incident, index) => (
              <tr key={incident.sid || index}>
                ...
                <td>
                  <span 
                    className="incident-link fw-bold text-primary cursor-pointer d-flex align-items-center"
                    onClick={() => handleOpenModal(incident)}
                ...
                <td>
                  <div className="description-text text-secondary truncate-2-lines" title={incident.short_description}>
                ...
                <td><span className="date-text text-nowrap">{formatDate(incident.updated_at) || "—"}</span></td>
                <td>
                   <div className="d-flex align-items-center">
                      <div className="avatar-mini-sm me-2">{incident.qc_analyst?.charAt(0) || "U"}</div>
                      <span className="qc-name fw-semibold" style={{ fontSize: '0.8rem' }}>{incident.qc_analyst || "NA"}</span>
                   </div>
                </td>
              </tr>
            ))
          ) : (
            <tr><td colSpan="7" className="text-center py-5 text-muted small">No grooming records available.</td></tr>
          )}
        </tbody>
      </Table>
    </div>
  ...
  <Col md={2} className="text-end">
    <div className="info-field fw-semibold">
      <span className="fw-bold me-2 d-block mb-1 text-muted small">TOTAL ACHIEVED</span>
 ...
 <Col md={2}>
  <div className="info-field">
    <span className="fw-bold me-2">SLA Breached:</span>
    <span>{formData.sla_breached ?? selectedIncident.sla_breached}</span>
  </div>
</Col>
</Row>
