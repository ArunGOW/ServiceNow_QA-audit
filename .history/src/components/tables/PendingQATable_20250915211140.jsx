...
// const PendingQATable = ({ incidents, loading, refreshData }) => {
//   const { user } = useAuth();
//   const [selectedIncident, setSelectedIncident] = useState(null);
//   const [showModal, setShowModal] = useState(false);
//   const [isEditing, setIsEditing] = useState(false);
...
//         `http://localhost:8000/api/users/update-incident/${selectedIncident.sid}`,
//         {
//           ...formData,
//           user_sid: user?.user_sid, // ✅ include user context if required
//         }
//       );
...
//             </Button>
//           ) : (
//             <Button variant="success" onClick={handleUpdate} disabled={saving}>
//               {saving ? "Saving..." : "Save"}
//             </Button>
//           )}
//           <Button variant="outline-dark" onClick={handleCloseModal}>
          ...
          {incidents.length > 0 ? (
            incidents.map((incident, index) => (
              <tr key={incident.sid || index}>
                ...
                <td>{incident.incident_date}</td>
                <td>{incident.audit_status || incident.resolution_status}</td>
              </tr>
            ))
          ) : (
            <tr>
