...
const PendingQATable = ({ incidents, loading }) => {
  ...
  const handleAssign = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:8000/api/users/assign/incidents/", {
        user_id: selectedUser, // based on API requirement (user_sid or id)
  ...
  <div className="d-flex flex-wrap gap-3">
    {users.map((user) => (
      <Form.Check
        key={user.user_sid || user.id}
        type="radio"  // 👉 use "radio" if only one user can be assigned
        // type="checkbox" 👉 if multiple users allowed
        name="userSelect"
        id={`user-${user.user_sid || user.id}`}
        label={user.name || user.username}
        value={user.user_sid || user.id}
        checked={selectedUser === (user.user_sid || user.id)}
