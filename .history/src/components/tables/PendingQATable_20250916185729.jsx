...
const PendingQATable = ({ incidents, loading }) => {
  ...
  // ✅ Select all
  // ✅ Select individual incident by SID instead of incident_number
...
const handleSelectAll = () => {
  if (selectedIncidents.length === incidents.length) {
    ...
    setSelectedIncidents(incidents.map((i) => i.sid)); // use sid not number
  }
};
 ...
 const handleAssign = async () => {
  ...
  try {
    const res = await axios.post("http://localhost:8000/api/users/assign/incidents/", {
      assigned_by_sid: "a17692bf-2164-4929-90f2-8b25f65c9445", // TODO: replace with logged-in user sid
      assigned_to_sid: selectedUser,
      incident_sid: selectedIncidents, // <-- make sure this contains incident.sids, not incident_number
    });
    ...
    if (error.response) {
      console.error("❌ Error assigning incidents:", error.response.data);
