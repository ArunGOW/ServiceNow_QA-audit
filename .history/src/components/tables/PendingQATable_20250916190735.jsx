...
const PendingQATable = ({ incidents, loading }) => {
  ...
  const loggedUserSid = user?.sid; // <-- sid from login response
 ...
 const handleAssign = async () => {
  ...
  try {
    const res = await axios.post("http://localhost:8000/api/users/assign/incidents/", {
      assigned_by_sid: loggedUserSid,   // ✅ from AuthContext
      assigned_to_sid: selectedUser,    // ✅ from dropdown
      incident_sid: selectedIncidents,  // ✅ must be incident.sids
    });
