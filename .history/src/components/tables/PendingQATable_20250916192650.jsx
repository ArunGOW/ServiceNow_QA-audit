...
const PendingQATable = ({ incidents, loading }) => {
  ...
  const { user } = useAuth();   // <-- get logged-in user from context
   const loggedUserSid = user?.user_sid;   // <-- sid from login response
 ...
 const handleAssign = async () => {
  ...
  try {
    const res = await axios.post("http://localhost:8000/api/users/assign/incidents/", {
      assigned_by_sid: loggedUserSid,   // ✅ now correct
      assigned_to_sid: selectedUser,
      incident_sid: selectedIncidents,  // ✅ make sure this is incident.sids
    });
