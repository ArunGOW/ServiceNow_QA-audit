...
const PendingQATable = ({ incidents, loading, refresh }) => {
  ...
  useEffect(() => {
    if (Array.isArray(incidents)) {
      setLocalIncidents(
        incidents.map((i) => ({ ...i, sid: i.sid || i.incident_sid }))
      );
    }
  }, [incidents]);
  ...
  try {
    // Loop through selected incidents
    for (let sid of selectedIncidents) {
      await assignIncident(sid, selectedUser);
    }
    alert("✅ Incidents assigned successfully!");
    setSelectedIncidents([]);
    setSelectedUser(null);
    ...
    alert("Failed to assign incidents. Check console for details.");
  }
};
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div className="d-flex gap-2">
          ...
          <Button
            size="sm"
            variant="primary"
            disabled={selectedIncidents.length === 0 || !selectedUser || assigning}
            onClick={handleAssign}
          >
