...
//   // ✅ Always exclude resolved incidents from local state
//   useEffect(() => {
//     const filtered = incidents.filter(
//       (i) => i.resolution_status?.toLowerCase() !== "resolved"
//     );
...
//       toast.success(
//         `✅ ${res.data.message || "Incident updated successfully"}`
//       );

//       // ✅ Remove resolved incidents from table
//       if (formData.resolution_status?.toLowerCase() === "resolved") {
//         setLocalIncidents((prev) =>
//           prev.filter((i) => i.sid !== formData.incident_sid)
//         );
...
<div className="mb-4 p-3 border rounded shadow-sm bg-light">
  ...
  {/* Quality Check Points Dropdown */}
  <Row className="mb-3 align-items-center">
    <Col md={6}>
      <Form.Group>
        <Form.Label className="fw-bold">Quality Check Comment</Form.Label>
        <Form.Select
          value={selectedComment}
          onChange={(e) => {
            const comment = e.target.value;
            setSelectedComment(comment);
            const found = qualityPoints.find((item) => item.comment === comment);
            setSelectedScore(found ? found.score : 0);
          }}
          className="border border-secondary"
        >
