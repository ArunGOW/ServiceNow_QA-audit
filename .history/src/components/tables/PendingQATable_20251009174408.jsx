...
<div className="mb-4 p-3 border rounded shadow-sm bg-light">
  ...
  <Form.Group className="mb-3">
    <Form.Label className="fw-bold">Select QA Comments</Form.Label>
    <Form.Select
      multiple
      value={selectedComments.map(c => c.sid)}
      onChange={handleDropdownChange}
      className="border border-secondary"
    >
      {qualityPoints.map((item) => (
        <option key={item.sid} value={item.sid}>
          {item.check_point_name} ({item.point_value} pts)
        </option>
      ))}
    </Form.Select>
    <Form.Text className="text-muted">Hold Ctrl (Windows) or Cmd (Mac) to select multiple options</Form.Text>
  </Form.Group>
