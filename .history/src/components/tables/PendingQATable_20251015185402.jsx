...
//   const filtered = incidents.filter(
//     (i) => i.resolution_status?.trim().toLowerCase() !== "resolved"
//   );
...
//       prev.every((p, idx) => p.sid === filtered[idx].sid);
...
//     // Dynamic payload merging edited formData with original incident values
//     const payload = [
//       {
...
//         kba_resolution_needed: formData.kba_resolution_needed ?? selectedIncident.kba_resolution_needed ?? false,
//         kba_resolution_updated: formData.kba_resolution_updated ?? selectedIncident.kba_resolution_updated ?? false,
//         updated_at: new Date().toISOString(),
//         scores: selectedComments.reduce((acc, item) => {
...
//   const status = i.resolution_status?.trim().toLowerCase();
//   return status !== "resolved"; // keeps even empty or undefined ones
// });
// setLocalIncidents(filtered);

// console.log("Local incidents count:", localIncidents.length);
// console.log("Incoming incidents:", incidents.map(i => i.resolution_status));
...
//           <option value="Fail">f ail</option>
//         </Form.Select>
//       </Form.Group>
//     </Col>
//   <Col md={4}>
...
const PendingQATable = ({ incidents, loading, refresh }) => {
  ...
  // Filter unresolved incidents safely
  useEffect(() => {
    if (!Array.isArray(incidents) || incidents.length === 0) {
      setLocalIncidents([]);
      console.log("Local incidents count: 0");
      return;
    }
    ...
    console.log("Local incidents count:", filtered.length);
    console.log("Incoming incidents:", incidents.map((i) => i.resolution_status));
  }, [incidents]);
  ...
  useEffect(() => {
    const fetchQualityPoints = async () => {
      try {
        const res = await api.get("/api/users/quality-check-points");
        const points = Array.isArray(res.data)
          ? res.data
          : res.data?.data || res.data?.results || [];
        setQualityPoints(points);
  ...
  // Update formData QA analyst automatically when modal opens
  useEffect(() => {
    if (user && showModal) {
      setFormData((prev) => ({
        ...prev,
  ...
  // Open modal and prefill formData
  const handleOpenModal = (incident) => {
    setSelectedIncident(incident);
    ...
    if (incident.qa_comment) {
      ...
      setSelectedComments(preSelected);
      setTotalScore(preSelected.reduce((sum, item) => sum + (item.point_value || 0), 0));
    } else {
      setSelectedComments([]);
      setTotalScore(0);
    }
  ...
  const handleAssign = async () => {
    ...
    try {
      await api.post("/users/assign/incidents/", {
        assigned_by_sid: loggedUserSid,
        assigned_to_sid: selectedUser,
        incident_sid: selectedIncidents,
      });
...
// const PendingQATable = ({ incidents, loading, refresh }) => {
//   const { user } = useAuth();
//   const loggedUserName = user?.full_name;
//   const loggedUserSid = user?.user_sid;
...
//         setQualityPoints(points);
//       } catch (error) {
//         console.error("Error fetching QA points:", error.response?.data || error.message);
//         toast.error("Failed to load quality check points!");
//       }
//     };
//     fetchQualityPoints();
...
//     // Preselect comments if available
//     if (incident.qa_comment) {
//       const names = incident.qa_comment.split(",").map((n) => n.trim());
//       const preSelected = qualityPoints.filter((q) => names.includes(q.check_point_name));
//       setSelectedComments(preSelected);
...
//   // Toggle QA comment selection
//   const handleCommentToggle = (comment) => {
//     setSelectedComments((prev) => {
//       const updated = prev.find((c) => c.sid === comment.sid)
//         ? prev.filter((c) => c.sid !== comment.sid)
//         : [...prev, comment];
//       setTotalScore(updated.reduce((sum, item) => sum + (item.point_value || 0), 0));
//       return updated;
//     });
//   };
...
//   const handleAssign = async () => {
//     if (!selectedUser) return toast.warn("Please select a user first.");
//     if (selectedIncidents.length === 0) return toast.warn("Select at least one incident.");
//     if (!loggedUserSid) return toast.error("Logged-in user SID not found.");
...
//                 {/* QA Comments and Total Score in one row */}
//                 <Row className="mb-3 align-items-end">
//                   <Col md={8}>
//                     <Dropdown
//                       show={dropdownOpen}
