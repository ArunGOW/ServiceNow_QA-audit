...
const PendingQATable = ({ incidents, loading, refresh }) => {
  ...
  useEffect(() => {
    const fetchQualityPoints = async () => {
      try {
        const res = await api.get("/users/quality-check-points");
        setQualityPoints(Array.isArray(res.data) ? res.data : res.data?.data || []);
      } catch {
        toast.error("Failed to load quality check points!");
      }
    };
  ...
  const handleCommentToggle = (comment) => {
    setSelectedComments((prev) => {
      const updated = prev.find((c) => c.sid === comment.sid)
        ...
        : [...prev, comment];
      setTotalScore(updated.reduce((sum, i) => sum + (i.point_value || 0), 0));
      return updated;
    });
  };
  ...
  const handleOpenModal = (incident) => {
    ...
    if (incident.qa_comment) {
      ...
      setSelectedComments(preSelected);
      setTotalScore(preSelected.reduce((sum, i) => sum + (i.point_value || 0), 0));
    } else {
      setSelectedComments([]);
      setTotalScore(0);
    }
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "grooming_needed" ? value === "Yes" : type === "checkbox" ? checked : value,
    }));
  };
  ...
  const formatDate = (dateStr) => {
    ...
    return dateStr.includes("-")
      ? dateStr.split("T")[0]
      : new Date(dateStr.split("/").reverse().join("/")).toISOString().split("T")[0];
  };
    ...
    const payload = [
      {
        ...
        audit_date: new Date().toISOString(),
        scores: selectedComments.reduce((acc, item) => {
          acc[item.check_point_name] = { description: item.check_point_description, point_value: item.point_value };
          return acc;
        }, {}),
      },
    ];
  ...
  return (
    <div className="bg-white rounded shadow-sm p-3">
      <ToastContainer position="top-right" autoClose={2500} hideProgressBar />
      <h5 className="fw-bold mb-3 text-center">Pending QA</h5>
            ...
            <tr>
              <th style={{ width: "65px" }}>
                <Form.Check
                  ...
                  checked={selectedIncidents.length === incidents.length && incidents.length > 0}
                  onChange={(e) =>
                    setSelectedIncidents(e.target.checked ? incidents.map((i) => i.sid) : [])
                  }
                />
                <span className="ms-1">Select All</span>
              </th>
        ...
        <Modal.Body>
          {formData && (
            <Form>
              ...
              <Row className="mb-3">
                <Col md={4}>
                  <Form.Label className="fw-bold">Incident No</Form.Label>
                  <Form.Control readOnly value={formData.incident_number || "N/A"} />
                </Col>
                <Col md={8}>
                  <Form.Label className="fw-bold">Description</Form.Label>
                  <Form.Control readOnly value={formData.short_description || "N/A"} />
                </Col>
              </Row>
              ...
              <Row className="mb-3">
                <Col md={4}>
                  <Form.Label className="fw-bold">QA Status</Form.Label>
                  <Form.Select name="qa_status" value={formData.qa_status || ""} onChange={handleChange}>
                ...
                <Col md={4}>
                  <Form.Label className="fw-bold">QA Done On</Form.Label>
                  <Form.Control type="date" readOnly value={formatDate(formData.audit_date)} />
                </Col>
                <Col md={4}>
                  <Form.Label className="fw-bold">QA Agent</Form.Label>
                  <Form.Control
              ...
              <Row className="mb-3">
                <Col md={8}>
                  <Form.Label className="fw-bold">QA Comments</Form.Label>
                  <Dropdown show={dropdownOpen} onToggle={() => setDropdownOpen(!dropdownOpen)}>
                    <Dropdown.Toggle className="w-100 text-truncate">
                ...
                <Col md={4}>
                  <Form.Label className="fw-bold">Total Score</Form.Label>
                  <Form.Control readOnly value={totalScore} />
                </Col>
              </Row>
