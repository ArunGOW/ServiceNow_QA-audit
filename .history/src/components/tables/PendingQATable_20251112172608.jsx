...
const PendingQATable = ({ incidents, loading, refresh }) => {
  ...
  // Load QA quality points
  useEffect(() => {
    const fetchQualityPoints = async () => {
      try {
        const res = await api.get("/users/quality-check-points");
        const points = Array.isArray(res.data) ? res.data : res.data?.data || res.data?.results || [];
        setQualityPoints(points);
      } catch (error) {
        console.error("Error fetching quality points:", error.response?.data || error.message);
        toast.error("Failed to load quality check points!");
  ...
  // Toggle QA comment selection and update total score
  const handleCommentToggle = (comment) => {
    setSelectedComments((prev) => {
  ...
  // Open modal with selected incident details
  const handleOpenModal = (incident) => {
    setSelectedIncident(incident);
    setFormData({
      ...incident,
  ...
  return (
    <div className="bg-white rounded shadow-sm p-3">
      ...
      <Modal show={showModal} onHide={handleCloseModal} size="lg" centered>
        ...
        <Modal.Body>
          {formData && <Form>{/* Incident, QA, Grooming sections rendered here */}</Form>}
        </Modal.Body>
        <Modal.Footer>
          <Button variant="success" onClick={handleUpdate}>
            Save
