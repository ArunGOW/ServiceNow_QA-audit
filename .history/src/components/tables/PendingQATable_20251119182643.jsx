...
//       const scoreSum = updated.reduce(
//         (sum, item) => sum + (item.point_value || 0),
//         0
//       );
//       setTotalScore(scoreSum);

//       logger.info("Updated QA comment list", updated);
...
//   // Close modal and reset
//   const handleCloseModal = () => {
//     logger.info("Closing incident modal");
...
//       toast.warn("Incident SID missing!");
//       logger.warn("Attempted update with missing incident SID.");
//       return;
//     }
...
//       logger.info("Update API Success:", res.data);
...
const PendingQATable = ({ incidents, loading, refresh }) => {
  ...
  const handleCommentToggle = (comment) => {
    setSelectedComments((prev) => {
      ...
      logger.info("Updated selected QA comments:", updated);
      logger.info("Updated total score:", scoreSum);
  ...
  const handleOpenModal = (incident) => {
    ...
    if (incident.qa_comment) {
      ...
      setSelectedComments(preSelected);
      const scoreSum = preSelected.reduce((sum, item) => sum + item.point_value, 0);
      setTotalScore(scoreSum);

      logger.info("Preselected QA comments:", preSelected);
      logger.info("Preselected QA total score:", scoreSum);
    } else {
      setSelectedComments([]);
      setTotalScore(0);
    }
  ...
  const handleChange = (e) => {
    ...
    logger.info(`Field updated: ${name} → ${finalValue}`);
  };
  ...
  const handleUpdate = async () => {
    if (!formData?.incident_sid) {
      toast.warn("Incident SID missing!");
      logger.warn("Incident SID missing!");
      return;
    }
