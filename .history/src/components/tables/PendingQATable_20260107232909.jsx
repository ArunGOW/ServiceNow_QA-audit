...
const PendingQATable = ({ incidents, loading, refresh }) => {
  ...
  <Form.Label className="fw-bold">QA Comments & Points</Form.Label>
  <div className="border rounded p-3 bg-light" style={{ maxHeight: "400px", overflowY: "auto" }}>
    {qualityPoints.map((category) => (
      <div key={category.sid} className="mb-3 pb-2 border-bottom">
        <div className="d-flex justify-content-between align-items-center mb-1">
          <span className="fw-bold text-primary">{category.check_point_name}</span>
          <small className="text-muted">Total Category: {category.point_value} pts</small>
        </div>
            ...
            <Form.Check
              ...
              label={
                <div className="d-flex justify-content-between w-100">
                  <span>{sub.sub_check_point_name} <small className="text-muted">({sub.sub_check_point_description})</small></span>
                  <span className="badge bg-info ms-2">+{sub.max_points}</span>
                </div>
              }
...
{/* Score Section */}
<div className="d-flex justify-content-end align-items-center mt-2 p-2 bg-white border rounded">
  <h6 className="mb-0 me-2">Total Score:</h6>
  <span className="badge bg-success fs-6">{totalScore}</span>
</div>
                  </Col>
