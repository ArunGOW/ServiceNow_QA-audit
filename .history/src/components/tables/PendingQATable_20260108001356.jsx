...
scores: selectedComments.reduce((acc, item) => {
  ...
  <Form.Label className="fw-bold">Quality Check Points</Form.Label>
  
  {/* Category Selection Dropdown */}
  <Dropdown className="w-100 mb-3">
    <Dropdown.Toggle variant="outline-primary" id="dropdown-basic" className="w-100 text-start d-flex justify-content-between align-items-center">
      {selectedUser ? selectedUser.check_point_name : "Select QA Category to View Sub-points"}
    </Dropdown.Toggle>

    <Dropdown.Menu className="w-100">
      {qualityPoints.map((category) => {
        const count = selectedComments.filter(c => c.parent_sid === category.sid).length;
        return (
          <Dropdown.Item 
            key={category.sid} 
            onClick={() => setSelectedUser(category)} // Reusing selectedUser state or use a new one like 'activeCategory'
            className="d-flex justify-content-between align-items-center"
          >
            {category.check_point_name}
            {count > 0 && <span className="badge bg-primary rounded-pill">{count}</span>}
          </Dropdown.Item>
        );
      })}
    </Dropdown.Menu>
  ...
  {/* Display Sub-points for the Selected Category */}
  {selectedUser && (
    <div className="p-3 border rounded bg-white shadow-sm">
      <div className="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
        <h6 className="mb-0 text-primary fw-bold">{selectedUser.check_point_name}</h6>
        <small className="text-muted">Max Points: {selectedUser.point_value}</small>
      </div>
      
      {selectedUser.sub_categories?.map((sub) => (
        <div key={sub.sid} className="mb-2 p-2 border-bottom last-child-0">
          <Form.Check
            type="checkbox"
            id={`sub-${sub.sid}`}
            label={
              <div className="d-flex justify-content-between align-items-center w-100">
                <div className="me-3">
                  <div className="fw-bold small">{sub.sub_check_point_name}</div>
                  <div className="text-muted extra-small" style={{ fontSize: '0.75rem' }}>
                    {sub.sub_check_point_description}
                  </div>
                </div>
                <span className="badge bg-info text-dark">+{sub.max_points}</span>
              </div>
            }
            checked={selectedComments.some((c) => c.sid === sub.sid)}
            onChange={() => handleCommentToggle(selectedUser, sub)}
          />
        </div>
      ))}
    </div>
...
{/* Total Score Footer */}
<div className="mt-3 p-3 bg-light border rounded d-flex justify-content-between align-items-center">
  <span className="fw-bold">Total Score:</span>
  <span className={`h5 mb-0 ${totalScore > 0 ? 'text-success' : 'text-muted'}`}>
    {totalScore} <small>Points</small>
  </span>
</div>
