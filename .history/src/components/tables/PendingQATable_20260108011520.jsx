...
<div className="mt-4 p-2 bg-dark text-white rounded d-flex justify-content-between px-3">
 ...
 <Form.Group className="mb-3">
  <Form.Label className="fw-bold">QA Comments & Sub-Points</Form.Label>
  
  {/* Display the summary input field as you requested earlier */}
  <Form.Control
    type="text"
    readOnly
    className="bg-light mb-3"
    value={[...new Set(selectedComments.map(c => c.parent_name))].join(", ")}
  />

  <div className="border rounded bg-white">
    {qualityPoints.map((category) => {
      const isExpanded = activeCategory?.sid === category.sid;
      const hasSelections = selectedComments.some(c => c.parent_sid === category.sid);

      return (
        <div key={category.sid} className="border-bottom">
          {/* Main Category Header - Always Visible */}
          <div 
            className={`d-flex justify-content-between align-items-center p-2 ${hasSelections ? 'bg-light text-primary' : ''}`}
            style={{ cursor: 'pointer', transition: '0.2s' }}
            onClick={() => setActiveCategory(isExpanded ? null : category)}
          >
            <div className="fw-bold">
              {category.check_point_name}
              {hasSelections && <span className="ms-2 badge bg-success small">Selected</span>}
            </div>
            <Button variant="outline-secondary" size="sm" className="py-0 px-2">
              {isExpanded ? "Hide Sub-points" : "View Sub-points"}
            </Button>
          </div>

          {/* Sub-categories - Only shows when this specific category is expanded */}
          {isExpanded && (
            <div className="p-3 bg-white border-top">
                  ...
                  label={
                    <div className="d-flex justify-content-between w-100 small">
                      <span>
                        ...
                        <div className="text-muted" style={{ fontSize: '0.75rem' }}>{sub.sub_check_point_description}</div>
                      </span>
                      <span className="text-success fw-bold ms-2">+{sub.max_points}</span>
                    </div>
                  }
              ...
              <div className="text-end mt-2">
                <Button variant="link" size="sm" className="text-decoration-none p-0" onClick={() => setActiveCategory(null)}>
                  Close Section
                </Button>
              </div>
            </div>
...
<Form.Group className="mb-3">
      <Form.Label className="fw-bold text-primary">Quality Check Points</Form.Label>
      
      <Dropdown className="w-100" autoClose="outside">
        {/* The Toggle displays the selected main category names */}
        <Dropdown.Toggle 
          variant="white" 
          className="w-100 border text-start d-flex justify-content-between align-items-center bg-white shadow-sm"
          style={{ minHeight: '45px', whiteSpace: 'normal', color: '#495057' }}
        >
          <div className="text-truncate" style={{ maxWidth: '92%' }}>
            {[...new Set(selectedComments.map(c => c.parent_name))].join(", ") || "Select QA Categories..."}
          </div>
        </Dropdown.Toggle>

        <Dropdown.Menu className="w-100 shadow-lg">
          <Dropdown.Header>Select Category to add Points</Dropdown.Header>
          {qualityPoints.map((category) => {
            const isSelected = selectedComments.some(c => c.parent_sid === category.sid);
            return (
              <Dropdown.Item 
                key={category.sid} 
                onClick={() => setActiveCategory(category)}
                className={`d-flex justify-content-between align-items-center py-2 ${activeCategory?.sid === category.sid ? 'bg-primary text-white' : ''}`}
              >
                <span>
                  {category.check_point_name}
                  {isSelected && <i className="ms-2 text-success small">●</i>}
                </span>
                {isSelected && <span className="badge bg-info text-dark small">Selected</span>}
              </Dropdown.Item>
            );
          })}
        </Dropdown.Menu>
      ...
      {/* Sub-points panel: Appears when a category is selected from the dropdown */}
      {activeCategory && (
        <div className="mt-3 p-3 border-start border-primary border-4 rounded bg-light shadow-sm animate-fade-in">
          <div className="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
            <h6 className="mb-0 fw-bold text-primary">{activeCategory.check_point_name}</h6>
            <Button size="sm" variant="outline-secondary" className="border-0" onClick={() => setActiveCategory(null)}>
              Close
            </Button>
          </div>
          ...
          <div style={{ maxHeight: "200px", overflowY: "auto" }} className="px-2">
            {activeCategory.sub_categories?.map((sub) => (
              <Form.Check
                key={sub.sid}
                type="checkbox"
                id={`sub-check-${sub.sid}`}
                className="mb-2"
                label={
                  <div className="d-flex justify-content-between w-100 align-items-center">
                    <div>
                      <span className="fw-semibold d-block">{sub.sub_check_point_name}</span>
                      <small className="text-muted">{sub.sub_check_point_description}</small>
                    </div>
                    <span className="badge bg-white text-primary border border-primary ms-3">+{sub.max_points}</span>
                  </div>
                }
                checked={selectedComments.some((c) => c.sid === sub.sid)}
                onChange={() => handleCommentToggle(activeCategory, sub)}
              />
            ))}
          </div>
          <div className="text-end mt-3">
            <Button size="sm" variant="primary" onClick={() => setActiveCategory(null)}>
              Done with Section
            </Button>
          </div>
        </div>
