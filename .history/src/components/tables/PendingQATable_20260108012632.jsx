...
<div className="mt-4 p-2 bg-dark text-white rounded d-flex justify-content-between px-3">
 ...
 {/* QA CATEGORY DROPDOWN */}
    <Form.Group className="mb-3">
      <Form.Label className="fw-bold text-primary">QA Comments</Form.Label>
      
      <Dropdown className="w-100" autoClose="outside">
        {/* Toggle styled as an Input Field */}
        <Dropdown.Toggle 
          variant="white" 
        ...
        >
          <div className="text-truncate" style={{ maxWidth: '90%', color: '#495057' }}>
            {[...new Set(selectedComments.map(c => c.parent_name))].join(", ") || "Select QA Categories..."}
          </div>
        </Dropdown.Toggle>

        {/* Dropdown Menu showing Main Categories */}
        <Dropdown.Menu className="w-100 shadow-lg p-0" style={{ maxHeight: '400px', overflowY: 'auto' }}>
          {qualityPoints.map((category) => {
            ...
            return (
              <div key={category.sid} className="border-bottom">
                {/* Main Category Header inside Dropdown */}
                <div 
                ...
                >
                  <span className="fw-bold">
                    ...
                    {hasSelections && !isExpanded && <i className="ms-2 text-success small">✔</i>}
                  </span>
                  <small>{isExpanded ? '▲' : '▼'}</small>
                </div>

                {/* Sub-categories visible only when Main Category is clicked */}
                {isExpanded && (
                  <div className="p-3 bg-light border-top">
                    {category.sub_categories?.map((sub) => (
                      <Form.Check
                        key={sub.sid}
                        type="checkbox"
                        id={`drop-check-${sub.sid}`}
                        className="mb-2"
                        label={
                          <div className="d-flex justify-content-between w-100 small">
                            <span>{sub.sub_check_point_name}</span>
                            <span className="badge bg-white text-dark border ms-2">+{sub.max_points}</span>
                          </div>
                        }
