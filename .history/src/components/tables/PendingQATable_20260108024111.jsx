...
scores: selectedComments.reduce((acc, item) => {
  ...
  <Modal.Header closeButton style={{ 
    background: 'linear-gradient(90deg, #007bff 0%, #29d0f1 100%)', 
    borderBottom: 'none' 
  }}>
    <Modal.Title className="fw-bold text-white shadow-sm">Incident Details</Modal.Title>
  </Modal.Header>

  <Modal.Body className="bg-white p-4">
    {formData && (
      <Form>
        {/* Incident Info Section - Card Style */}
        <div className="mb-4 p-4 border-0 rounded-4 shadow-sm" style={{ backgroundColor: '#f8f9fa', border: '1px solid #e9ecef' }}>
          <h6 className="fw-bold mb-3 text-dark opacity-75">Incident Information</h6>
          <Row className="gy-3">
            <Col md={4}>
              <div className="info-field">
                <span className="text-muted small d-block">Incident Number</span>
                <span className="fw-bold">{formData.incident_number || "N/A"}</span>
              </div>
            </Col>
            <Col md={8}>
              <div className="info-field">
                <span className="text-muted small d-block">Description</span>
                <span className="text-dark">{formData.short_description || "N/A"}</span>
              </div>
            </Col>
            <Col md={4}>
              <div className="info-field">
                <span className="fw-bold mb-2 d-block small">Handled By:</span>
                {loadingUsers ? (
                  <Spinner animation="border" size="sm" />
                ) : (
                  <Form.Select
                  ...
                  >
                    <option value="">Select Agent</option>
                    {users.map((agent, index) => (
                      <option key={index} value={agent.full_name}>{agent.full_name}</option>
                    ))}
                  </Form.Select>
                )}
              </div>
            ...
            <Col md={4}>
              <div className="info-field">
                <span className="text-muted small d-block">Handled On</span>
                <span className="fw-semibold">{formatDate(formData.incident_date) || "N/A"}</span>
              </div>
            </Col>
            <Col md={4}>
              <div className="info-field">
                <span className="text-muted small d-block">Current Status</span>
                <span className="badge bg-info-subtle text-info px-3 py-2 rounded-pill">{formData.resolution_status || "resolved"}</span>
              </div>
            </Col>
          </Row>
        </div>
        ...
        {/* QA Details Section */}
        <div className="mb-4 p-4 rounded-4 shadow-sm" style={{ backgroundColor: '#ffffff', border: '1px solid #dee2e6' }}>
          <h6 className="fw-bold mb-3 text-dark opacity-75">QA Details</h6>
          <Row className="align-items-center">
            <Col md={4}>
              <Form.Group className="mb-3">
                <Form.Label className="fw-bold small">QA Done On</Form.Label>
                <Form.Control type="date" value={formatDate(formData.audit_date)} readOnly className="bg-light border-0" />
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Group className="mb-3">
                <Form.Label className="fw-bold small">QA Agent</Form.Label>
                <Form.Control type="text" value={formData.qc_analyst || "Arun Kumar"} readOnly className="bg-light border-0" />
              </Form.Group>
            </Col>
            <Col md={4}>
              {/* Modernized Score Display */}
              <div className="p-3 rounded-3 shadow-lg d-flex justify-content-between align-items-center" 
                   style={{ background: '#212529', color: '#fff' }}>
                <span className="small opacity-75">Total Score:</span>
                <span className="h4 mb-0 fw-bold text-warning">{totalScore}</span>
              </div>
            </Col>
          </Row>

          <Form.Group className="mt-3">
            <Form.Label className="fw-bold text-primary small">Quality Check Categories</Form.Label>
            <Dropdown className="w-100 shadow-sm rounded-3">
              <Dropdown.Toggle variant="white" className="w-100 border text-start d-flex justify-content-between align-items-center">
                <span className="text-muted">Incident Resolution Accuracy, Communication...</span>
              </Dropdown.Toggle>
              {/* ... Rest of your Dropdown Menu logic ... */}
            </Dropdown>
          </Form.Group>
        </div>

        {/* Grooming Details Section */}
        <div className="p-4 rounded-4 shadow-sm" style={{ backgroundColor: '#ffffff', border: '1px solid #dee2e6' }}>
          <h6 className="fw-bold mb-3">Grooming Details</h6>
          <Row>
            <Col md={6}>
              <Form.Group className="mb-3">
                <Form.Label className="fw-bold small">Grooming Instructions</Form.Label>
                <Form.Control as="textarea" rows={2} className="border-secondary-subtle" />
              </Form.Group>
            </Col>
            <Col md={6} className="d-flex align-items-center">
               <Form.Check type="checkbox" id="grooming" label="Mark as Done" className="fw-semibold text-muted" />
            </Col>
          </Row>
          <Form.Group className="mt-2">
            <Form.Label className="fw-bold small">Audit Notes / Observations</Form.Label>
            <Form.Control as="textarea" rows={3} placeholder="Enter feedback..." className="border-secondary-subtle" />
          </Form.Group>
        </div>
      </Form>
    )}
  ...
  <Modal.Footer className="border-0 px-4 pb-4">
    <Button variant="outline-dark" className="px-4 rounded-pill" onClick={handleCloseModal}>Close</Button>
    <Button variant="success" className="px-5 rounded-pill shadow" onClick={handleUpdate}>Save Changes</Button>
  </Modal.Footer>
</Modal>
    </div>
  );
