...
//     type="text"
//     name="qc_analyst"
//     // ✅ Apply the formatting function to the value
//     value={formatDisplayName(formData.qc_analyst   || user?.name)}
//     readOnly
//     className="border border-secondary bg-light fw-semibold"
//   />
// </Form.Group>
//                   </Col>
//                   <Col md={4}>
...
<div className="bg-white rounded-4 shadow-sm border overflow-hidden position-relative">
  ...
  {/* ✅ Premium Header for Pending Tasks */}
  <div className="premium-lite-banner d-flex justify-content-between align-items-center px-4 py-2 border-bottom"
       style={{ background: 'linear-gradient(to right, #f8f9fa 0%, #e9ecef 100%)' }}>
    <div className="d-flex align-items-center">
      <div className="header-accent-pill me-3" style={{ height: '24px', backgroundColor: '#0d6efd' }}></div>
      <div>
        <h5 className="fw-bold mb-0 text-dark" style={{ fontSize: '1.1rem' }}>Pending QA Reviews</h5>
        <small className="text-muted fw-medium" style={{ fontSize: '0.75rem' }}>Active Audit Queue</small>
      </div>
    </div>

    <div className="modern-count-badge py-1 border-primary-subtle">
      <span className="badge-text" style={{ fontSize: '0.6rem', color: '#0d6efd' }}>IN-PROGRESS</span>
      <span className="badge-number bg-primary text-white" style={{ width: '22px', height: '22px', fontSize: '0.7rem' }}>
        {incidents.length}
      </span>
    </div>
  </div>

  {/* ✅ High-Density Table (10 Rows) */}
  {loading ? (
    <div className="text-center py-5">
       <Spinner animation="border" size="sm" variant="primary" />
       <p className="text-muted mt-2 small">Fetching your queue...</p>
    </div>
  ) : (
    <div className="table-responsive">
      <Table hover className="align-middle mb-0 refined-table compact-view">
        <thead>
          <tr>
            <th style={{ width: "65px" }} className="ps-4">
              <div className="d-flex align-items-center">
                <Form.Check
                  type="checkbox"
                ...
                />
                <span className="ms-2 extra-small-label">ALL</span>
              </div>
            </th>
        ...
        <tbody>
          {incidents.length > 0 ? (
              ...
              <tr key={incident.sid || index}>
                ...
                <td>
                  <Button
                    variant="link"
                    onClick={() => handleOpenModal(incident)}
                    className="incident-link p-0 fw-bold text-decoration-none"
                    style={{ fontSize: '0.85rem' }}
                  >
                    <span className="text-muted fw-normal me-1">#</span>{incident.incident_number}
                  </Button>
                </td>
                <td>
                ...
                <td className="date-text">{formatDate(incident.incident_date)}</td>
                <td>
                  <span className="badge bg-light text-dark border fw-semibold px-2 py-1" style={{ fontSize: '0.7rem' }}>
                    <i className="bi bi-shield-check text-primary me-1"></i>
                    {incident.qc_analyst || "NA"}
                  </span>
                </td>
              </tr>
            ))
            ...
            <tr>
              <td colSpan="6" className="text-center py-5 text-muted small">No pending incidents in your queue.</td>
            </tr>
          )}
        </tbody>
      </Table>
  ...
  <Modal.Header closeButton className="bg-dark text-white py-2">
    <Modal.Title className="fw-bold fs-6">
      ...
      Audit Worksheet: <span className="text-info">#</span>{formData?.incident_number}
    </Modal.Title>
  </Modal.Header>
  ...
  <Modal.Body className="bg-light p-0">
    {formData && (
      <div className="p-4">
        {/* TOP HUD: SCORE & ASSIGNMENT */}
        <Row className="mb-3 g-3">
          <Col md={8}>
            <div className="bg-white p-3 rounded-3 shadow-sm border h-100">
               <h6 className="fw-bold text-primary mb-2 small text-uppercase">Incident Overview</h6>
               <Row>
                 <Col xs={6}><small className="text-muted d-block">Handled By</small>
                   <span className="fw-bold text-dark">{formData.assigned_analyst}</span>
                 </Col>
                 <Col xs={6}><small className="text-muted d-block">Incident Date</small>
                   <span className="fw-bold text-dark">{formatDate(formData.incident_date)}</span>
                 </Col>
               </Row>
            </div>
          </Col>
          <Col md={4}>
            <div className="bg-primary text-white p-3 rounded-3 shadow-sm h-100 text-center d-flex flex-column justify-content-center">
               <small className="text-uppercase fw-bold opacity-75" style={{ fontSize: '0.7rem' }}>Running Score</small>
               <div className="display-5 fw-bold">{totalScore}</div>
            </div>
          </Col>
        </Row>
        ...
        <div className="card border-0 shadow-sm mb-3">
          <div className="card-body">
            <Form.Group className="mb-0">
              <Form.Label className="fw-bold text-dark small">QUALITY CHECK CATEGORIES</Form.Label>
              {/* Dropdown component stays as you had it, but inside this styled card */}
              <Dropdown className="w-100 mt-1" autoClose="outside">
        ...
        <Row className="g-3">
          <Col md={12}>
            <Form.Group className="bg-white p-3 rounded-3 border shadow-sm">
              <Form.Label className="fw-bold small text-muted text-uppercase">Audit Notes / Observations</Form.Label>
              <Form.Control
                as="textarea"
                rows={3}
                className="border-0 bg-light mt-1"
  ...
  <Modal.Footer className="border-top-0 bg-light pb-4 px-4">
    <Button variant="outline-secondary" onClick={handleCloseModal} className="px-4 fw-bold small">Discard</Button>
    <Button variant="primary" onClick={handleUpdate} className="px-5 fw-bold shadow">Save Progress</Button>
  </Modal.Footer>
</Modal>
</div>
  );
