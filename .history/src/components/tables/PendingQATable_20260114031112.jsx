        ...
        <div className="d-flex justify-content-between align-items-center">
          <div className="d-flex align-items-center">
            ...
            <div>
              ...
              <small className="text-muted fw-medium">Incidents awaiting final audit</small>
            </div>
          </div>
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div className="d-flex align-items-center bg-white border rounded-pill px-3 py-1 shadow-sm">
              <span className="text-primary fw-bold small me-2">TOTAL PENDING:</span>
              <span className="badge bg-primary rounded-pill">{incidents.length}</span>
            </div>
          </div>
        </div>
      </div>

      {/* ✅ High-Density Table (Optimized for 10 Rows) */}
      {loading ? (
        <div className="text-center py-5">
          <Spinner animation="border" size="sm" variant="primary" />
          <p className="text-muted mt-2 small">Syncing queue...</p>
        </div>
      ) : (
              ...
              {incidents.length > 0 ? (
                incidents.slice(0, 10).map((incident, index) => (
                  <tr key={incident.sid || index}>
                    ...
                    <td className="text-truncate" style={{ maxWidth: "250px", fontSize: '0.82rem', color: '#64748b' }}>
                      {incident.short_description || "N/A"}
                    </td>
                    <td className="fw-medium text-dark" style={{ fontSize: '0.82rem' }}>
                      {incident.assigned_analyst ?
                    ...
                    <td>
                      <span className="badge bg-blue-subtle text-primary border border-primary-subtle rounded-pill px-2 py-1" style={{ fontSize: '0.72rem' }}>
  ...
  {incidents.length > 0 ? (
    incidents.slice(0, 10).map((incident, index) => (
      <tr 
        ...
        onClick={() => handleOpenModal(incident)} // ✅ Entire row is now clickable
                  
        className="cursor-pointer"
      >
        <td className="ps-4 text-center" onClick={(e) => e.stopPropagation()}> 
          {/* ✅ e.stopPropagation() prevents modal opening when clicking checkbox */}
          <Form.Check
            type="checkbox"
            className="custom-check-sm"
            checked={selectedIncidents.includes(incident.sid)}
