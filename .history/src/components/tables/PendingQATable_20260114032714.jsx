...
const PendingQATable = ({ incidents, loading, refresh }) => {
  ...
  <div className="bg-white rounded-4 shadow-lg border-0 overflow-hidden position-relative animate__animated animate__fadeIn">
  ...
  <div className="px-4 py-3 border-bottom d-flex justify-content-between align-items-center" 
    ...
    <div className="d-flex align-items-center">
      <div className="shadow-sm p-2 rounded-3 me-3 d-flex align-items-center justify-content-center" 
           style={{ background: '#4361ee', width: '42px', height: '42px' }}>
        <i className="bi bi-shield-check text-white fs-5"></i>
      </div>
      <div>
        <h6 className="fw-bold mb-0 text-dark" style={{ letterSpacing: '-0.2px', fontSize: '1.1rem' }}>
          Pending Quality Audit
        </h6>
        <div className="d-flex align-items-center">
          <span className="badge bg-success-subtle text-success border border-success-subtle rounded-pill me-2" style={{ fontSize: '10px' }}>
            LIVE QUEUE
          </span>
          <small className="text-muted fw-normal" style={{ fontSize: '12px' }}>Awaiting analyst verification</small>
        </div>
      </div>
    </div>

    <div className="d-flex align-items-center gap-2">
      <div className="bg-light border rounded-3 px-3 py-2 d-flex flex-column align-items-end">
        <span className="text-muted text-uppercase fw-bold" style={{ fontSize: '9px', letterSpacing: '0.5px' }}>Total Queue</span>
        <span className="text-primary fw-bolder lh-1" style={{ fontSize: '1.1rem' }}>{incidents.length}</span>
      </div>
    </div>
  </div>
  ...
  <div className="table-responsive" style={{ maxHeight: 'calc(100vh - 250px)', overflowY: 'auto' }}>
    <Table hover className="align-middle mb-0 premium-table">
      <thead style={{ position: 'sticky', top: 0, zIndex: 5, background: '#f8f9fa' }}>
        <tr className="border-bottom">
          ...
          <th className="premium-th">INCIDENT</th>
          <th className="premium-th">DESCRIPTION</th>
          <th className="premium-th">OWNER</th>
          <th className="premium-th">TIMELINE</th>
          <th className="premium-th text-end pe-4">AUDITOR</th>
        </tr>
      </thead>
      <tbody className="bg-white">
        {incidents.slice(0, 10).map((incident, index) => (
          <tr key={incident.sid || index} className="premium-row">
            <td className="ps-4 text-center">
              <Form.Check
                ...
                onChange={() =>
                  setSelectedIncidents((prev) =>
                    prev.includes(incident.sid) ? prev.filter((id) => id !== incident.sid) : [...prev, incident.sid]
                  )
                }
              />
            </td>
            <td>
              <div className="d-flex flex-column" onClick={() => handleOpenModal(incident)} style={{ cursor: 'pointer' }}>
                <span className="fw-bold text-primary mb-0" style={{ fontSize: '0.85rem' }}>
                  #{incident.incident_number}
                </span>
                <span className="text-muted" style={{ fontSize: '10px' }}>Priority: Normal</span>
              </div>
            </td>
            <td>
              <div className="text-truncate fw-medium text-dark" style={{ maxWidth: "280px", fontSize: '0.82rem' }}>
                {incident.short_description || "N/A"}
              </div>
            </td>
            <td>
              <div className="d-flex align-items-center">
                <div className="avatar-circle-sm me-2">
                  {(incident.assigned_analyst || "U").charAt(0).toUpperCase()}
                </div>
                <span className="fw-semibold text-secondary" style={{ fontSize: '0.82rem' }}>
                  {incident.assigned_analyst?.split(' ')[0]}
                </span>
              </div>
            </td>
            <td>
              <div className="d-flex flex-column">
                <span className="text-dark fw-medium" style={{ fontSize: '0.8rem' }}>{formatDate(incident.incident_date)}</span>
                <span className="text-muted" style={{ fontSize: '10px' }}>Created 2h ago</span>
              </div>
            </td>
            <td className="text-end pe-4">
              <span className="badge-premium">
                {incident.qc_analyst || "Pending"}
              </span>
            </td>
          </tr>
        ))}
