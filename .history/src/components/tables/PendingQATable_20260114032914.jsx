...
const PendingQATable = ({ incidents, loading, refresh }) => {
  ...
  {/* ✅ Premium Banner with Refined Badge */}
  <div className="px-4 py-2 border-bottom" style={{
    ...
    <div className="d-flex justify-content-between align-items-center">
      <div className="d-flex align-items-center">
        ...
        <div>
          <h6 className="fw-bold mb-0 text-dark" style={{ letterSpacing: '-0.3px' }}>
            Pending QA Queue
          </h6>
          <small className="text-muted fw-medium" style={{ fontSize: '11px' }}>Incidents awaiting final audit</small>
        </div>
      </div>

      <div className="d-flex align-items-center">
        {/* ✅ Premium Counter Badge */}
        <div className="premium-counter shadow-sm">
  ...
  {loading ? (
    ...
    <div className="table-responsive">
      <Table hover className="align-middle mb-0 compact-view">
          ...
          <tr>
            <th style={{ width: "60px" }} className="ps-4">
              <div className="d-flex flex-column align-items-center">
                <Form.Check
                  type="checkbox"
                  className="custom-check-sm mb-0"
                  checked={selectedIncidents.length === incidents.length && incidents.length > 0}
                  onChange={(e) => {
                    if (e.target.checked) setSelectedIncidents(incidents.map((i) => i.sid));
                    else setSelectedIncidents([]);
        ...
        <tbody>
          {incidents.length > 0 ? (
            incidents.slice(0, 10).map((incident, index) => (
              <tr key={incident.sid || index} className="incident-row">
                ...
                <td>
                  {/* ✅ Underline on Hover applied via 'incident-link' class */}
                  <div 
                    onClick={() => handleOpenModal(incident)} 
                    className="incident-link-container fw-bold text-primary cursor-pointer"
                ...
                <td className="text-truncate" style={{ maxWidth: "250px", fontSize: '0.8rem', color: '#64748b' }}>
                  {incident.short_description || "N/A"}
                </td>
                ...
                <td>
                  <span className="badge bg-blue-subtle text-primary border border-primary-subtle rounded-pill px-2 py-1" style={{ fontSize: '0.7rem' }}>
            ...
            <tr>
              <td colSpan="6" className="text-center py-4 text-muted small">No pending items found.</td>
            </tr>
          )}
        </tbody>
      </Table>
