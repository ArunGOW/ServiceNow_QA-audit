...
const PendingQATable = ({ incidents, loading, refresh }) => {
  ...
  < className="bg-white rounded-4 shadow-sm border overflow-hidden position-relative">
  <ToastContainer position="top-right" autoClose={2500} hideProgressBar />

  {/* ✅ Header with slightly more vertical padding (py-3) */}
  <div className="px-4 py-3 border-bottom" style={{
    ...
    <div className="d-flex justify-content-between align-items-center">
      <div className="d-flex align-items-center">
        ...
        <div>
          <h5 className="fw-bold mb-0 text-dark" style={{ letterSpacing: '-0.2px' }}>
            Pending QA Queue
          </h5>
          <small className="text-muted fw-medium" style={{ fontSize: '13px' }}>Incidents awaiting final audit</small>
        </div>
      </div>

      <div className="d-flex align-items-center">
        {/* ✅ Larger, More Readable Premium Badge */}
        <div className="premium-counter-large shadow-sm">
  ...
  {loading ? (
    <div className="text-center py-5">
      ...
      <p className="text-muted mt-2">Syncing queue...</p>
    </div>
  ) : (
    <div className="table-responsive">
      <Table hover className="align-middle mb-0">
        <thead className="bg-light">
          <tr>
            <th style={{ width: "70px" }} className="ps-4 text-center">
              <Form.Check
                type="checkbox"
                className="custom-check-md"
                checked={selectedIncidents.length === incidents.length && incidents.length > 0}
                onChange={(e) => {
                  if (e.target.checked) setSelectedIncidents(incidents.map((i) => i.sid));
                  else setSelectedIncidents([]);
            ...
            <th className="standard-header">INCIDENT NO</th>
            <th className="standard-header">DESCRIPTION</th>
            <th className="standard-header">HANDLED BY</th>
            <th className="standard-header">HANDLED ON</th>
            <th className="standard-header">QC ANALYST</th>
          </tr>
        </thead>
        <tbody className="bg-white">
          {incidents.slice(0, 10).map((incident, index) => (
            <tr key={incident.sid || index} className="incident-row-refined">
              ...
              <td>
                <div 
                  onClick={() => handleOpenModal(incident)} 
                  className="incident-link-main fw-bold text-primary cursor-pointer"
                >
                  <span className="text-muted fw-normal me-1">#</span>
                  <span className="link-text">{incident.incident_number}</span>
                </div>
              </td>
              <td className="text-truncate" style={{ maxWidth: "300px", fontSize: '0.9rem', color: '#475569' }}>
                {incident.short_description || "N/A"}
              </td>
              ...
              <td>
                <span className="badge bg-primary-subtle text-primary border border-primary-subtle rounded-pill px-3 py-2" style={{ fontSize: '0.75rem', fontWeight: '700' }}>
