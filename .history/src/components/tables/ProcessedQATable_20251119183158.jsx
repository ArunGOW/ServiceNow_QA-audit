...
//   // ================================
//   const handleOpenModal = (incident) => {
//     logger.info("Opening Processed QA Modal for:", incident);
...
//   const handleCloseModal = () => {
//     logger.info("Closing Processed QA Modal");
...
//   const formatDate = (dateStr) => {
//     if (!dateStr) return "";
//     if (dateStr.includes("-")) return dateStr.split("T")[0]; // Already ISO
...
//   // ================================
//   // ✅ Update processed QA incident
//   // ================================
//   const handleUpdate = async () => {
//     try {
//       if (!formData.incident_sid || !formData.incident_number) {
//         logger.warn("Incident SID or Number missing!", formData);
//         alert("Incident SID or Incident Number is missing!");
//         return;
//       }
...
//       logger.info("Submitting Processed QA Update Payload →", payload);

//       const res = await api.post("/users/update/incident-status", payload);

//       logger.info("Processed QA Update Success:", res.data);
...
//       if (formData.resolution_status?.toLowerCase() === "resolved") {
//         logger.info("Removing resolved incident from table:", formData.incident_sid);
...
//       handleCloseModal();
//     } catch (error) {
//       logger.error("Error updating processed incident:", error.response?.data || error.message);
...
//   // ================================
//   const handleSelectIncident = (incidentSid) => {
//     logger.info("Toggling selection for incident:", incidentSid);
...
//       return alert("Please select at least one incident.");
//     if (!loggedUserSid) return alert("Unable to find logged-in user SID.");
...
//     logger.info("Assigning Incidents Payload →", payload);

//     try {
//       const res = await api.post("/users/assign/incidents/", payload);

//       logger.info("Assign API Success:", res.data);
...
//             {incidents.length > 0 ? (
//               incidents.map((incident, index) => (
//                 <tr key={incident.sid || index} onClick={() => handleOpenModal(incident)}>
//                   <td>{incident.incident_number || "N/A"}</td>
...
const ProcessedQATable = ({ incidents, loading, refresh }) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await api.get("/users/get/list_users");
        logger.success("Fetched users", res.data);
        setUsers(res.data || []);
      } catch (error) {
  ...
  const handleUpdate = async () => {
    try {
      if (!formData.incident_sid || !formData.incident_number) {
        logger.warn("Incident sid or number missing");
        alert("Incident SID or Incident Number is missing!");
        return;
      }
      ...
      logger.info("Submitting update payload", payload);

      const res = await api.post("/users/update/incident-status", payload);
      logger.success("Update successful", res.data);
      ...
      if (formData.resolution_status?.toLowerCase() === "resolved") {
        ...
        logger.info("Removed resolved incident", formData.incident_sid);
      }
  ...
  const handleSelectIncident = (sid) => {
    ...
    logger.info("Incident selected", sid);
  };

  // 🚀 Assign incidents
  const handleAssign = async () => {
    if (!selectedUser) return alert("Please select a user first.");
    if (selectedIncidents.length === 0) return alert("Please select incidents.");
    if (!loggedUserSid) return alert("Logged-in user SID missing.");
    ...
    try {
      const res = await api.post("/users/assign/incidents/", payload);
      logger.success("Assignment successful", res.data);
      alert("Incidents assigned successfully!");
