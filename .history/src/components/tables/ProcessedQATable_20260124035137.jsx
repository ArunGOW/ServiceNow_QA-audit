...
// import "../PendingQATable.css";
import "../../styles/ProcessedTable.css";
...
const ProcessedQATable = ({ incidents, loading, refresh }) => {
  ...
  {/* ✅ Unified Premium Banner (Matching Pending Table) */}
  <div className="px-4 py-3 border-bottom" style={{
    background: 'linear-gradient(135deg, #f8fafd 0%, #eef2f7 100%)',
    borderLeft: '5px solid #198754' // Green accent for Processed/Success
  }}>
    <div className="d-flex justify-content-between align-items-center">
      <div className="d-flex align-items-center">
        ...
        <div>
          ...
          <small className="text-muted fw-medium">Historical audit and performance data</small>
        </div>
      </div>
      ...
      <div className="d-flex align-items-center gap-3">
        ...
        <div className="d-flex align-items-center bg-white border rounded-pill px-3 py-1 shadow-sm">
          <span className="text-success fw-bold small me-2">TOTAL COMPLETED:</span>
          <span className="badge bg-success rounded-pill">{incidents.length}</span>
        </div>
      </div>
    </div>
  </div>
            ...
            incidents.slice(0, 10).map((incident, index) => (
              <tr key={incident.sid || index} className="density-row">
                <td className="ps-4 fw-bold">
                  <div className="incident-link-slim text-primary cursor-pointer" onClick={() => handleRowClick(incident)}>
                    <span className="text-muted fw-normal me-1">#</span>
                    <span className="link-text">{incident.incident_number || "N/A"}</span>
                  </div>
                </td>
                <td>
                  <div className="text-truncate" style={{ maxWidth: "250px", fontSize: '13px', color: '#475569' }} title={incident.short_description}>
                    {incident.short_description || "N/A"}
                  </div>
                </td>
