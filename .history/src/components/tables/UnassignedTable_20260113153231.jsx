...
<div className="bg-white rounded-3 shadow-sm border position-relative overflow-hidden">
  {/* Toast Notifications */}
  <ToastContainer position="top-end" className="p-3">
    <Toast
      show={toast.show}
    ...
    >
      <Toast.Body className="text-white fw-medium py-3">{toast.message}</Toast.Body>
    </Toast>
  </ToastContainer>

  {/* Header Section - Premium Gradient or Soft Tint */}
  <div className="d-flex justify-content-between align-items-center p-4 bg-light border-bottom">
    <h5 className="fw-bold mb-0 text-dark tracking-tight">Unassigned Incidents</h5>
    ...
    <div className="d-flex gap-3">
      <Form.Select
        ...
        value={selectedUser}
        onChange={(e) => setSelectedUser(e.target.value)}
        className="form-select-lg border-secondary-subtle shadow-sm fw-medium"
        style={{ width: '220px', fontSize: '0.85rem' }}
      >
        <option value="">-- Select QA Admin --</option>
        {users.map((user) => (
          <option key={user.sid} value={user.sid}>
            {user.full_name.toLowerCase().replace(/\b\w/g, (char) => char.toUpperCase())}
          </option>
        ))}
      </Form.Select>
      ...
      <Table hover responsive className="mb-0 align-middle">
        <thead className="bg-white">
          <tr className="border-bottom">
            ...
            <th className="text-uppercase text-muted fw-bold small">Incident No</th>
            <th className="text-uppercase text-muted fw-bold small">Description</th>
            <th className="text-uppercase text-muted fw-bold small">Handled On</th>
            <th className="text-uppercase text-muted fw-bold small">Handled By</th>
            <th className="text-uppercase text-muted fw-bold small text-end pe-4">Status</th>
          </tr>
        </thead>
        <tbody className="border-top-0">
          {localIncidents.length > 0 ? (
            localIncidents.map((incident) => (
              <tr key={incident.sid} style={{ cursor: 'pointer', transition: '0.2s' }}>
                <td className="ps-4">
                  <Form.Check
                    type="checkbox"
                    checked={selectedIncidents.includes(incident.sid)}
                ...
                <td className="text-truncate" style={{ maxWidth: '250px' }}>
                  <span className="text-dark fw-medium">{incident.short_description || "N/A"}</span>
                </td>
                <td className="text-muted small">{incident.incident_date}</td>
                <td>
                  <div className="d-flex align-items-center">
                    <div className="bg-info-subtle text-info rounded-circle d-flex align-items-center justify-content-center me-2" style={{width: '24px', height: '24px', fontSize: '10px'}}>
                      {incident.assigned_analyst?.[0] || 'U'}
                    </div>
                    <span className="fw-medium">
                        {incident.assigned_analyst ? incident.assigned_analyst.toLowerCase().replace(/\b\w/g, c => c.toUpperCase()) : "N/A"}
                    </span>
                  </div>
                </td>
                <td className="text-end pe-4">
                  <span className="badge rounded-pill bg-warning-subtle text-warning-emphasis border border-warning-subtle px-3">
                    {incident.resolution_status || "Unassigned"}
                  </span>
                </td>
              </tr>
            ...
            <tr>
              <td colSpan="6" className="py-5 text-muted">No unassigned incidents found.</td>
            </tr>
          )}
        </tbody>
      </Table>
