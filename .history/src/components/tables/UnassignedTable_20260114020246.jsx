...
const UnassignedTable = ({ incidents = [], loading, refresh }) => {
  ...
  <div className="premium-lite-banner d-flex justify-content-between align-items-center px-4 py-2 border-bottom">
    <div className="d-flex align-items-center">
      <div className="header-accent-pill me-3" style={{ height: '24px' }}></div>
    ...
    <div className="d-flex gap-3 align-items-center">
      <div className="d-flex gap-2 align-items-center bg-white p-1 rounded-3 border shadow-sm">
        <Form.Select
          ...
          style={{ width: "160px", fontSize: '0.8rem', boxShadow: 'none' }}
          value={selectedUser}
          onChange={(e) => setSelectedUser(e.target.value)}
        >
        ...
        <Button size="sm" variant="primary" className="px-3 py-0 fw-bold" style={{ fontSize: '0.75rem', height: '28px' }} onClick={handleAssign}>
          Assign
        </Button>
      </div>
      <div className="modern-count-badge py-1">
        <span className="badge-text" style={{ fontSize: '0.6rem' }}>PENDING</span>
        <span className="badge-number bg-warning text-dark" style={{ width: '22px', height: '22px', fontSize: '0.7rem' }}>
  ...
  <div className="table-responsive">
    <Table hover className="align-middle mb-0 refined-table compact-view">
      ...
      <tbody>
        {localIncidents.slice(0, 10).map((incident) => (
          <tr key={incident.sid}>
            <td className="ps-4">
              <Form.Check
                type="checkbox"
            ...
            <td>
              <span className="incident-link fw-bold text-primary">{incident.incident_number}</span>
            </td>
            <td>
              <div className="description-text text-secondary">{incident.short_description || "—"}</div>
            </td>
            <td className="date-text text-muted">{incident.incident_date || "—"}</td>
            <td className="analyst-name">{incident.assigned_analyst || "N/A"}</td>
            <td className="text-center">
              <span className="badge bg-warning-subtle text-warning-emphasis border-warning-subtle border rounded-pill px-2 py-1" style={{ fontSize: '0.7rem' }}>
                UNASSIGNED
              </span>
            </td>
          </tr>
