...
const UnassignedTable = ({ incidents = [], loading, refresh }) => {
    ...
    <div className="d-flex gap-3 align-items-center">
      <div className="d-flex gap-2 align-items-center bg-white p-1 rounded-3 border shadow-sm">
        <Form.Select
          ...
          style={{ width: "180px", fontSize: '0.85rem', outline: 'none', boxShadow: 'none' }}
        >
          <option value="">-- Select QA Admin --</option>
          {users.map((user) => (
            <option key={user.sid} value={user.sid}>
      ...
      <div className="modern-count-badge">
        <span className="badge-text">UNASSIGNED</span>
        <span className="badge-number bg-warning text-dark">{localIncidents.length}</span>
      </div>
    </div>
  </div>
  ...
  {loading ? (
    <div className="text-center py-5">
      ...
      <p className="text-muted mt-2 small">Fetching records...</p>
    </div>
  ) : (
          ...
          {localIncidents.length > 0 ? (
            localIncidents.map((incident) => (
              <tr key={incident.sid}>
                ...
                <td>
                  <span className="incident-link fw-bold text-primary cursor-pointer">
                    {incident.incident_number}
                  </span>
                </td>
                <td>
                ...
                <td>
                  <span className="badge bg-warning-subtle text-warning-emphasis border border-warning-subtle px-3 py-2 rounded-pill fw-bold" style={{ fontSize: '0.75rem' }}>
                    {incident.resolution_status || "UNASSIGNED"}
                  </span>
                </td>
              </tr>
            ...
            <tr>
              <td colSpan="6" className="text-center py-5 text-muted">No unassigned incidents found.</td>
            </tr>
          )}
        </tbody>
      </Table>
  ...
  {/* ✅ Reusing the same Premium Modal design logic if needed */}
</div>
