...
const AllusersDashboard = () => {
  ...
  useEffect(() => {
    const fetchUserList = async () => {
      try {
        ...
        // Assuming res.data is an array of strings or objects like [{name: 'sagnik'}, ...]
        // If it's a simple array of strings, use res.data. If objects, map them.
  ...
  return (
    <div style={styles.wrapper}>
      ...
      {activeTab === 'individual' && (
        <div style={styles.filterBar}>
          <div style={styles.filterGroup}>
            ...
            <select style={styles.select} value={selectedAgent} onChange={(e) => setSelectedAgent(e.target.value)}>
               {/* Dynamically Map the User List from API */}
               {userList.map((u, idx) => (
                 <option key={idx} value={typeof u === 'string' ? u : u.username}>
                   {typeof u === 'string' ? u : u.name || u.username}
                 </option>
               ))}
            </select>
          </div>
      ...
      <div style={styles.contentGrid}>
        <div style={styles.chartCard}>
          <h3 style={styles.cardHeader}>{activeTab === 'team' ? "Team Distribution" : `${selectedAgent}'s Performance`}</h3>
        ...
        <div style={styles.tableCard}>
          ...
          {activeTab === 'team' ? (
            ...
            <div style={styles.individualDetail}>
               <div style={styles.analysisItem}><span>Avg. Quality Score:</span> <strong>{data?.filtered?.agent_performance?.[0]?.average_score}%</strong></div>
               <div style={styles.analysisItem}><span>Process Compliance:</span> <strong>{data?.filtered?.agent_performance?.[0]?.needs_training ? "Action Required" : "Compliant"}</strong></div>
               <div style={{marginTop: '20px', fontSize: '12px', color: '#94a3b8'}}>* Report generated for the period of {fromDate} to {toDate}.</div>
            </div>
          )}
        </div>
      </div>
