...
const AllusersDashboard = () => {
 ...
 const fetchUserList = async () => {
  try {
    ...
    // Auto-select the first user from the list if one exists
    if (list.length > 0) {
      // We convert to lowercase to match the typical 'agent' parameter format
      setSelectedAgent(list[0].full_name.toLowerCase());
    }
  } catch (err) {
