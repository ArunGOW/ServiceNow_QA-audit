 ...
 useEffect(() => {
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // If we are in 'individual' tab, we send the SID to the agent parameter
      // Note: If your backend specifically needs sid=... instead of agent=..., 
      // just change the key name below.
      const url = activeTab === 'team' 
        ? '/users/admin/dashboard' 
        : `/users/admin/dashboard?agent=${selectedAgent}&from_date=${fromDate}&to_date=${toDate}`;
