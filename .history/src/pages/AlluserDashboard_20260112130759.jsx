...
useEffect(() => {
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Add params only if in individual tab
      if (activeTab === 'individual') {
        config.params = {
          agent: selectedAgent, // Sending the SID
          from_date: fromDate,
          to_date: toDate
        };
      }

      // Final call: http://.../dashboard?agent=SID&from_date=...
      const res = await api.get('/users/admin/dashboard', config);
      
      console.log("API Response Data:", res.data); // Verify if 'filtered' key exists here
      setData(res.data);
    } catch (err) {
      console.error("Dashboard Error:", err);
    } finally {
...
// 2. Map the Pie Chart data using the correct summary
const pieData = [
  { name: 'Passed', value: summary?.passed || 0 },
  { name: 'Failed', value: summary?.failed || 0 },
  { name: 'Pending', value: summary?.pending || 0 }
...
const summary = displaySummary || { total_tickets: 0, passed: 0, failed: 0, pending: 0, pass_percentage: 0 };
const agentsList = displayAgents;
  return (
    <div style={styles.wrapper}>
 ...
 <div style={styles.filterGroup}>
  ...
  >
    {userList.map((u) => (
      // In the dropdown
<option key={u.sid} value={u.email}> {u.full_name} </option>
    ))}
  </select>
</div>
