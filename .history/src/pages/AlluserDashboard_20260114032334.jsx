...
const AllusersDashboard = () => {
  ...
  const renderActiveShape = (props) => {
    ...
    return (
      <g>
        <text x={cx} y={cy} dy={-5} textAnchor="middle" fill="#1e293b" style={{ fontSize: '14px', fontWeight: 'bold' }}>{payload.name}</text>
        <text x={cx} y={cy} dy={15} textAnchor="middle" fill="#64748b" style={{ fontSize: '11px' }}>{`${value} Tickets`}</text>
  ...
  return (
    <div style={styles.wrapper}>
      ...
      {activeTab === 'individual' && (
        <div style={styles.filterBar}>
          <div style={styles.filterGroup}>
            <label style={styles.filterLabel}>Analyst</label>
            <select style={styles.select} value={selectedAgent} onChange={(e) => setSelectedAgent(e.target.value)}>
              {userList.map(u => <option key={u.sid} value={u.sid}>{u.full_name}</option>)}
            </select>
          </div>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={styles.contentGrid}>
          ...
          <div style={styles.chartCard}>
            <h3 style={styles.cardHeader}>Distribution</h3>
          ...
          {/* Table Card - Fixed Scrollable Area */}
          <div style={styles.tableCard}>
            <h3 style={styles.cardHeader}>{activeTab === 'team' ? "Team Rankings" : "Analyst Profile"}</h3>
...
const styles = {
  wrapper: { backgroundColor: '#f8fafc', height: '100vh', padding: '15px 30px', fontFamily: "'Inter', sans-serif", overflow: 'hidden' },
  navbar: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' },
  logo: { margin: 0, fontSize: '20px', fontWeight: '800' },
  tabContainer: { display: 'flex', gap: '8px' },
  activeTab: { padding: '6px 12px', background: '#6366f1', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' },
  inactiveTab: { padding: '6px 12px', background: '#fff', color: '#64748b', border: '1px solid #e2e8f0', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' },
  filterBar: { display: 'flex', gap: '15px', background: '#fff', padding: '10px 15px', borderRadius: '10px', marginBottom: '15px', boxShadow: '0 1px 2px rgba(0,0,0,0.05)', alignItems: 'center' },
  filterGroup: { display: 'flex', flexDirection: 'column' },
  filterLabel: { fontSize: '10px', fontWeight: 'bold', color: '#94a3b8', marginBottom: '2px' },
  select: { padding: '5px', borderRadius: '4px', border: '1px solid #e2e8f0', fontSize: '12px' },
  dateInput: { padding: '4px', borderRadius: '4px', border: '1px solid #e2e8f0', fontSize: '12px' },
  resetBtn: { padding: '5px 10px', fontSize: '11px', border: '1px solid #cbd5e1', borderRadius: '4px', background: '#fff', alignSelf: 'flex-end' },
  statsRow: { display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', marginBottom: '15px' },
  metricCard: { background: '#fff', padding: '12px 15px', borderRadius: '12px', border: '1px solid #f1f5f9' },
  metricTop: { display: 'flex', justifyContent: 'space-between', marginBottom: '4px' },
  metricLabel: { fontSize: '11px', color: '#64748b', fontWeight: '600' },
  metricValue: { fontSize: '20px', fontWeight: '800', margin: 0 },
  contentGrid: { display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '20px' },
  chartCard: { background: '#fff', padding: '15px', borderRadius: '15px', height: '320px' },
  tableCard: { background: '#fff', padding: '15px', borderRadius: '15px', height: '320px', overflow: 'hidden' },
  cardHeader: { fontSize: '14px', fontWeight: '700', marginBottom: '10px', color: '#1e293b' },
  scrollArea: { height: '250px', overflowY: 'auto' },
  table: { width: '100%', borderCollapse: 'collapse' },
  th: { textAlign: 'left', padding: '8px', fontSize: '10px', color: '#94a3b8', borderBottom: '1px solid #f1f5f9', position: 'sticky', top: 0, background: '#fff' },
  td: { padding: '8px', fontSize: '12px', borderBottom: '1px solid #f8fafc' },
  pItem: { display: 'flex', justifyContent: 'space-between', padding: '12px', background: '#f8fafc', borderRadius: '8px', marginBottom: '8px', fontSize: '13px' },
  overlayLoader: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(255,255,255,0.7)', zIndex: 10, display: 'flex', justifyContent: 'center', alignItems: 'center' },
