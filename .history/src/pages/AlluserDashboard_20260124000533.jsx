...
const AllusersDashboard = () => {
  ...
  const isAdmin = user?.role === "qa_admin";
  ...
  // ✅ Force agents to TEAM tab only
  useEffect(() => {
    if (isAgent) setActiveTab('team');
  }, [isAgent]);
  ...
  useEffect(() => {
    if (isAgent && activeTab === 'individual') return;
  ...
  const renderActiveShape = (props) => {
    ...
    return (
      <g>
        <text x={cx} y={cy} dy={-8} textAnchor="middle" fill="#1e293b" style={{ fontSize: '14px', fontWeight: '800' }}>
          {payload.name}
        </text>
        <text x={cx} y={cy} dy={12} textAnchor="middle" fill="#64748b" style={{ fontSize: '10px' }}>
          {`${value} Tickets`}
        </text>
  ...
  return (
    <div style={styles.container}>
      ...
      <header style={styles.header}>
        ...
        {/* ✅ Filters only for ADMIN + Individual tab */}
        {activeTab === 'individual' && !isAgent && (
          <div style={styles.filterStrip}>
            <select
        ...
        <section style={styles.card}>
          <div style={styles.cardHeader}>
            <h6 style={styles.cardTitle}>
              {activeTab === 'team' ? "Performance Rankings" : "Analyst Details"}
            </h6>
          </div>
...
/* ---------- Helper Components & styles stay SAME ---------- */
