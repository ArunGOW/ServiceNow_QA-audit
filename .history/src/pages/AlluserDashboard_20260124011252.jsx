...
const AllusersDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // Only admin + individual view can send agent/date filters
        if (isAdmin && activeTab === "individual") {
          params.agent_sid = selectedAgent;
          if (fromDate) params.from_date = fromDate;
          if (toDate) params.to_date = toDate;
        }
    ...
    fetchDashboardData();
  }, [activeTab, selectedAgent, fromDate, toDate, user, isAdmin]);
  ...
  if (data) {
    if (isAdmin && activeTab === "individual" && data.filtered) {
      summary = data.filtered.summary || emptySummary;
      agentsList = data.filtered.agent_performance || [];
    } else {
      summary = data.all_time?.summary || emptySummary;
  ...
  const renderActiveShape = (props) => {
    ...
    return (
      <g>
        <text x={cx} y={cy} dy={-8} textAnchor="middle" fill="#1e293b" style={{ fontSize: 14, fontWeight: 800 }}>
          {payload.name}
        </text>
        <text x={cx} y={cy} dy={12} textAnchor="middle" fill="#64748b" style={{ fontSize: 10 }}>
          {value} Tickets
        </text>
        <Sector {...props} outerRadius={outerRadius + 4} fill={fill} />
      </g>
    );
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <header style={styles.header}>
        ...
        {/* ===== FILTERS (ADMIN + INDIVIDUAL ONLY) ===== */}
        {isAdmin && activeTab === 'individual' && (
          <div style={styles.filterStrip}>
            <select
      ...
      {/* ===== KPI ROW ===== */}
      <div style={styles.kpiRow}>
        <MetricBox label="TOTAL" value={summary.total_tickets} />
        <MetricBox label="PASSED" value={summary.passed} />
        <MetricBox label="FAILED" value={summary.failed} />
        <MetricBox label="PENDING" value={summary.pending} />
        <MetricBox label="PASS %" value={`${summary.pass_percentage}%`} isLast />
      </div>

      {/* ===== PIE CHART ===== */}
      <div style={{ height: 260, background: "#fff", borderRadius: 10 }}>
        <ResponsiveContainer>
          <PieChart>
            <Pie
              data={pieData}
...
/* ================= SMALL COMPONENTS ================= */
const MetricBox = ({ label, value, isLast }) => (
  <div style={{ ...styles.metricBox, borderRight: isLast ? 'none' : '1px solid #e5e7eb' }}>
...
/* ================= STYLES ================= */
const styles = {
  container: { padding: 16, background: "#f4f7fa", height: "100vh" },
  header: { display: "flex", justifyContent: "space-between", background: "#fff", padding: 16, borderRadius: 10 },
  titleArea: { display: "flex", gap: 20, alignItems: "center" },
  logo: { margin: 0 },
  tabGroup: { display: "flex", gap: 6 },
  activeTab: { background: "#4f46e5", color: "#fff", border: "none", padding: "6px 12px", borderRadius: 6 },
  inactiveTab: { background: "#e5e7eb", border: "none", padding: "6px 12px", borderRadius: 6 },
  filterStrip: { display: "flex", gap: 6 },
  inputSlim: { padding: 6 },
  kpiRow: { display: "flex", background: "#fff", borderRadius: 10, margin: "12px 0" },
  metricBox: { flex: 1, padding: 12 },
