...
const fetchPendingIncidents = async () => {
  ...
  // Safety check: ensure we have a SID before calling
  const currentSid = user?.sid || user?.user_sid || localStorage.getItem("user_sid");
  
  if (!currentSid) {
    console.error("No User SID found for pending request");
    return;
  }
  ...
  try {
    ...
    // Exact payload structure required by your Swagger/Curl
    const payload = {
      page: currentPage,
      per_page: perPage,
      user_sid: currentSid // Ensure this is a string UUID
    };
    ...
    // Check if res.data contains the list directly or inside an 'incidents' key
    setPendingIncidents(res.data?.incidents || res.data?.data || []);
    setTotalPending(res.data?.total_count || res.data?.total || 0);
  } catch (err) {
    console.error("Pending API Error:", err.response?.data || err.message);
  } finally {
    setLoading(false);
  }
};
                ...
                <table style={styles.table}>
                  ...
                  <tbody>
                    {pendingIncidents.map((incident, i) => (
                      <tr key={i} style={styles.tr}>
                        <td style={styles.td}><strong>#{incident.incident_number}</strong></td>
                        <td style={styles.td}>{incident.short_description || "No Description"}</td>
                        <td style={styles.td}>{new Date(incident.created_at).toLocaleDateString()}</td>
                      </tr>
                    ))}
                    {pendingIncidents.length === 0 && !loading && (
                      <tr><td colSpan="3" style={{textAlign: 'center', padding: '40px', color: '#94a3b8'}}>No pending items found.</td></tr>
                    )}
                  </tbody>
                </table>
