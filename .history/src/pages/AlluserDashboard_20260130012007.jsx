...
const AllusersDashboard = () => {
  ...
  const selectedUserName =
    userList.find(u => u.sid === selectedAgent)?.full_name || "Select Analyst";

  // ======================================================
  // 🔥 LOGIC ADDITION #1
  // Agent users should ALSO see QA Admin (team) data
  // ======================================================
  const effectiveTab =
    user?.user_type === "agent" && activeTab === "team"
      ? "team"
      : activeTab;
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // 🔥 LOGIC ADDITION #2
        // Only apply individual filters when tab is individual
        if (effectiveTab === 'individual') {
          params.agent_sid = selectedAgent;
          if (fromDate) params.from_date = fromDate;
          if (toDate) params.to_date = toDate;
        }
    ...
    fetchDashboardData();
  }, [effectiveTab, selectedAgent, fromDate, toDate, user]);
  ...
  if (data) {
    if (effectiveTab === 'individual' && data.filtered) {
      summary = data.filtered.summary || emptySummary;
      agentsList = data.filtered.agent_performance || [];
    } else {
      summary = data.all_time?.summary || emptySummary;
