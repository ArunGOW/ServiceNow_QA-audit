...
const AllusersDashboard = () => {
  ...
  // Options matched to your screenshot icons
  const options = [
    { id: '', label: 'All Queues', color: '#4f46e5', icon: 'bi-grid-fill' },
    { id: 'resolved', label: 'Resolved', color: '#10b981', icon: 'bi-check-circle-fill' },
    { id: 'escalated', label: 'Escalated', color: '#f43f5e', icon: 'bi-fire' },
    { id: 'in progress', label: 'In Progress', color: '#f59e0b', icon: 'bi-clock-history' },
    { id: 'on hold', label: 'On Hold', color: '#64748b', icon: 'bi-pause-circle-fill' },
  ...
  const emptySummary = { total_tickets: 0, passed: 0, failed: 0, pending: 0, pass_percentage: 0 };
  let summary = data ? (activeTab === 'individual' ? data.filtered?.summary : data.all_time?.summary) : emptySummary;
  ...
  return (
    <div style={styles.container}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div style={styles.dropdownContainer}>
                <div style={styles.premiumButton} onClick={() => setIsUserOpen(!isUserOpen)}>
                  <div style={{ ...styles.statusDot, backgroundColor: '#6366f1' }} />
                  <span style={styles.selectedText}>{selectedUserName}</span>
                  <i className={`bi bi-chevron-${isUserOpen ? 'up' : 'down'}`} style={{ fontSize: '11px' }}></i>
                </div>
                {isUserOpen && (
                  <div style={styles.scrollableMenu}>
                    {userList.map((u) => (
      ...
      <div style={activeTab === 'pending' ? styles.fullView : styles.mainGrid}>
        ...
        <section style={styles.card}>
          {/* ✅ UPDATED BANNER STYLE FROM SCREENSHOT */}
          <div style={activeTab === 'pending' ? styles.screenshotBanner : styles.cardHeader}>
            <div style={styles.bannerLeft}>
              {activeTab === 'pending' && (
            ...
            {activeTab === 'pending' && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                {/* Custom Filter Select from Screenshot */}
                <div style={styles.filterBox}>
                   <span style={styles.filterLabelLite}>FILTER STATUS</span>
                   <div style={styles.statusDropdownTrigger} onClick={() => setIsOpen(!isOpen)}>
                      <i className={`bi ${selectedOption.icon}`} style={{ color: selectedOption.color }}></i>
                   ...
                   {isOpen && (
                      <div style={styles.statusMenu}>
                         {options.map(opt => (
                           <div key={opt.id} style={styles.menuItem} onClick={() => { setStatusFilter(opt.id); setIsOpen(false); }}>
                ...
                {/* Total Count Badge from Screenshot */}
                <div style={styles.totalBadge}>
                   <span style={styles.totalLabel}>TOTAL TICKETS</span>
                   <span style={styles.totalCount}>{totalPending}</span>
                </div>
              </div>
            )}
          </div>
              ...
              <tbody>
                {pendingIncidents.length > 0 ? (
                  pendingIncidents.map((incident, i) => (
                    <tr key={i} style={styles.tr}>
                      <td style={styles.td}>
                        <span style={styles.ticketIdLink}>{incident.incident_number}</span>
                      </td>
                      <td style={styles.td}>
                        <div style={styles.descriptionText}>{incident.short_description}</div>
                      </td>
                      <td style={styles.td}>
                        <span style={{
                          ...styles.statusBadge,
                          backgroundColor: incident.status?.toLowerCase() === 'resolved' ? '#dcfce7' : '#ffedd5',
                          color: incident.status?.toLowerCase() === 'resolved' ? '#10b981' : '#9a3412',
                          borderColor: incident.status?.toLowerCase() === 'resolved' ? '#bbf7d0' : '#fed7aa',
                        }}>
                          {incident.status?.toUpperCase()}
                        </span>
                      </td>
                      <td style={styles.td}>
                        <span style={styles.updateText}>{incident.resolution_shared || "N/A"}</span>
                      </td>
                    </tr>
                  ))
                ) : (
          ...
          {activeTab === 'pending' && (
            <div style={styles.paginationArea}>
               ...
               <span style={{fontSize: '12px', color: '#64748b'}}>Page {currentPage}</span>
...
const styles = {
  container: { backgroundColor: '#f8fafc', height: '100vh', padding: '12px 20px', fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', backgroundColor: '#1e293b', padding: '10px 20px', borderRadius: '8px' },
  logo: { margin: 0, fontSize: '18px', fontWeight: '900', color: '#fff' },
  tabGroup: { background: 'rgba(255,255,255,0.1)', padding: '3px', borderRadius: '8px', display: 'flex', marginLeft: '30px' },
  activeTab: { padding: '5px 15px', background: '#fff', color: '#1e293b', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '700', fontSize: '12px' },
  inactiveTab: { padding: '5px 15px', background: 'transparent', color: '#cbd5e1', border: 'none', cursor: 'pointer', fontWeight: '500', fontSize: '12px' },
  ...
  rocketIcon: {
    width: '45px', height: '45px', background: 'rgba(255,255,255,0.2)', borderRadius: '12px',
    display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '20px', marginRight: '15px'
  },
  bannerTitleLarge: { margin: 0, fontSize: '22px', fontWeight: '800', color: '#fff' },
  bannerSub: { margin: 0, fontSize: '12px', color: 'rgba(255,255,255,0.8)', fontWeight: '500' },
  
  filterBox: { display: 'flex', flexDirection: 'column', gap: '5px' },
  filterLabelLite: { fontSize: '10px', color: 'rgba(255,255,255,0.7)', fontWeight: '800' },
  statusDropdownTrigger: {
    background: 'rgba(255,255,255,0.2)', border: '1px solid rgba(255,255,255,0.3)',
    borderRadius: '10px', padding: '8px 15px', display: 'flex', alignItems: 'center', gap: '10px',
    color: '#fff', cursor: 'pointer', minWidth: '160px', justifyContent: 'space-between', fontSize: '14px', fontWeight: '600'
  },
  statusMenu: {
    position: 'absolute', top: '100%', background: '#fff', borderRadius: '12px', width: '180px',
    boxShadow: '0 10px 25px rgba(0,0,0,0.1)', zIndex: 100, padding: '8px'
  },
  menuItem: {
  ...
  totalBadge: { background: '#fff', padding: '10px 20px', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '15px' },
  totalLabel: { fontSize: '10px', color: '#6366f1', fontWeight: '800', width: '50px', lineHeight: '1' },
  totalCount: { fontSize: '28px', fontWeight: '900', color: '#6366f1' },
  verticalDivider: { height: '40px', width: '1px', background: 'rgba(255,255,255,0.3)' },

  // TABLE STYLES
  table: { width: '100%', borderCollapse: 'collapse', marginTop: '10px' },
  th: { textAlign: 'left', padding: '15px 12px', fontSize: '11px', color: '#94a3b8', fontWeight: '800', borderBottom: '1px solid #f1f5f9' },
  td: { padding: '15px 12px', fontSize: '13px', borderBottom: '1px solid #f8fafc' },
  ticketIdLink: { color: '#4f46e5', fontWeight: '800', cursor: 'pointer', textDecoration: 'none' },
  descriptionText: { color: '#475569', fontWeight: '500', maxWidth: '400px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  statusBadge: { padding: '4px 12px', borderRadius: '6px', fontSize: '10px', fontWeight: '800', border: '1px solid' },
  updateText: { color: '#94a3b8', fontSize: '12px', fontStyle: 'italic' },

  // REST
  card: { background: '#fff', borderRadius: '16px', overflow: 'hidden', border: '1px solid #e2e8f0', height: '100%', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' },
  tableScroll: { padding: '0 20px', overflowY: 'auto' },
  ...
  metricLabel: { fontSize: '10px', fontWeight: '800', color: '#94a3b8' },
  metricValue: { margin: 0, fontSize: '24px', fontWeight: '900' },
  ...
  pagBtn: { padding: '5px 15px', background: '#f1f5f9', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: '700' }
};
