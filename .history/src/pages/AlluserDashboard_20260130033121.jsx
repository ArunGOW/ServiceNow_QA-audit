...
//           background: 'linear-gradient(145deg, #d0cbcb, #f9fafb)'
//         }}
...
//   <div style={styles.bannerLeft}>
//               {activeTab === 'pending' && (
//                 <div style={styles.rocketIcon}><i className="bi bi-rocket-takeoff-fill"></i></div>
//               )}
//               <div>
//                 <h6 style={activeTab === 'pending' ? styles.bannerTitleLarge : styles.cardTitle}>
//                   {activeTab === 'pending' ? "My Pending Queues" : "Performance Rankings"}
//                 </h6>
//                 {activeTab === 'pending' && <p style={styles.bannerSub}>Incident Management System</p>}
//               </div>
//             </div>
...
//   // Existing styles...
//   container: { backgroundColor: '#f4f7fa', height: '100vh', overflow: 'hidden', padding: '12px 20px', fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column', position: 'relative' },
//   header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', backgroundColor: '#1e293b', padding: '15px 20px', borderRadius: '10px', boxShadow: '0 2px 4px rgba(0,0,0,0.03)' },
//   titleArea: { display: 'flex', alignItems: 'center', gap: '20px' },
//   logo: { margin: 0, fontSize: '20px', fontWeight: '900', letterSpacing: '-0.4px', color: '#ffff' },
//   tabGroup: { background: '#c8cbcf', padding: '3px', borderRadius: '8px', display: 'flex' },
//   activeTab: { padding: '5px 12px', background: '#fff', color: '#4f46e5', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '700', fontSize: '12px', boxShadow: '0 1px 2px rgba(0,0,0,0.08)' },
//   inactiveTab: { padding: '5px 12px', background: 'transparent', color: '#64748b', border: 'none', cursor: 'pointer', fontWeight: '500', fontSize: '12px' },
//   filterStrip: { display: 'flex', gap: '6px' },
...
//   // The container for the icon and text
//   bannerLeft: {
//     display: 'flex',
//     alignItems: 'center',
//     gap: '20px',
...
//   // The glass-morphism rocket icon box
//   rocketIcon: {
//     width: '35px',
//     height: '35px',
...
//   // Large title for the Pending Banner
//   bannerTitleLarge: {
//     color: '#ffffff',
//     margin: 0,
//     fontSize: '22px',
...
//   // Small subtitle under the Pending title
//   bannerSub: {
...
//   // Standard title for Team/Individual cards (Non-banner)
//   cardTitle: {
//     margin: 0,
//     fontSize: '16px',
//     fontWeight: '700',
...
//   mediumBanner: {
//    background: 'linear-gradient(135deg, rgb(143 141 181) 0%, rgb(165 156 156) 100%)',
//     padding: '10px 25px',
//     display: 'flex',
...
//   filterLabelLite: { fontSize: '9px', fontWeight: '800', opacity: 0.6, textTransform: 'uppercase', display: 'block', marginBottom: '4px',color:'white' },
//   glassButton: { 
...
const AllusersDashboard = () => {
  ...
  const options = [
    { id: '', label: 'All Incidents', color: '#ffffff', icon: 'bi-grid-fill' },
    { id: 'resolved', label: 'Resolved', color: '#10b981', icon: 'bi-check-circle-fill' },
    { id: 'escalated', label: 'Escalated', color: '#f43f5e', icon: 'bi-fire' },
    { id: 'in progress', label: 'In Progress', color: '#f59e0b', icon: 'bi-clock-history' },
  ];
  ...
  // 2. Fetch Dashboard (Team/Individual)
  useEffect(() => {
    if (activeTab === 'pending') return;
    const fetchDashboardData = async () => {
      setLoading(true);
      try {
        const token = user?.token || localStorage.getItem("session_token");
        const params = activeTab === 'individual' ? { agent_sid: selectedAgent, from_date: fromDate, to_date: toDate } : {};
        const res = await api.get('/users/admin/dashboard', { headers: { Authorization: `Bearer ${token}` }, params });
        setData(res.data);
      } catch (err) { console.error(err); }
      finally { setLoading(false); }
    };
    ...
    const fetchPending = async () => {
      ...
      try {
        ...
        const res = await api.post('/users/get-pending/incidents/all', 
          { page: currentPage, per_page: perPage, user_sid: user?.sid },
          { params: { resolution_status: statusFilter || undefined }, headers: { Authorization: `Bearer ${token}` }}
        );
        setPendingIncidents(res.data.response || []);
        setTotalPending(res.data.total || 0);
      } catch (err) { console.error(err); }
  ...
  // Summary Logic
  const summary = (activeTab === 'individual' ? data?.filtered?.summary : data?.all_time?.summary) || { total_tickets: 0, passed: 0, failed: 0, pending: 0, pass_percentage: 0 };
  const agentsList = (activeTab === 'individual' ? data?.filtered?.agent_performance : data?.all_time?.agent_performance) || [];
  const pieData = [{ name: 'Passed', value: summary.passed }, { name: 'Failed', value: summary.failed }, { name: 'Pending', value: summary.pending }];
    ...
    <div style={styles.container}>
      <header style={styles.header}>
        ...
        <div style={styles.filterStrip}>
          {activeTab === 'individual' && (
            <>
              <div style={styles.dropdownContainer}>
                <div style={styles.premiumButton} onClick={() => setIsUserOpen(!isUserOpen)}>
                ...
                {isUserOpen && (
                  <div style={styles.scrollableMenu}>
                    {userList.map(u => (
                      <div key={u.sid} style={styles.menuItem} onClick={() => { setSelectedAgent(u.sid); setIsUserOpen(false); }}>{u.full_name}</div>
                    ))}
                  </div>
                )}
              </div>
              <input type="date" style={styles.premiumDateInput} value={fromDate} onChange={e => setFromDate(e.target.value)} />
              <input type="date" style={styles.premiumDateInput} value={toDate} onChange={e => setToDate(e.target.value)} />
            </>
          )}
          ...
          {activeTab === 'pending' && (
             <div style={styles.dropdownContainer}>
                <div style={styles.premiumButton} onClick={() => setIsOpen(!isOpen)}>
                  <div style={{...styles.statusDot, background: selectedOption.color === '#ffffff' ? '#6366f1' : selectedOption.color }} />
                ...
                {isOpen && (
                  <div style={styles.scrollableMenu}>
                    {options.map(opt => (
                      <div key={opt.id} style={styles.menuItem} onClick={() => { setStatusFilter(opt.id); setIsOpen(false); }}>{opt.label}</div>
                    ))}
                  </div>
                )}
             </div>
      ...
      <div style={activeTab === 'pending' ? styles.fullView : styles.mainGrid}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} dataKey="value">
                    {pieData.map((_, i) => <Cell key={i} fill={['#10b981', '#f43f5e', '#f59e0b'][i]} />)}
                  </Pie>
                  <Tooltip />
                </PieChart>
              </ResponsiveContainer>
        ...
        <section style={styles.card}>
          ...
          <div style={styles.tableScroll}>
            {loading && <div style={styles.inlineLoader}>Loading Data...</div>}
            <table style={{...styles.table, opacity: loading ? 0.5 : 1}}>
              <thead>
                <tr>
                  <th style={styles.th}>{activeTab === 'pending' ? 'Incident No' : 'Analyst'}</th>
                  <th style={styles.th}>{activeTab === 'pending' ? 'Short Description' : 'Tickets'}</th>
                  <th style={styles.th}>{activeTab === 'pending' ? 'Status' : 'Success Rate'}</th>
                </tr>
              </thead>
              <tbody>
                {activeTab === 'pending' ? (
                  pendingIncidents.map((inc, i) => (
                    <tr key={i} style={styles.tr}>
                      <td style={styles.td}><span style={styles.incidentLink}>{inc.incident_number}</span></td>
                      <td style={styles.td}>{inc.short_description}</td>
                      <td style={styles.td}><span style={{...styles.badge, background: inc.status === 'resolved' ? '#dcfce7' : '#fee2e2'}}>{inc.status}</span></td>
                    </tr>
                  ))
                ) : (
                  agentsList.map((a, i) => (
                    <tr key={i} style={styles.tr}>
                      <td style={styles.td}>{a.agent}</td>
                      <td style={styles.td}>{a.total_tickets}</td>
                      <td style={styles.td}>
                        <div style={styles.barContainer}>
                          <div style={{...styles.progressBar, width: `${a.pass_percentage}%`, background: '#4f46e5'}} />
                          <span style={styles.barText}>{a.pass_percentage}%</span>
                        </div>
                      </td>
                    </tr>
...
const styles = {
  container: { backgroundColor: '#f4f7fa', height: '100vh', padding: '15px 25px', fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', background: '#1e293b', padding: '15px 20px', borderRadius: '12px' },
  logo: { color: '#fff', margin: 0, fontSize: '20px', fontWeight: '900' },
  tabGroup: { background: '#334155', padding: '4px', borderRadius: '8px', display: 'flex', marginLeft: '20px' },
  activeTab: { padding: '6px 15px', background: '#fff', color: '#4f46e5', border: 'none', borderRadius: '6px', fontWeight: '700', fontSize: '12px', cursor: 'pointer' },
  inactiveTab: { padding: '6px 15px', background: 'transparent', color: '#94a3b8', border: 'none', fontWeight: '600', fontSize: '12px', cursor: 'pointer' },
  
  // Anti-Shake Layout
  mainGrid: { display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '15px', flex: 1, minHeight: '500px' },
  fullView: { display: 'block', flex: 1, minHeight: '500px' },
  card: { background: '#fff', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', height: '100%' },
  
  // Banner (The Purple Request)
  mediumBanner: { background: 'linear-gradient(135deg, #6366f1 0%, #a855f7 100%)', padding: '20px 25px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  bannerLeft: { display: 'flex', alignItems: 'center', gap: '15px' },
  rocketIcon: { width: '40px', height: '40px', background: 'rgba(255,255,255,0.2)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '20px' },
  bannerTitleLarge: { color: '#fff', margin: 0, fontSize: '20px', fontWeight: '800' },
  bannerSub: { color: 'rgba(255,255,255,0.7)', margin: 0, fontSize: '11px' },
  
  // Loader & Table
  inlineLoader: { position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: 'rgba(255,255,255,0.9)', padding: '10px 20px', borderRadius: '20px', zIndex: 10, fontWeight: '700', color: '#4f46e5', boxShadow: '0 4px 15px rgba(0,0,0,0.1)' },
  tableScroll: { flex: 1, overflowY: 'auto', padding: '15px', position: 'relative', minHeight: '300px' },
  table: { width: '100%', borderCollapse: 'collapse', transition: 'opacity 0.2s' },
  th: { textAlign: 'left', padding: '12px', fontSize: '10px', color: '#94a3b8', fontWeight: '800', textTransform: 'uppercase' },
  td: { padding: '12px', fontSize: '13px', borderBottom: '1px solid #f8fafc', color: '#334155' },
  incidentLink: { color: '#4f46e5', fontWeight: '700' },
  badge: { padding: '2px 8px', borderRadius: '6px', fontSize: '10px', fontWeight: '700' },
  
  // Filters
  filterStrip: { display: 'flex', gap: '10px' },
  premiumButton: { background: '#334155', color: '#fff', padding: '8px 15px', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '12px' },
  statusDot: { width: '8px', height: '8px', borderRadius: '50%', background: '#6366f1' },
  scrollableMenu: { position: 'absolute', top: '100%', right: 0, background: '#fff', borderRadius: '8px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', zIndex: 100, padding: '5px' },
  menuItem: { padding: '8px 15px', fontSize: '12px', cursor: 'pointer', color: '#334155' },
  premiumDateInput: { background: '#334155', border: 'none', color: '#fff', borderRadius: '8px', padding: '0 10px', fontSize: '12px' },

  // KPI
  kpiRow: { display: 'flex', background: '#fff', borderRadius: '12px', marginBottom: '15px', padding: '20px 0', border: '1px solid #e2e8f0' },
  metricBox: { flex: 1, padding: '0 20px' },
  metricLabelArea: { display: 'flex', alignItems: 'center', gap: '8px' },
  metricLabel: { fontSize: '10px', color: '#94a3b8', fontWeight: '800' },
  metricValue: { margin: '5px 0 0 0', fontSize: '22px', fontWeight: '900' },
  
  cardHeader: { padding: '15px 20px', borderBottom: '1px solid #f1f5f9' },
  cardTitle: { margin: 0, fontSize: '14px', fontWeight: '700', color: '#1e293b' },
  barContainer: { display: 'flex', alignItems: 'center', gap: '10px' },
  progressBar: { height: '6px', borderRadius: '3px' },
  barText: { fontSize: '11px', fontWeight: '700' },
  glassBadge: { background: 'rgba(255,255,255,0.1)', padding: '5px 12px', borderRadius: '8px', color: '#fff', fontSize: '11px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '8px' },
  pulseDot: { width: '6px', height: '6px', background: '#34d399', borderRadius: '50%' }
};
