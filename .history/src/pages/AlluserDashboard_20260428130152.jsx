...
//   statValue: { fontSize: '18px', fontWeight: '900', color: '#1e293b' },
//   mainGrid: { display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '15px', flex: 1, minHeight: 0 },
...
const AllusersDashboard = () => {
  ...
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(false);
  const [timeframe, setTimeframe] = useState('overall'); // overall, weekly, monthly
        ...
        <div style={styles.premiumTooltip}>
          ...
          <div style={styles.tooltipBody}>
            <div style={styles.tooltipRow}>
              ...
              <span style={styles.tooltipLabel}>Tickets:</span>
              <span style={styles.tooltipValue}>{payload[0].value}</span>
            </div>
            <div style={styles.tooltipFooter}>Click bar for details</div>
          </div>
        </div>
      );
    }
          ...
          {activeTab === 'pending' ? (
            ...
            <div style={styles.dropdownContainer}>
              ...
              <select 
                value={timeframe} 
                onChange={(e) => setTimeframe(e.target.value)}
                style={{...styles.premiumDateInput, fontWeight: '700', cursor: 'pointer'}}
              >
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        {activeTab === 'team' && (
          <div style={styles.bannerContainer}>
            ...
            <div style={styles.statBox}>
              <div style={{ ...styles.iconCircle, background: 'rgba(16, 185, 129, 0.1)', color: '#10b981' }}><i className="bi bi-shield-check"></i></div>
              <div style={styles.statInfo}><span style={styles.statLabel}>Passed / Audited</span><span style={{ ...styles.statValue, color: '#10b981' }}>{summary.passed}</span></div>
            </div>
            {/* ADDED BREACHED SLA SECTION */}
            <div style={styles.statBox}>
              <div style={{ ...styles.iconCircle, background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444' }}><i className="bi bi-exclamation-octagon-fill"></i></div>
              <div style={styles.statInfo}>
                <span style={styles.statLabel}>Team Health</span>
                <span style={{ ...styles.statValue, color: '#ef4444', fontSize: '14px' }}>BREACHED SLA</span>
              </div>
            </div>
            <div style={styles.statBox}>
              <div style={{ ...styles.iconCircle, background: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b' }}><i className="bi bi-graph-up-arrow"></i></div>
              <div style={styles.statInfo}><span style={styles.statLabel}>Quality Rate</span><span style={{ ...styles.statValue, color: '#f59e0b' }}>{summary.pass_percentage}%</span></div>
            </div>
          </div>
        )}
        ...
        <div style={activeTab === 'pending' ? styles.fullView : styles.mainGrid}>
          {activeTab === 'team' ? (
            <>
              <section style={styles.card}>
                <div style={styles.cardHeader}><h6 style={styles.cardTitle}>Performance Analytics</h6></div>
                <div style={styles.tableScroll}>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={agentsList} margin={{ top: 20, right: 30, left: 0, bottom: 40 }}>
                      ...
                      <XAxis 
                        dataKey="agent" 
                        axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 700, fill: '#94a3b8' }}
                        interval={0} angle={-25} textAnchor="end"
                        tickFormatter={(val) => formatName(val.split(' ')[0])}
                      />
                      <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 700, fill: '#94a3b8' }} />
                      <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(99, 102, 241, 0.05)' }} />
                      <Bar dataKey="total_tickets" fill="url(#barGradient)" radius={[6, 6, 0, 0]} barSize={32}>
                        {agentsList.map((entry, index) => (
                          <Cell key={`cell-${index}`} style={{ transition: 'all 0.3s ease', cursor: 'pointer' }} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
              ...
              <section style={styles.card}>
                <div style={styles.cardHeader}><h6 style={styles.cardTitle}>Analyst Ranking</h6></div>
                <div style={styles.tableScroll}>
                  <table style={styles.table}>
                    <thead>
                      <tr><th style={styles.th}>Analyst</th><th style={styles.th}>Tickets</th><th style={styles.th}>Quality</th></tr>
                    </thead>
                    <tbody>
                      {agentsList.map((a, i) => (
                        <tr key={i} style={styles.tr}>
                          ...
                          <td style={styles.td}>{a.total_tickets}</td>
                          <td style={styles.td}><span style={{fontWeight:'800', color: '#6366f1'}}>{a.pass_percentage}%</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <tbody>
                    {pendingIncidents.map((incident, i) => (
                      <tr key={i} style={styles.tr}>
                        ...
                        <td style={styles.td}>{incident.short_description}</td>
                        <td style={styles.td}><span style={styles.miniBadgeGray}>{incident.status}</span></td>
                        <td style={styles.td}>{incident.resolution_shared || "---"}</td>
                      </tr>
                    ))}
                  </tbody>
...
const styles = {
  ...
  inactiveTab: { padding: '5px 12px', background: 'transparent', color: '#64748b', border: 'none', cursor: 'pointer', fontWeight: '600', fontSize: '11px' },
  filterStrip: { display: 'flex', gap: '8px' },
  mainGrid: { display: 'grid', gridTemplateColumns: '0.80fr 1fr', gap: '12px', height: '100%', minHeight: 0 },
  fullView: { display: 'block', height: '100%' },
  bannerContainer: { display: 'flex', gap: '12px', marginBottom: '12px', flexShrink: 0 },
  statBox: { flex: 1, background: '#fff', padding: '12px 15px', borderRadius: '12px', border: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '12px' },
  iconCircle: { width: '40px', height: '40px', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px' },
  ...
  tooltipHeader: { fontSize: '11px', fontWeight: '800', color: '#1e293b', marginBottom: '5px' },
  tooltipLabel: { fontSize: '10px', color: '#64748b' },
  tooltipValue: { fontSize: '11px', fontWeight: '800', color: '#6366f1', marginLeft: '5px' },
  tooltipFooter: { fontSize: '8px', color: '#94a3b8', marginTop: '5px', textAlign: 'center' },
};
