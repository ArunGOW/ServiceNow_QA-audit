  ...
  <div style={{ ...styles.tableScroll, flex: 1, overflowY: 'auto' }}>
    <table style={styles.table}>
      ...
      <tbody>
        {agentsList.map((a, i) => (
          <tr key={i} style={styles.tr} onClick={() => handleAnalystClick(a)}>
            {/* Column 1: Analyst Info */}
            <td style={styles.td}>
              <div style={styles.avatarRow}>
                <div style={styles.miniAvatarIcon}>{a.agent?.charAt(0).toUpperCase() || 'A'}</div>
                <span>{formatName(a.agent || 'Unknown')}</span>
              </div>
            </td>

            {/* Column 2: Total Tickets */}
            <td style={styles.td}>
              <div style={{ fontWeight: '700', color: '#1e293b' }}>{a.total_tickets || 0}</div>
            </td>

            {/* Column 3: INC / RITM Breakdown */}
            <td style={styles.td}>
              <div style={{ display: 'flex', gap: '8px' }}>
                <div style={styles.countBadgeInc}>
                  ...
                  <span style={styles.countValue}>
                    {Array.isArray(a.tickets) 
                      ? a.tickets.filter(t => t.incident_number?.startsWith('INC')).length 
                      : (a.incident_count || 0)} 
                  </span>
                </div>
                <div style={styles.countBadgeRitm}>
                  ...
                  <span style={styles.countValue}>
                    {Array.isArray(a.tickets) 
                      ? a.tickets.filter(t => t.incident_number?.startsWith('RITM')).length 
                      : (a.ritm_count || 0)}
                  </span>
                </div>
              </div>
            ...
            <td style={styles.td}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ flex: 1, height: '6px', background: '#f1f5f9', borderRadius: '10px', minWidth: '60px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${a.pass_percentage || 0}%`, background: '#6366f1', borderRadius: '10px' }}></div>
                </div>
                <span style={{ fontWeight: '800', width: '35px' }}>{a.pass_percentage || 0}%</span>
              </div>
            </td>
          </tr>
        ))}
