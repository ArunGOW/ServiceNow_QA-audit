...
// Example of how to calculate these counts while mapping your analysts
const processedAnalysts = analysts.map(analyst => {
  const tickets = analyst.tickets || []; // Assuming tickets is an array in your API
  
  const incidentCount = tickets.filter(t => t.incident_number.startsWith('INC')).length;
  const requestCount = tickets.filter(t => t.incident_number.startsWith('RITM')).length;

  return {
    ...analyst,
    incidentCount,
    requestCount,
    totalHandled: tickets.length
  };
});
    ...
    <table style={styles.table}>
      <thead style={{ position: 'sticky', top: 0, background: '#fff', zIndex: 10 }}>
        <tr>
          ...
          <th style={styles.th}>Type Breakdown</th>
          <th style={styles.th}>Quality Score</th>
          <th style={styles.th}>Pass %</th> {/* Added to match body */}
        </tr>
      </thead>
       <tbody>
  {/* Changed agentsList to userList to match your state */}
  {userList?.map((a, i) => {
    // Safety check for tickets array
    const ticketArray = Array.isArray(a?.tickets) ? a.tickets : [];
    const incCount = ticketArray.filter(t => t.incident_number?.startsWith('INC')).length;
    const ritmCount = ticketArray.filter(t => t.incident_number?.startsWith('RITM')).length;

    return (
      <tr key={i} style={styles.tr} onClick={() => handleAnalystClick?.(a)}>
        <td style={styles.td}>
          <div style={styles.avatarRow}>
            <div style={styles.miniAvatarIcon}>
              {a.agent ? a.agent.charAt(0).toUpperCase() : 'A'}
            </div>
            <span>{a.agent ? formatName(a.agent) : 'Unknown Analyst'}</span>
          </div>
        </td>

        <td style={styles.td}>
          <div style={{ fontWeight: '700', color: '#1e293b' }}>{a.total_tickets || 0}</div>
        </td>

        <td style={styles.td}>
          <div style={{ display: 'flex', gap: '8px' }}>
            <div style={styles.countBadgeInc}>
              <span style={styles.countLabel}>INC</span>
              <span style={styles.countValue}>{incCount}</span>
            </div>
            <div style={styles.countBadgeRitm}>
              <span style={styles.countLabel}>RITM</span>
              <span style={styles.countValue}>{ritmCount}</span>
            </div>
          </div>
        </td>

        <td style={styles.td}>⭐ {a.average_score?.toFixed(1) || '0.0'}</td>
        ...
        <td style={styles.td}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontWeight: '800', width: '35px', fontSize: '11px' }}>
              {a.pass_percentage || 0}%
            </span>
          </div>
        </td>
...
mainGrid: { 
 ...
 premiumPill: {
  ...
  pillResolved: {
    backgroundColor: '#6a8173', // Dark green
    color: '#ffffff',           // White text for high contrast
    borderColor: '#15803d',     // Slightly lighter border
  },

  // Deep Crimson/Ruby for Escalated
  pillEscalated: {
    backgroundColor: '#f27272', // Dark red
  ...
  pillDefault: {
    backgroundColor: '#97baeb', 
    color: '#ffffff',
    borderColor: '#475569',
  },
  countBadgeInc: { display: 'flex', alignItems: 'center', backgroundColor: '#eff6ff', border: '1px solid #dbeafe', borderRadius: '4px', overflow: 'hidden' },
  countBadgeRitm: { display: 'flex', alignItems: 'center', backgroundColor: '#f5f3ff', border: '1px solid #ede9fe', borderRadius: '4px', overflow: 'hidden' },
  countLabel: { fontSize: '8px', fontWeight: '900', padding: '2px 4px', backgroundColor: 'rgba(0,0,0,0.05)', color: '#64748b' },
  countValue: { fontSize: '10px', fontWeight: '700', padding: '2px 6px', color: '#1e293b' },
};
