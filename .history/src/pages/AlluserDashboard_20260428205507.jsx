...
/>
  ...
  <div style={{ ...styles.tableScroll, flex: 1, overflowY: 'auto' }}>
   ...
   <tbody>
  {/* We use agentsList here as it usually contains the performance metrics */}
  {agentsList?.map((a, i) => {
    // 1. Find the tickets for this specific analyst from the userList 
    // or from a 'tickets' property if it exists on the agent object
    const analystTickets = a.tickets || []; 

    // 2. Filter for INC and RITM
    const incCount = analystTickets.filter(t => 
      t.incident_number?.toUpperCase().startsWith('INC')
    ).length;

    const ritmCount = analystTickets.filter(t => 
