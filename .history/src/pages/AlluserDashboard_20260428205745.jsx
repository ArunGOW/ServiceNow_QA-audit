...
/>
  ...
  <div style={{ ...styles.tableScroll, flex: 1, overflowY: 'auto' }}>
   ...
   <tbody>
  {agentsList?.map((a, i) => {
    // 1. If your API sends the counts directly, use them. 
    // 2. Otherwise, we calculate them from a 'tickets' array if available.
    const analystTickets = a.tickets || []; 

    let incCount = 0;
    let ritmCount = 0;

    if (analystTickets.length > 0) {
      incCount = analystTickets.filter(t => 
        t.incident_number?.toUpperCase().startsWith('INC')
      ).length;
      
      ritmCount = analystTickets.filter(t => 
        t.incident_number?.toUpperCase().startsWith('RITM')
      ).length;
    } else {
      // Fallback: Check if your API sends pre-calculated fields like a.inc_count
      incCount = a.inc_count || 0;
      ritmCount = a.ritm_count || 0;
    }
