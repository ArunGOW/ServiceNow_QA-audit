...
/>
  ...
  <div style={{ ...styles.tableScroll, flex: 1, overflowY: 'auto' }}>
   ...
   <tbody>
  {agentsList?.map((a, i) => {
    // 1. Find the full data for this analyst from the userList 
    // This assumes userList contains the 'tickets' or 'response' array you shared
    const analystInfo = userList.find(u => 
      u.full_name?.toLowerCase() === a.agent?.toLowerCase()
    );

    // 2. Access the ticket array (adjust property name based on your exact state)
    const tickets = analystInfo?.tickets || a.tickets || [];

    // 3. Perform the count
    const incCount = tickets.filter(t => 
      t.incident_number?.toUpperCase().startsWith('INC')
    ).length;

    const ritmCount = tickets.filter(t => 
      t.incident_number?.toUpperCase().startsWith('RITM')
    ).length;
      ...
      <tr key={i} style={styles.tr} onClick={() => handleAnalystClick(a)}>
        <td style={styles.td}>
          <div style={styles.avatarRow}>
            ...
            <span>{a.agent ? formatName(a.agent) : 'Unknown'}</span>
          </div>
        </td>
        ...
        <td style={styles.td}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ 
               ...
               borderRadius: '10px', minWidth: '60px', overflow: 'hidden' 
            }}>
              <div style={{ 
                height: '100%', 
