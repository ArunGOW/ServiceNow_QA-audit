  ...
  <section style={styles.card}>
    <div style={styles.detailHeader}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        <h3 style={{ margin: 0, fontSize: '14px', fontWeight: '800' }}>
          {formatName(selectedAgentData?.agent)}
        </h3>
        <span style={{ fontSize: '11px', color: '#64748b', fontWeight: '600' }}>
          Audit Phase: {selectedQaDetail.label}
        </span>
      </div>
    </div>
    ...
    <div style={styles.tableScroll}>
      {agentTickets.length > 0 ? (
        <>
            ...
            <tbody>
              {agentTickets.map((t, idx) => (
                <tr key={idx} style={styles.tr} onClick={() => openIncidentModal(t)}>
                  ...
                  <td style={styles.td}>{t.short_description}</td>
                  <td style={styles.td}>
                    <span style={styles.miniBadgeGray}>{t.status}</span>
                  </td>
                  <td style={styles.td}>
                    <span style={{
                      ...styles.premiumPill,
          ...
          <div style={styles.paginationArea}>
            <button 
              ...
              style={{ ...styles.pagBtn, ...(detailPage === 1 ? styles.pagBtnDisabled : {}) }}
            >
              <i className="bi bi-chevron-left"></i> Previous
            </button>
            <div style={styles.pageIndicator}>
              Page <span style={styles.pageNumber}>{detailPage}</span> of {Math.ceil((totalDetailTickets || 0) / 10) || 1}
            </div>
            <button 
              disabled={detailPage >= Math.ceil(totalDetailTickets / 10)} 
              onClick={() => setDetailPage(p => p + 1)} 
              style={{ ...styles.pagBtn, ...(detailPage >= Math.ceil(totalDetailTickets / 10) ? styles.pagBtnDisabled : {}) }}
            >
        ...
        <div style={styles.noDataContainer}>
          <i className="bi bi-search" style={styles.noDataIcon}></i>
          <p style={styles.noDataText}>No tickets found</p>
          <span style={styles.noDataSubtext}>
            {formatName(selectedAgentData?.agent)} has no tickets in the "{selectedQaDetail.label}" phase for the selected dates.
          </span>
        </div>
      )}
    </div>
