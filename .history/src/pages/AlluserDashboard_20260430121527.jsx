...
const AllusersDashboard = () => {
  ...
  <table style={styles.table} className="align-middle mb-0 refined-table compact-view">
    <thead style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 10 }}>
      <tr>
        {/* Checkbox Column - Matches Example */}
        <th style={{ width: "70px", padding: '12px' }}>
          <div className="d-flex flex-column align-items-center">
            <input
              type="checkbox"
              className="form-check-input custom-check-sm mb-1"
              style={{ cursor: 'pointer' }}
              checked={selectedIncidents.length === agentsList.length && agentsList.length > 0}
              onChange={(e) => {
                if (e.target.checked) setSelectedIncidents(agentsList.map((a) => a.agent));
                else setSelectedIncidents([]);
              }}
            />
            <span style={{ fontSize: '9px', fontWeight: '800', color: '#94a3b8' }}>ALL</span>
          </div>
        </th>
        <th style={styles.th} className="small-header">ANALYST</th>
        <th style={styles.th} className="small-header">TICKETS</th>
        <th style={styles.th} className="small-header">AVG SCORE</th>
        <th style={styles.th} className="small-header">QUALITY RATE</th>
      </tr>
    </thead>
    <tbody>
      {agentsList.map((a, i) => (
        <tr key={i} style={styles.tr} className="density-row" onClick={() => handleAnalystClick(a)}>
          {/* Individual Checkbox */}
          <td className="text-center" onClick={(e) => e.stopPropagation()}>
            <input
              type="checkbox"
              className="form-check-input"
              checked={selectedIncidents.includes(a.agent)}
              onChange={() => {
                setSelectedIncidents((prev) =>
                  prev.includes(a.agent)
                    ? prev.filter((id) => id !== a.agent)
                    : [...prev, a.agent]
                );
              }}
            />
          </td>
          {/* Analyst Name with Avatar */}
          <td style={styles.td}>
            <div style={styles.avatarRow}>
              <div style={styles.miniAvatarIcon} className="bg-light-primary text-primary">
          ...
          {/* Tickets - Matches fw-bold from example */}
          <td style={styles.td} className="fw-bold">
            <div className="incident-link-slim">
              <span className="text-muted fw-normal me-1">#</span>
              <span className="link-text" style={{ color: '#6366f1' }}>{a.total_tickets}</span>
            </div>
          </td>
          {/* Score */}
          <td style={styles.td} className="text-muted" style={{ fontSize: '13px' }}>
            <span className="badge-slim">⭐ {a.average_score?.toFixed(1) || 0}</span>
          </td>
          {/* Quality Rate with Progress Bar */}
          <td style={styles.td}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ flex: 1, height: '6px', background: '#f1f5f9', borderRadius: '10px', minWidth: '60px', overflow: 'hidden' }}>
                <div 
              ...
              <span style={{ fontWeight: '800', width: '35px', fontSize: '11px', color: '#475569' }}>
                {a.pass_percentage}%
              </span>
            </div>
          </td>
