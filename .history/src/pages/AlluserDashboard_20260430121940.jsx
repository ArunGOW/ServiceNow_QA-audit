  ...
  <table style={styles.table} className="align-middle mb-0 refined-table compact-view">
    <thead style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 10 }}>
      <tr>
        ...
        <th style={styles.th} className="small-header ps-4">ANALYST</th>
        <th style={styles.th} className="small-header">TICKETS</th>
        <th style={styles.th} className="small-header">AVG SCORE</th>
        <th style={styles.th} className="small-header pe-4">QUALITY RATE</th>
      </tr>
    </thead>
    <tbody>
      {agentsList.map((a, i) => (
        <tr key={i} style={styles.tr} className="density-row" onClick={() => handleAnalystClick(a)}>
          ...
          <td style={styles.td} className="ps-4">
            <div style={styles.avatarRow}>
          ...
          {/* Average Score */}
          <td style={styles.td}>
            <span className="badge-slim" style={{ fontSize: '13px', color: '#475569' }}>
              ⭐ {a.average_score?.toFixed(1) || "0.0"}
            </span>
          </td>

          {/* Quality Rate Progress Bar */}
          <td style={styles.td} className="pe-4">
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ flex: 1, height: '6px', background: '#f1f5f9', borderRadius: '10px', minWidth: '80px', overflow: 'hidden' }}>
                <div 
                  style={{ 
                    height: '100%', 
