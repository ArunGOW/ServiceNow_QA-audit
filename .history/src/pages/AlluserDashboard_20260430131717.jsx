...
const AllusersDashboard = () => {
      ...
      {agentsList.map((a, i) => (
        ...
        >
          ...
          {activeTab === 'team' && viewMode === 'detail' && (
            ...
            //   </div>
            // </section>
            <section style={{ ...styles.card, padding: '12px' }}>
  {/* Header Section */}
  <div style={{ 
    display: 'flex', 
    alignItems: 'center', 
    ...
    <div style={{ textAlign: 'right' }}>
      <span style={{ fontSize: '9px', color: '#94a3b8', fontWeight: '800', textTransform: 'uppercase' }}>Analyst Overview</span>
  ...
  <div style={{ ...styles.tableScroll, maxHeight: '420px', overflowY: 'auto' }}>
    <table style={{ ...styles.table, borderCollapse: 'separate' }} className="refined-table compact-view">
      <thead style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 10 }}>
        <tr>
          <th style={{ ...styles.th, padding: '8px 16px' }} className="small-header">INCIDENT NO</th>
          <th style={{ ...styles.th, padding: '8px 16px' }} className="small-header">DESCRIPTION</th>
          <th style={{ ...styles.th, padding: '8px 16px' }} className="small-header text-center">STATUS</th>
          <th style={{ ...styles.th, padding: '8px 16px' }} className="small-header text-center">PHASE</th>
        </tr>
      </thead>
      <tbody>
        {/* Using slice(0, 10) to ensure only 10 items show per page */}
        {agentTickets.slice(0, 10).map((t, idx) => (
          <tr 
            key={idx} 
            style={{ ...styles.tr, cursor: 'pointer' }} 
            className="density-row"
          ...
          >
            <td style={{ ...styles.td, padding: '4px 16px' }}>
              <div 
                style={{
                  ...
                  fontWeight: '700',
                  fontSize: '12px',
                  textDecoration: hoveredIndex === t ? 'underline' : 'none'
                }}
                onMouseEnter={() => setHoveredIndex(t)}
                onMouseLeave={() => setHoveredIndex(null)}
              >
                <span style={{ color: '#94a3b8', fontWeight: '400', marginRight: '2px' }}>#</span>
                {t.incident_number}
              </div>
            </td>
            ...
            <td style={{ ...styles.td, padding: '4px 16px' }}>
              <div style={{ 
                ...
                {t.short_description || "No description provided"}
              </div>
            </td>

            <td style={{ ...styles.td, padding: '4px 16px', textAlign: 'center' }}>
              <span style={{
                ...getStatusStyle(t.status),
                padding: '2px 8px',
                borderRadius: '50px',
            ...
            <td style={{ ...styles.td, padding: '4px 16px', textAlign: 'center' }}>
              <span style={{
                backgroundColor: (selectedQaDetail?.color || '#6366f1') + '15',
                color: selectedQaDetail?.color || '#6366f1',
                padding: '2px 8px',
                borderRadius: '50px',
                fontSize: '9px',
                fontWeight: '700',
                border: `1px solid ${(selectedQaDetail?.color || '#6366f1')}30`
              }}>
                {qaState.toUpperCase()}
              </span>
            </td>
  ...
  <div style={{ 
    ...
    <button
      ...
      style={{
        ...
        background: detailPage === 1 ? '#f1f5f9' : '#fff',
        color: detailPage === 1 ? '#94a3b8' : '#475569',
    ...
    <div style={{ fontSize: '11px', color: '#64748b', fontWeight: '700' }}>
      PAGE <span style={{ color: '#6366f1' }}>{detailPage}</span> OF {Math.ceil((totalDetailTickets || 0) / 10) || 1}
    </div>
    ...
    <button
      ...
      style={{
        ...
        borderRadius: '4px',
        border: '1px solid #cbd5e1',
        background: detailPage >= Math.ceil(totalDetailTickets / 10) ? '#f1f5f9' : '#fff',
        color: detailPage >= Math.ceil(totalDetailTickets / 10) ? '#94a3b8' : '#475569',
        cursor: detailPage >= Math.ceil(totalDetailTickets / 10) ? 'not-allowed' : 'pointer'
      }}
    >
      NEXT <i className="bi bi-chevron-right"></i>
    </button>
