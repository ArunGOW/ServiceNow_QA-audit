...
const AllusersDashboard = () => {
  ...
  const fetchGlobalSlaBreaches = async (page = 1) => {
    ...
    try {
      const token = user?.token || localStorage.getItem("session_token");
      const res = await api.post('/users/get/sla-breached/incidents',
        { user_sid: user?.sid || user?.user_sid, page, per_page: 10 },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setGlobalSlaData(res.data.response || []);
      setGlobalSlaTotal(res.data.total || 0);
  ...
  useEffect(() => {
    const fetchMyPending = async () => {
      ...
      try {
        const token = user?.token || localStorage.getItem("session_token");
        const res = await api.post('/users/get-pending/incidents/all',
          { user_sid: user?.sid || user?.user_sid, page: currentPage, per_page: perPage },
          {
            headers: { Authorization: `Bearer ${token}` },
            params: {
              resolution_status: statusFilter || undefined,
  ...
  const agentsList = data?.all_time?.agent_performance     || [];
  const qaAdminList= data?.all_time?.qa_admin_performance  || [];

  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '10px 14px', fontSize: '12px' }}>
          <p style={{ margin: 0, fontWeight: 700, color: '#1e293b' }}>{label}</p>
          <p style={{ margin: '4px 0 0', color: '#6366f1', fontWeight: 600 }}>{payload[0].value} tickets</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="dashboard-container">
      ...
      {(loading || detailLoading) && (
        <div className="shimmer-overlay"><div className="spinner"></div></div>
      )}
        ...
        <div className="filter-strip">
          ...
          {activeTab === 'team' && viewMode === 'detail' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div className="dropdown-container">
                <label className="filter-label">Reset</label>
                <button onClick={resetDetailDates} className="reset-btn"><i className="bi bi-arrow-counterclockwise"></i></button>
              </div>
            </div>
          )}
          ...
          {activeTab === 'pending' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div className="dropdown-container">
                <label className="filter-label">Reset</label>
                <button onClick={resetPendingDates} className="reset-btn"><i className="bi bi-arrow-counterclockwise"></i></button>
              </div>
            </div>
          )}
        </div>
      ...
      {/* ── MAIN CONTENT ── */}
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflowY: 'auto', gap: '12px', paddingBottom: '12px' }}>

        {/* ── STATS BANNER (Team tab) ── */}
        {activeTab === 'team' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', flexShrink: 0 }}>
            <div className={`stat-box stat-box--clickable ${!showGlobalSla && viewMode === 'list' ? 'stat-box--active-default' : ''}`} onClick={() => setShowGlobalSla(false)}>
              <div className="icon-circle icon-circle--indigo"><i className="bi bi-ticket-perforated-fill"></i></div>
              <div className="stat-info">
                <span className="stat-label">Total Tickets</span>
                <span className="stat-value">{viewMode === 'list' ? (summary.total_tickets || 0) : (selectedAgentData?.total_tickets || 0)}</span>
              </div>
            </div>
            ...
            <div className="stat-box">
              ...
              <div className="stat-info">
                ...
                <span className="stat-value stat-value--green">{viewMode === 'list' ? (summary.passed || 0) : (selectedAgentData?.passed || 0)}</span>
              </div>
            </div>

            <div className="stat-box">
              <div className="icon-circle" style={{ background: 'rgba(99,102,241,0.1)', color: '#6366f1' }}><i className="bi bi-inbox-fill"></i></div>
              <div className="stat-info">
                <span className="stat-label">Unassigned QA</span>
                <span className="stat-value" style={{ color: '#6366f1' }}>{summary.unassigned_qa_tickets || 0}</span>
              </div>
            </div>

            {viewMode === 'list' ? (
              <div className={`stat-box stat-box--clickable ${showGlobalSla ? 'stat-box--active-sla' : ''}`} onClick={() => setShowGlobalSla(true)}>
                <div className="icon-circle icon-circle--red"><i className="bi bi-exclamation-octagon-fill"></i></div>
                <div className="stat-info">
                  <span className="stat-label">SLA Breached</span>
                  <span className="stat-value stat-value--red">{summary.sla_breached || 0}</span>
                </div>
              </div>
            ) : (
              <div className="stat-box">
            ...
            <div className="stat-box">
              ...
              <div className="stat-info">
                <span className="stat-label">Quality Rate</span>
                <span className="stat-value stat-value--amber">
                  {viewMode === 'list' ? `${summary.pass_percentage || 0}%` : `${selectedAgentData?.pass_percentage || 0}%`}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="table-scroll">
              {globalSlaData.length > 0 ? (
                <>
                  <table className="data-table">
                    <thead>
                      <tr><th>Incident No</th><th>Actioned By</th><th>Description</th><th>Status</th></tr>
                    </thead>
                    <tbody>
                      {globalSlaData.map((item, i) => (
                        <tr key={i}>
                          <td className="incident-link">{item.incident_number}</td>
                          <td>{formatName(item.done_by)}</td>
                          <td className="desc-cell">{item.short_description}</td>
                          <td><span className="badge-red">SLA Breached</span></td>
                        </tr>
                      ))}
                    </tbody>
                  ...
                  <div className="pagination-area">
                    <button disabled={globalSlaPage === 1} onClick={() => setGlobalSlaPage(p => p - 1)} className="pag-btn">
                      <i className="bi bi-chevron-left"></i> Prev
                    </button>
                    <button disabled={globalSlaPage >= Math.ceil(globalSlaTotal / 10)} onClick={() => setGlobalSlaPage(p => p + 1)} className="pag-btn">
                      Next <i className="bi bi-chevron-right"></i>
                    </button>
                  </div>
                </>
                ...
                <div className="no-data-container">
                  <i className="bi bi-patch-check no-data-icon"></i>
                  <p className="no-data-text">No SLA breaches found</p>
                  <span className="no-data-subtext">All incidents are within service level targets.</span>
                </div>
              )}
            </div>
          </section>
        ...
        {activeTab === 'team' && viewMode === 'list' && !showGlobalSla && (
          <>
            {/* Row 1: Chart + Expanded Analyst Table */}
            <div className="main-grid" style={{ flexShrink: 0 }}>
              ...
              <section className="card">
                ...
                <div className="table-scroll" style={{ padding: '10px' }}>
                  <ResponsiveContainer width="100%" height={240}>
                    <BarChart data={agentsList} margin={{ top: 16, right: 20, left: 0, bottom: 20 }}>
                      <defs>
                        <linearGradient id="barGradient" x1="0" y1="0" x2="0" y2="1">
              ...
              {/* ── EXPANDED Analyst Performance Table ── */}
              <section className="card" style={{ minHeight: 0, display: 'flex', flexDirection: 'column' }}>
                <div className="card-header">
                  <h6 className="card-title">Analyst Performance Directory</h6>
                  <span className="pill" style={{ background: '#eef2ff', color: '#6366f1', border: '0.5px solid #c7d2fe' }}>
                ...
                <div style={{ flex: 1, overflowX: 'auto', overflowY: 'auto' }}>
                  <table className="data-table" style={{ minWidth: '640px' }}>
                    <thead style={{ background: '#f8fafc', position: 'sticky', top: 0, zIndex: 10 }}>
                      <tr>
                        <th style={{ padding: '8px 12px', whiteSpace: 'nowrap' }}>Analyst</th>
                        <th style={{ padding: '8px 10px', textAlign: 'center', whiteSpace: 'nowrap' }}>Total</th>
                        <th style={{ padding: '8px 10px', textAlign: 'center', whiteSpace: 'nowrap' }}>INC</th>
                        <th style={{ padding: '8px 10px', textAlign: 'center', whiteSpace: 'nowrap' }}>RIT</th>
                        <th style={{ padding: '8px 10px', textAlign: 'center', whiteSpace: 'nowrap' }}>Passed</th>
                        <th style={{ padding: '8px 10px', textAlign: 'center', whiteSpace: 'nowrap' }}>Failed</th>
                        <th style={{ padding: '8px 10px', textAlign: 'center', whiteSpace: 'nowrap' }}>Unassigned</th>
                        <th style={{ padding: '8px 12px', whiteSpace: 'nowrap' }}>Quality</th>
                      </tr>
                    </thead>
                    <tbody>
                      {agentsList.map((a, i) => (
                        <tr key={i} onClick={() => handleAnalystClick(a)} style={{ cursor: 'pointer' }}>

                          {/* Analyst name + performer badge */}
                          <td style={{ padding: '6px 12px' }}>
                            <div className="avatar-row">
                              <div className="mini-avatar mini-avatar--small">
                                {a.agent ? a.agent.charAt(0).toUpperCase() : 'A'}
                              </div>
                              <span style={{ fontSize: '11px', fontWeight: 500 }}>{formatName(a.agent)}</span>
                              {a.is_top_performer && (
                                <span style={{ background: '#dcfce7', color: '#166534', padding: '1px 5px', borderRadius: '3px', fontSize: '8px', fontWeight: 600 }}>Top</span>
                              )}
                              {a.is_bottom_performer && (
                                <span style={{ background: '#fee2e2', color: '#991b1b', padding: '1px 5px', borderRadius: '3px', fontSize: '8px', fontWeight: 600 }}>Low</span>
                              )}
                            </div>
                          </td>
                          ...
                          <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                            <span style={{ color: '#6366f1', fontWeight: 600, fontSize: '11px' }}>{a.total_tickets || 0}</span>
                          </td>

                          {/* INC pill */}
                          <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                            <span style={{ background: '#eef2ff', color: '#6366f1', padding: '2px 7px', borderRadius: '20px', fontSize: '9px', fontWeight: 600 }}>
                              {a.ticket_split?.INC ?? 0}
                            </span>
                          </td>

                          {/* RIT pill */}
                          <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                            <span style={{ background: '#fef3c7', color: '#92400e', padding: '2px 7px', borderRadius: '20px', fontSize: '9px', fontWeight: 600 }}>
                              {a.ticket_split?.RIT ?? 0}
                            </span>
                          </td>

                          {/* Passed */}
                          <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                            <span style={{ background: '#dcfce7', color: '#166534', padding: '2px 7px', borderRadius: '20px', fontSize: '9px', fontWeight: 600 }}>
                              {a.passed || 0}
                            </span>
                          </td>
                          ...
                          {/* Failed */}
                          <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                            <span style={{ background: '#fee2e2', color: '#991b1b', padding: '2px 7px', borderRadius: '20px', fontSize: '9px', fontWeight: 600 }}>
                              {a.failed || 0}
                            </span>
                          </td>

                          {/* Unassigned (qc_remaining) */}
                          <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                            <span style={{ background: '#f1f5f9', color: '#475569', padding: '2px 7px', borderRadius: '20px', fontSize: '9px', fontWeight: 600 }}>
                              {a.qc_remaining || 0}
                            </span>
                          </td>
                            ...
                            <div className="progress-wrap">
                              <div className="progress-track">
                                <div
                                  ...
                                  style={{
                                    width: `${a.pass_percentage || 0}%`,
                                    background: (a.pass_percentage >= 80) ? '#10b981' : (a.pass_percentage >= 50) ? '#f59e0b' : '#e24b4a',
                                  }}
                                />
                              </div>
                              <span className="progress-label">
                                {a.pass_percentage !== null && a.pass_percentage !== undefined ? `${a.pass_percentage}%` : '—'}
                              </span>
                            </div>
                          </td>
                        </tr>
            ...
            <section style={{
              ...
              <div style={{
                background: 'linear-gradient(180deg, #2c3e97 0%, #1a255e 100%)',
                padding: '11px 18px',
                display: 'flex',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <i className="bi bi-person-badge-fill" style={{ color: '#fff', fontSize: '14px' }}></i>
                  <h6 style={{ margin: 0, fontSize: '13px', fontWeight: 600, color: '#fff' }}>
                ...
                <span style={{
                  ...
                  {qaAdminList.length} Auditors
                </span>
              </div>

              {/* Card columns — one per QA admin */}
              {qaAdminList.length === 0 ? (
                <div className="no-data-container" style={{ padding: '40px' }}>
                  <p className="no-data-text">No QA admin data available</p>
                </div>
              ) : (
                <div style={{
                  display: 'grid',
                  gridTemplateColumns: `repeat(${qaAdminList.length}, 1fr)`,
                  borderTop: '0.5px solid #f1f5f9',
                }}>
                  {qaAdminList.map((qa, idx) => {
                    const pct = qa.qc_coverage_percentage ?? 0;
                    const badgeStyle = coverageBadgeStyle(pct);
                    const barColor = pct >= 80 ? '#10b981' : pct >= 40 ? '#f59e0b' : '#e24b4a';

                    return (
                      <div key={idx} style={{
                        padding: '14px 16px',
                        borderRight: idx < qaAdminList.length - 1 ? '0.5px solid #f1f5f9' : 'none',
                        position: 'relative',
                      }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                          <div style={{
                            ...
                            border: '0.5px solid #c7d2fe', flexShrink: 0,
                          }}>
                            {qa.qa_admin?.charAt(0)?.toUpperCase() || '?'}
                          </div>
                          <span style={{ fontSize: '12px', fontWeight: 600, color: '#1e293b' }}>
                            {formatName(qa.qa_admin)}
                          </span>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '10px' }}>
                          {[
                            { label: 'Assigned',  value: qa.assigned_incidents ?? 0 },
                            { label: 'QC done',   value: qa.qc_done ?? 0 },
                            { label: 'Remaining', value: qa.qc_remaining ?? 0 },
                          ].map(row => (
                            <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <span style={{ fontSize: '10px', color: '#64748b' }}>{row.label}</span>
                              <span style={{ fontSize: '11px', fontWeight: 600, color: '#1e293b' }}>{row.value}</span>
                            </div>
                          ))}
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            {qa.pass_percentage === null || qa.pass_percentage === undefined ? (
                              <span style={{ background: '#f1f5f9', color: '#64748b', padding: '1px 7px', borderRadius: '20px', fontSize: '9px', fontWeight: 600 }}>—</span>
                            ) : qa.pass_percentage >= 80 ? (
                              <span style={{ background: '#dcfce7', color: '#166534', padding: '1px 7px', borderRadius: '20px', fontSize: '9px', fontWeight: 600 }}>{qa.pass_percentage}%</span>
                            ) : (
                              <span style={{ background: '#fee2e2', color: '#991b1b', padding: '1px 7px', borderRadius: '20px', fontSize: '9px', fontWeight: 600 }}>{qa.pass_percentage}%</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div>
                          <div style={{ fontSize: '9px', color: '#94a3b8', marginBottom: '3px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                            Coverage
                          </div>
          ...
          <section className="card" style={{ flex: 1 }}>
            ...
            <div className="table-scroll">
                ...
                <tbody>
                  {agentTickets.map((t, idx) => (
                    <tr key={idx} onClick={() => openIncidentModal(t)}>
                      ...
                      <td className="desc-cell">{t.short_description}</td>
                      <td>
                        <span className={`premium-pill ${getStatusBadgeClass(t.status)}`}>{t.status}</span>
                      </td>
                      <td>
                        <span className="premium-pill" style={{ backgroundColor: selectedQaDetail.color + '15', color: selectedQaDetail.color }}>
                          {qaState.toUpperCase()}
                        </span>
                      </td>
                    </tr>
              ...
              <div className="pagination-area">
                ...
                <div className="page-indicator">
                  Page <span className="page-number">{detailPage}</span> of {Math.ceil((totalDetailTickets || 0) / 10) || 1}
                </div>
                <button disabled={detailPage >= Math.ceil(totalDetailTickets / 10)} onClick={() => setDetailPage(p => p + 1)} className="pag-btn">
                  Next <i className="bi bi-chevron-right"></i>
                </button>
              ...
              {pendingIncidents.length > 0 ? (
                <>
                  ...
                  <div className="pagination-area">
                    ...
                    <div className="page-indicator">
                      Page <span className="page-number">{currentPage}</span> of {Math.ceil((totalPending || 0) / perPage) || 1}
                    </div>
                    <button disabled={currentPage >= Math.ceil(totalPending / perPage)} onClick={() => setCurrentPage(p => p + 1)} className="pag-btn">
                      Next <i className="bi bi-chevron-right"></i>
                    </button>
                ...
                <div className="no-data-container">
                  ...
                  <p className="no-data-text">No records found</p>
                  <span className="no-data-subtext">
                    No incidents matching "{selectedQaPending.label}" for the selected criteria.
                  </span>
                </div>
              )}
            </div>
      ...
      {isModalOpen && selectedIncident && (
        <div className="modal-overlay" onClick={() => setIsModalOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="agent-info-card">
                <div className="mini-avatar">{selectedIncident.display_name.charAt(0).toUpperCase()}</div>
                <div>
                  <div className="agent-info-meta">ACTIONED BY</div>
                  <div className="agent-info-name">{formatName(selectedIncident.display_name)}</div>
                </div>
