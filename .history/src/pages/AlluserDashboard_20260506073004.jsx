...
const AllusersDashboard = () => {
  ...
  const [userList, setUserList]               = useState([]);
  const [data, setData]                       = useState(null);
  const [initLoading, setInitLoading]         = useState(false);  // full overlay — tab/filter switches only
  const [pageLoading, setPageLoading]         = useState(false);  // inline — pagination only
  const [detailLoading, setDetailLoading]     = useState(false);
  const [hoveredRow, setHoveredRow]           = useState(null);
  ...
  /* ── fetches ── */
  const fetchSla = async (page = 1, isPageChange = false) => {
    isPageChange ? setPageLoading(true) : setInitLoading(true);
    try {
      const r = await api.post('/users/get/sla-breached/incidents', { user_sid: sid(), page, per_page: 10 }, { headers: { Authorization: `Bearer ${tok()}` } });
      setSlaData(r.data.response || []); setSlaTotal(r.data.total || 0);
    } catch (e) { console.error(e); } finally { setPageLoading(false); setInitLoading(false); }
  };

  useEffect(() => { if (showSla && activeTab === 'team') fetchSla(slaPage, slaPage > 1); }, [showSla, slaPage, activeTab]);
  ...
  const prevPendPageRef = React.useRef(1);
  useEffect(() => {
    if (activeTab !== 'pending') return;
    const isPageChange = pendPage !== prevPendPageRef.current && activeTab === 'pending';
    prevPendPageRef.current = pendPage;
    isPageChange ? setPageLoading(true) : setInitLoading(true);
    (async () => {
      try {
        const r = await api.post('/users/get-pending/incidents/all',
        ...
        setPending(r.data.response || []); setPendTotal(r.data.total || 0);
      } catch (e) { console.error(e); setPending([]); } finally { setPageLoading(false); setInitLoading(false); }
    })();
  }, [activeTab, pendPage, statusFilter, pendQa, pendFrom, pendTo, user]);

  const fetchDetail = async (agent, page = 1, isPageChange = false) => {
    isPageChange ? setPageLoading(true) : setDetailLoading(true);
    try {
      const found = userList.find(u => u.full_name.toLowerCase() === agent.agent.toLowerCase());
      if (found) {
        const r = await api.post('/users/get-pending/incidents/all',
    ...
    } catch (e) { console.error(e); setAgentTickets([]); } finally { setPageLoading(false); setDetailLoading(false); }
  };

  useEffect(() => { if (viewMode === 'detail' && selAgent) fetchDetail(selAgent, detPage, true); }, [detPage]);
  useEffect(() => { if (viewMode === 'detail' && selAgent) fetchDetail(selAgent, detPage, false); }, [detFrom, detTo, detQa]);

  const openDetail = (a) => { setSelAgent(a); setViewMode('detail'); setDetPage(1); fetchDetail(a, 1, false); };
  const openModal  = (t) => { setSelInc({ ...t, display_name: selAgent?.agent || t.done_by || 'System' }); setModal(true); };
  ...
  /* ── Inline page-loading bar (no overlay, no shake) ── */
  const PageLoadBar = () => pageLoading ? (
    <div style={{ height: 2, background: '#eef2ff', borderRadius: 2, margin: '0 16px', overflow: 'hidden' }}>
      <div style={{
        height: '100%', width: '40%', background: '#6366f1', borderRadius: 2,
        animation: 'slideBar 1s ease-in-out infinite alternate',
      }} />
      <style>{`@keyframes slideBar { from { margin-left: 0 } to { margin-left: 60% } }`}</style>
    </div>
  ) : null;
  const DD = ({ open, setOpen, selected, opts, onSelect, label }) => (
    <div className="db-flt" style={{ position: 'relative' }}>
  ...
  const Pager = ({ page, total, perP = 10, onChange }) => {
    ...
    const getPages = () => {
      ...
      const pages = [];
      const delta = 2; // siblings around current
    ...
    return (
      <div className="db-pag">
        {/* First */}
        <button
          className="db-pag-btn db-pag-ico"
          disabled={page === 1}
          onClick={() => onChange(1)}
          title="First page"
        >
        ...
        <div className="db-pag-nums">
          {pages.map((p, i) =>
            p === '...'
              ...
              : <button
                  key={p}
                  className={`db-pag-num-btn ${p === page ? 'active' : ''}`}
                  onClick={() => onChange(p)}
                >
                  {p}
                </button>
  ...
  return (
    <div className="db-root">
      ...
      {initLoading && <div className="db-overlay"><div className="db-spinner"></div></div>}
          ...
          <div className="db-grid-2">
            ...
            <div className="db-card">
              <div className="db-card-head" style={{ flexWrap: 'wrap', gap: 8 }}>
                <span className="db-card-title">Tickets by analyst</span>
                {/* Segmented timeframe control */}
                <div style={{ display: 'flex', background: '#f0f2f7', borderRadius: 8, padding: 3, gap: 2, border: '0.5px solid #e4e7ef' }}>
                  {[
                    { id: 'overall', label: 'Overall',   icon: 'bi-infinity' },
                    { id: 'monthly', label: 'Monthly',   icon: 'bi-calendar-month' },
                    { id: 'weekly',  label: 'This Week', icon: 'bi-calendar-week' },
              ...
              <div style={{ padding: '10px 14px 14px', position: 'relative', flex: 1 }}>
                {chartLoading && (
                  <div style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10, borderRadius: 8 }}>
                    <div className="db-spinner"></div>
                  </div>
                )}
                {chartAgents.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '40px 0', fontSize: 12, color: '#9aa0b4' }}>
                    No data for this period
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height={260}>
                    <BarChart
                      ...
                      margin={{ top: 16, right: 10, left: -18, bottom: 28 }}
                      barCategoryGap="30%"
                    >
                      <defs>
                        ...
                        <linearGradient id="vbarGradTop" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <XAxis
                        ...
                        axisLine={false}
                        tickLine={false}
                        tick={{ fontSize: 10, fontWeight: 600, fill: '#7b8099', fontFamily: 'DM Sans' }}
                        interval={0}
                        angle={-22}
                        textAnchor="end"
                        tickFormatter={name => fmt(name.split(' ')[0])}
                      />
                      <YAxis
                        axisLine={false}
                        tickLine={false}
                        tick={{ fontSize: 10, fontWeight: 600, fill: '#9aa0b4', fontFamily: 'DM Sans' }}
                        allowDecimals={false}
                      />
                      <Tooltip
                        cursor={{ fill: 'rgba(99,102,241,0.06)', radius: [6, 6, 0, 0] }}
                        content={({ active, payload, label }) => {
                          if (!active || !payload?.length) return null;
                          const a = payload[0].payload;
                          return (
                            <div style={{ background: '#fff', border: '0.5px solid #e4e7ef', borderRadius: 10, padding: '10px 14px', fontSize: 11, boxShadow: '0 8px 24px rgba(0,0,0,0.10)', minWidth: 140 }}>
                              <p style={{ margin: '0 0 6px', fontWeight: 700, color: '#1a1d2e', fontSize: 12 }}>{fmt(label)}</p>
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                                <span style={{ color: '#6366f1', fontWeight: 600 }}>🎫 {a.total_tickets} tickets</span>
                                <span style={{ color: '#166534' }}>✓ {a.passed || 0} passed</span>
                                <span style={{ color: '#991b1b' }}>✕ {a.failed || 0} failed</span>
                              </div>
                            </div>
                          );
                        }}
                      ...
                      >
                        {chartAgents.map((a, i) => (
                          <Cell
                            key={i}
                            fill={a.is_top_performer ? 'url(#vbarGradTop)' : 'url(#vbarGrad)'}
                            cursor="pointer"
