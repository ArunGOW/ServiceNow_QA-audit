...
const getDateRange = (timeframe) => {
  ...
  if (timeframe === 'weekly') {
    ...
    return { from_date: toDateStr(s), to_date: todayStr };
  }
  if (timeframe === 'monthly') {
    return { from_date: toDateStr(new Date(today.getFullYear(), today.getMonth(), 1)), to_date: todayStr };
  }
  return { from_date: undefined, to_date: undefined };
};
...
const StatCard = ({
  ...
  const pct = barPct !== null ? Math.min(Math.max(barPct, 0), 100) : null;
  return (
    <div
      className={`sbc sbc-${accent}${clickable ? ' sbc-clickable' : ''}${active ? ' sbc-active' : ''}`}
      onClick={clickable ? onClick : undefined}
      role={clickable ? 'button' : undefined}
    >
      <div className={`sbc-icon sbc-ic-${accent}`}>
        <i className={`bi ${icon}`}></i>
      </div>
      <div className="sbc-lbl">{label}</div>
      <div className={`sbc-val sbc-val-${accent}`}>{value}</div>

      {chips.length > 0 && (
        <div className="sbc-chips">
          {chips.map((c, i) => (
            <React.Fragment key={i}>
              <span className={`sbc-chip sbc-chip-${c.color || accent}`}>{c.text}</span>
              {i < chips.length - 1 && <span className="sbc-sep">·</span>}
            </React.Fragment>
          ))}
        </div>
      )}
      ...
      {pct !== null && (
        <div className="sbc-bar-track">
          <div className="sbc-bar-fill" style={{ width: `${pct}%`, background: barColor }} />
        </div>
      )}

      {(footLabel || footValue) && (
        <div className="sbc-foot">
          <span className="sbc-foot-lbl">{footLabel}</span>
          <span className="sbc-foot-val" style={footColor ? { color: footColor } : {}}>{footValue}</span>
        </div>
      )}
    </div>
  );
...
const StatusPill = ({ status }) => {
  ...
  if (s === 'escalated')   cls = 'sp-escalated';
  if (s === 'in progress') cls = 'sp-progress';
  return <span className={`sp ${cls}`}><span className="sp-dot"></span>{status || '—'}</span>;
};

/* ── Phase pill ── */
const PhasePill = ({ phase }) => (
  <span className={`pp ${phase === 'done' ? 'pp-done' : 'pp-pending'}`}>
    {phase?.toUpperCase()}
  </span>
);
...
const AllusersDashboard = () => {
  ...
  const totalTickets  = viewMode === 'list' ? (summary.total_tickets || 0)     : (selAgent?.total_tickets || 0);
  const incCount      = viewMode === 'list' ? (summary.ticket_split?.INC || 0) : (selAgent?.ticket_split?.INC || 0);
  const ritCount      = viewMode === 'list' ? (summary.ticket_split?.RIT || 0) : (selAgent?.ticket_split?.RIT || 0);
  const incPct        = totalTickets > 0 ? Math.round((incCount / totalTickets) * 100) : 0;
  const unassignedQA  = summary.unassigned_qa_tickets || 0;
  const unassignedPct = totalTickets > 0 ? Math.round((unassignedQA / totalTickets) * 100) : 0;
  const qcRemaining   = summary.qc_remaining || 0;
  const pendingCount  = summary.pending || 0;
  const pendingPct    = totalTickets > 0 ? Math.round((pendingCount / totalTickets) * 100) : 0;
  const passedCount   = viewMode === 'list' ? (summary.passed || 0)            : (selAgent?.passed || 0);
  const failedCount   = viewMode === 'list' ? (summary.failed || 0)            : (selAgent?.failed || 0);
  const qcDone        = summary.qc_done || 0;
  const coveragePct   = totalTickets > 0 ? Math.round((qcDone / totalTickets) * 100) : 0;
  const passPct       = viewMode === 'list' ? (summary.pass_percentage || 0)   : (selAgent?.pass_percentage || 0);
  const slaBreached   = summary.sla_breached || 0;
  const slaPct        = totalTickets > 0 ? Math.round((slaBreached / totalTickets) * 100) : 0;
  const reviewedPct   = qcDone > 0 ? Math.round((passedCount / qcDone) * 100) : 0;
  const failedPct     = qcDone > 0 ? Math.round((failedCount / qcDone) * 100) : 0;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await api.get('/users/admin/dashboard', { headers: { Authorization: `Bearer ${tok()}` } });
        setData(r.data); setChartData(r.data);
      } catch (e) { console.error(e); }
      finally { setTableLoading(false); }
    })();
  }, [activeTab, viewMode, user]);
  ...
  return (
    <div className="db-root">
        ...
        <div className="db-header-top">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div className="db-header-meta">
              ...
              <div className="db-meta-badge">
                <i className="bi bi-ticket-perforated" style={{ fontSize: 10 }}></i>
                {fmtNum(summary.total_tickets)} tickets
              </div>
              <div className="db-meta-badge">
                <i className="bi bi-graph-up" style={{ fontSize: 10 }}></i>
                {summary.pass_percentage || 0}% quality
      ...
      {/* ══════════════════════════════════════════════════════
          PREMIUM TWO-ROW STATS BANNER
      ══════════════════════════════════════════════════════ */}
      {activeTab === 'team' && (
        <div className="db-banner-wrap">

          {/* ─── Section label: Ticket Volume ─── */}
          <div className="db-banner-section-label">
            <i className="bi bi-stack"></i>
            Ticket Volume
          </div>

          {/* ─── ROW 1: 3 volume cards ─── */}
          <div className="db-banner-row db-banner-row-3">

            {/* Card 1 — Total Tickets + INC / RIT split */}
            <StatCard
              accent="violet"
              ...
              chips={[
                { text: `INC  ${fmtNum(incCount)}`,  color: 'violet' },
                { text: `RIT  ${fmtNum(ritCount)}`,  color: 'blue'   },
              ]}
              barPct={incPct}
              barColor="linear-gradient(90deg,#6366f1,#3b82f6)"
              footLabel="INC ratio"
              footValue={`${incPct}%`}
              footColor="#6366f1"
              clickable
              active={!showSla && viewMode === 'list'}
              onClick={() => { setShowSla(false); setViewMode('list'); }}
            />

            {/* Card 2 — Unassigned QA */}
            <StatCard
              accent="amber"
              icon="bi-inbox-fill"
              label="Unassigned QA"
              value={fmtNum(unassignedQA)}
              chips={[{ text: 'Awaiting assignment', color: 'amber' }]}
              barPct={unassignedPct}
              barColor="#f59e0b"
              footLabel="of total tickets"
              footValue={`${unassignedPct}%`}
              footColor="#d97706"
            />

            {/* Card 3 — Pending Assigned / QC Remaining */}
            <StatCard
              accent="slate"
              ...
              chips={[
                { text: `QC remaining  ${fmtNum(qcRemaining)}`, color: 'slate' },
              ]}
              barPct={pendingPct}
              barColor="#64748b"
              footLabel="QC backlog"
              footValue={`${fmtNum(qcRemaining)} tickets`}
              footColor="#475569"
            />
          </div>

          {/* ─── Section label: QA Outcomes ─── */}
          <div className="db-banner-section-label" style={{ marginTop: 8 }}>
            <i className="bi bi-patch-check-fill"></i>
            QA Outcomes
          </div>

          {/* ─── ROW 2: 5 outcome cards ─── */}
          <div className="db-banner-row db-banner-row-5">
            ...
            <StatCard
              ...
              label="QA Passed"
              value={fmtNum(passedCount)}
              chips={[{ text: `of ${fmtNum(qcDone)} reviewed`, color: 'green' }]}
              barPct={reviewedPct}
              barColor="#10b981"
              footLabel="pass rate"
              footValue={`${reviewedPct}%`}
              footColor="#16a34a"
            />
            ...
            <StatCard
              ...
              chips={[{ text: failedCount === 0 ? 'All clear' : `${failedPct}% fail rate`, color: failedCount === 0 ? 'green' : 'red' }]}
              barPct={failedPct}
              barColor="#ef4444"
              footLabel="fail rate"
              footValue={`${failedPct}%`}
              footColor={failedCount === 0 ? '#16a34a' : '#dc2626'}
            />

            {/* Card 6 — QA Completed + coverage */}
            <StatCard
              accent="teal"
              icon="bi-check2-all"
              label="QA Completed"
              value={fmtNum(qcDone)}
              chips={[{ text: `${coveragePct}% of tickets`, color: 'teal' }]}
              barPct={coveragePct}
              barColor="#14b8a6"
              footLabel={`of ${fmtNum(totalTickets)} total`}
              footValue={`${coveragePct}%`}
              footColor="#0d9488"
            />

            {/* Card 7 — SLA Breached (clickable → drill-in) */}
            <StatCard
              accent="rose"
              icon="bi-exclamation-octagon-fill"
              label="SLA Breached"
              value={fmtNum(slaBreached)}
              chips={[{ text: slaBreached === 0 ? 'All within SLA' : 'Needs attention', color: slaBreached === 0 ? 'green' : 'rose' }]}
              barPct={slaPct}
              barColor="#f43f5e"
              footLabel="of total tickets"
              footValue={`${slaPct}%`}
              footColor={slaBreached === 0 ? '#16a34a' : '#e11d48'}
              clickable
              active={showSla && viewMode === 'list'}
              onClick={() => { setShowSla(true); setViewMode('list'); }}
            />

            {/* Card 8 — Total QA Score */}
            <StatCard
              accent="purple"
              icon="bi-stars"
              label="Total QA Score"
              value={`${passPct}%`}
              chips={[{ text: 'Pass percentage', color: 'purple' }]}
              barPct={passPct}
              barColor="linear-gradient(90deg,#8b5cf6,#6366f1)"
              footLabel="performance"
              footValue={passPct >= 80 ? 'Excellent' : passPct >= 50 ? 'Average' : 'Needs work'}
              footColor={passPct >= 80 ? '#16a34a' : passPct >= 50 ? '#d97706' : '#dc2626'}
            />
          </div>
        </div>
      )}
        ...
        <div className="db-full">
          <div className="db-grid-2">
            ...
            <div className="db-card db-chart-card">
              ...
              <div style={{ padding: '4px 14px 14px', position: 'relative', flex: 1, minHeight: 260 }}>
                {chartLoading && <div className="db-chart-loader"><div className="db-spinner"></div></div>}
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart
                    data={[...chartAgents].sort((a, b) => (b.total_tickets || 0) - (a.total_tickets || 0))}
                    margin={{ top: 22, right: 14, left: -14, bottom: 30 }}
            ...
            <div className="db-card" style={{ minHeight: 0 }}>
              <div className="db-card-head">
                <div>
                  ...
                  <div style={{ marginTop: 3 }}>
                    <span className="db-timeframe-badge">
                      <i className={`bi ${chartTimeframe === 'today' ? 'bi-calendar-day' : chartTimeframe === 'weekly' ? 'bi-calendar-week' : chartTimeframe === 'monthly' ? 'bi-calendar-month' : 'bi-infinity'}`} style={{ marginRight: 4 }}></i>
                      {timeframeLabel}
                    </span>
                  </div>
                </div>
              ...
              <div className="db-table-wrap">
                {!tableLoading && !chartLoading && dirAgents.length === 0 && (
                  <div className="db-empty" style={{ padding: '40px 20px' }}>
                    <i className="bi bi-calendar-x db-empty-icon"></i>
                    <p className="db-empty-title">No data for {timeframeLabel}</p>
                    <span className="db-empty-sub">No analyst activity recorded for this period.</span>
                  </div>
                )}
                {(tableLoading || chartLoading || dirAgents.length > 0) && (
                  <table className="db-table" style={{ minWidth: 600 }}>
                    ...
                    <tbody>
                      {(tableLoading || chartLoading)
                        ? <SkeletonRows cols={8} rows={6} />
                        : dirAgents.map((a, i) => (
                            <tr key={i} className={a.is_top_performer ? 'row-top' : a.is_bottom_performer ? 'row-low' : 'row-norm'} onClick={() => openDetail(a)}>
                              <td>
                                <div className="db-arow">
                                  <div className="db-av">{a.agent?.charAt(0)?.toUpperCase()}</div>
                                  <span className="db-av-name">{fmt(a.agent)}</span>
                              ...
                              <td className="center"><span style={{ color: '#6366f1', fontWeight: 700, fontFamily: 'DM Mono,monospace' }}>{a.total_tickets || 0}</span></td>
                              <td className="center"><span className="badge badge-ind">{a.ticket_split?.INC ?? 0}</span></td>
                              <td className="center"><span className="badge badge-amb">{a.ticket_split?.RIT ?? 0}</span></td>
                              <td className="center"><span className="badge badge-grn">{a.passed || 0}</span></td>
                              <td className="center"><span className="badge badge-red">{a.failed || 0}</span></td>
      ...
      {activeTab === 'team' && viewMode === 'detail' && (
        <div className="db-card" style={{ flex: 1 }}>
          <div className="db-detail-header">
            <button className="db-back" onClick={() => setViewMode('list')}><i className="bi bi-arrow-left"></i> Back</button>
            <h3 className="db-detail-title">{fmt(selAgent?.agent)} <span>Tickets</span></h3>
          </div>
          <div className="db-table-wrap">
            <table className="db-table">
            ...
            <table className="db-table">
              ...
              <tbody>
                {tableLoading
                  ? <SkeletonRows cols={4} rows={8} />
                  : pending.length === 0
                    ? <tr><td colSpan={4}><div className="db-empty"><i className="bi bi-inbox db-empty-icon"></i><p className="db-empty-title">No records found</p><span className="db-empty-sub">No incidents matching "{selQaPend?.label}" for the selected criteria.</span></div></td></tr>
                    : pending.map((inc, i) => (
                        <tr key={i}>
