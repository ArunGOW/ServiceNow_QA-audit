...
const AllusersDashboard = () => {
  ...
  const [tableLoading, setTableLoading]   = useState(false); // skeleton rows inside table
  const [pageLoading, setPageLoading]     = useState(false); // slim bar during pagination
  ...
  /* ── refs to detect page-vs-filter changes ── */
  ...
  return (
    <div className="db-root">
      ...
      <header className="db-header">
        ...
        {/* bottom row: filters (only when relevant) */}
        {(( activeTab === 'team' && viewMode === 'detail') || activeTab === 'pending') && (
          <div className="db-header-bottom">
            <div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.45)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
            ...
            {/* <div className="db-card">
              <div className="db-card-head" style={{ flexWrap: 'wrap', gap: 8 }}>
                ...
                <div style={{ display: 'flex', background: '#f0f2f7', borderRadius: 8, padding: 3, gap: 2, border: '0.5px solid #e4e7ef' }}>
                  {[{ id: 'overall', label: 'Overall', icon: 'bi-infinity' }, { id: 'monthly', label: 'Monthly', icon: 'bi-calendar-month' }, { id: 'weekly', label: 'This Week', icon: 'bi-calendar-week' }].map(opt => (
                    <button key={opt.id} onClick={() => setChartTimeframe(opt.id)} style={{
                      ...
                      <i className={`bi ${opt.icon}`} style={{ fontSize: 11 }}></i> {opt.label}
                    </button>
                  ))}
                </div>
              </div>
              <div style={{ padding: '10px 14px 14px', position: 'relative', flex: 1, minHeight: 240 }}>
                {chartLoading && (
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={[...chartAgents].sort((a, b) => (b.total_tickets || 0) - (a.total_tickets || 0))} margin={{ top: 16, right: 10, left: -18, bottom: 28 }} barCategoryGap="30%">
                    <defs>
                      <linearGradient id="vg" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#6366f1" stopOpacity={0.95} />
                        <stop offset="100%" stopColor="#818cf8" stopOpacity={0.5} />
                      </linearGradient>
                      <linearGradient id="vgt" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Tooltip cursor={{ fill: 'rgba(99,102,241,0.06)', radius: [6, 6, 0, 0] }} content={<BarTip />} />
                    <Bar dataKey="total_tickets" radius={[5, 5, 0, 0]} maxBarSize={36} fill="url(#vg)"
                      label={{ position: 'top', fontSize: 9, fontWeight: 700, fill: '#7b8099' }}>
                      {chartAgents.map((a, i) => (
                        <Cell key={i} fill={a.is_top_performer ? 'url(#vgt)' : 'url(#vg)'} cursor="pointer" onClick={() => openDetail(a)} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
  ...
  <div style={{ padding: '4px 14px 14px', position: 'relative', flex: 1, minHeight: 260 }}>
    ...
    <ResponsiveContainer width="100%" height={260}>
      ...
      >
        ...
        />
        <Tooltip
          cursor={{ fill: 'rgba(99,102,241,0.08)', radius: [8, 8, 0, 0] }}
          content={<BarTip />}
        />
        <Bar
          dataKey="total_tickets"
