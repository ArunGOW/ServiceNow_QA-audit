...
//       <div className="db-period-dot"></div>
//       Showing data for <strong style={{ marginLeft: 3 }}>{periodLabel}</strong>
//       &nbsp;·&nbsp; {summary.total_tickets || 0} tickets
//       &nbsp;·&nbsp; {summary.pass_percentage || 0}% quality
//     </div>
//   </div>
...
/* ── Global timeframe → date range ── */
const getGlobalDateRange = (tf) => {
  const today = new Date();
  const todayStr = toDateStr(today);
  if (tf === '1m') {
  ...
  return { from_date: undefined, to_date: undefined }; // all time
};
...
const AllusersDashboard = () => {
  ...
  /* ── Global timeframe for banner + directory ── */
  const [globalTimeframe, setGlobalTimeframe] = useState('1m');
  ...
  /* ── Derived summary — prefers filtered key, falls back to all_time ── */
  const summary  = (globalTimeframe !== 'all' ? data?.filtered?.summary  : null) || data?.all_time?.summary              || {};
  const qaAdmins = (globalTimeframe !== 'all' ? data?.filtered?.qa_admin_performance : null) || data?.all_time?.qa_admin_performance || [];
  ...
  const sortedByPct   = [...dirAgents].filter(a => a.pass_percentage !== null && a.pass_percentage !== undefined).sort((a, b) => (b.pass_percentage ?? 0) - (a.pass_percentage ?? 0));
  const topAgentName    = sortedByPct[0]?.agent;
  const bottomAgentName = sortedByPct[sortedByPct.length - 1]?.agent;
  const isTop    = (a) => dirAgents.length > 1 && a.agent === topAgentName;
  const isBottom = (a) => dirAgents.length > 1 && a.agent === bottomAgentName && a.agent !== topAgentName;
  ...
  /* ── Main dashboard fetch — reacts to globalTimeframe ── */
  useEffect(() => {
    if (activeTab !== 'team' || viewMode === 'detail') return;
    setTableLoading(true);
    setChartLoading(true);
    (async () => {
      try {
        const { from_date, to_date } = getGlobalDateRange(globalTimeframe);
        const r = await api.get('/users/admin/dashboard', {
          headers: { Authorization: `Bearer ${tok()}` },
          params: {
            ...(from_date ? { from_date } : {}),
      ...
      finally { setTableLoading(false); setChartLoading(false); }
    })();
  }, [activeTab, viewMode, user, globalTimeframe]); // globalTimeframe drives everything

  /* ── Chart-only fetch — reacts to chartTimeframe (secondary filter on top of global) ── */
  useEffect(() => {
    if (activeTab !== 'team' || viewMode === 'detail') return;
    setChartLoading(true);
    const globalRange = getGlobalDateRange(globalTimeframe);
    const chartRange  = getDateRange(chartTimeframe);
    const from_date   = chartTimeframe === 'overall' ? globalRange.from_date : chartRange.from_date;
    const to_date     = chartTimeframe === 'overall' ? globalRange.to_date   : chartRange.to_date;
    (async () => {
      try {
        const r = await api.get('/users/admin/dashboard', {
          headers: { Authorization: `Bearer ${tok()}` },
  ...
  }, [chartTimeframe, activeTab, viewMode, user, globalTimeframe]);
  ...
  return (
    <div className="db-root">
      ...
      <header className="db-header">
        <div className="db-header-top">
            ...
            <div className="db-tf-tabs">
              {[
                ...
                <button
                  key={opt.id}
                  className={`db-tf-range-btn ${globalTimeframe === opt.id ? 'active' : ''}`}
                  onClick={() => { setGlobalTimeframe(opt.id); setShowSla(false); }}
                >
                  {opt.label}
                </button>
              ))}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div className="db-live-pill">
              <div className="db-meta-dot"></div> Live
            </div>
            <div className="db-tabs">
              <button className={`db-tab ${activeTab === 'team' ? 'active' : ''}`}
                onClick={() => { setActiveTab('team'); setViewMode('list'); setShowSla(false); }}>
        ...
        {/* Filter row — only for detail / pending views */}
        {((activeTab === 'team' && viewMode === 'detail') || activeTab === 'pending') && (
          <div className="db-header-bottom">
            <div style={{ fontSize: 10, fontWeight: 600, color: '#7b8099', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
              {activeTab === 'pending' ? 'Pending Queue Filters' : `Analyst: ${fmt(selAgent?.agent)}`}
      ...
      {/* ══════════════════════════════════════════
          STATS BANNER — hidden in detail view
      ══════════════════════════════════════════ */}
            ...
            <div className="dbs-card v">
              <div className="dbs-card-top">
                <div>
                  ...
                  <div className="dbs-val v">{(summary.total_tickets || 0).toLocaleString()}</div>
                </div>
                <div className="dbs-ico v"><i className="bi bi-ticket-perforated-fill" /></div>
              </div>
              <div className="dbs-chips">
            ...
            <div className="dbs-card s">
              <div className="dbs-card-top">
                <div>
                  ...
                  <div className="dbs-val s">{summary.unassigned_qa_tickets || 0}</div>
                </div>
                <div className="dbs-ico s"><i className="bi bi-inbox-fill" /></div>
              </div>
              <div className="dbs-chips">
            ...
            <div className="dbs-card a">
              <div className="dbs-card-top">
                <div>
                  ...
                  <div className="dbs-val a">{summary.pending || 0}</div>
                </div>
                <div className="dbs-ico a"><i className="bi bi-hourglass-split" /></div>
              </div>
              <div className="dbs-chips">
            ...
            <div className={`dbs-card or click ${showSla ? 'dbs-sla-active' : ''}`} onClick={() => setShowSla(true)}>
              <div className="dbs-card-top">
                <div>
                  ...
                  <div className="dbs-val or">{summary.sla_breached ?? 0}</div>
                </div>
                <div className="dbs-ico or"><i className="bi bi-clock-history" /></div>
              </div>
              <div className="dbs-chips">
              ...
              <div className="dbs-bar-track">
                <div className="dbs-bar-fill" style={{ width: `${Math.max(totalTix ? Math.round(((summary.sla_breached ?? 0) / totalTix) * 100) : 0, 1)}%`, background: '#f97316' }} />
              </div>
              <div className="dbs-divider" />
            ...
            <div className="dbs-card g">
              <div className="dbs-card-top">
                <div>
                  ...
                  <div className="dbs-val g">{summary.passed || 0}</div>
                </div>
                <div className="dbs-ico g"><i className="bi bi-shield-check" /></div>
              </div>
              <div className="dbs-chips">
            ...
            <div className={`dbs-card r click ${showSla ? '' : 'dbs-red-active'}`} onClick={() => setShowSla(false)}>
              <div className="dbs-card-top">
                <div>
                  ...
                  <div className="dbs-val r">{summary.failed || 0}</div>
                </div>
                <div className="dbs-ico r"><i className="bi bi-exclamation-octagon-fill" /></div>
              </div>
              <div className="dbs-chips">
            ...
            <div className="dbs-card t">
              <div className="dbs-card-top">
                <div>
                  ...
                  <div className="dbs-val t">{summary.qc_done || 0}</div>
                </div>
                <div className="dbs-ico t"><i className="bi bi-check2-all" /></div>
              </div>
              <div className="dbs-chips">
            ...
            <div className="dbs-card p">
              <div className="dbs-card-top">
                <div>
                  ...
                  <div className="dbs-val p">{summary.pass_percentage || 0}%</div>
                </div>
                <div className="dbs-ico p"><i className="bi bi-graph-up-arrow" /></div>
              </div>
              <div className="dbs-chips">
                            ...
                            <tr key={i} onClick={() => openDetail(a)}>
                              <td>
                                <div className="db-arow">
                                  <div className="db-av">{a.agent?.charAt(0)?.toUpperCase()}</div>
                                  <span className="db-av-name">{fmt(a.agent)}</span>
                                  {isTop(a)    && <span className="badge badge-top">Top</span>}
                                  {isBottom(a) && <span className="badge badge-low">Low</span>}
                                </div>
                              </td>
                              <td className="center"><span style={{ color: '#6366f1', fontWeight: 700 }}>{a.total_tickets || 0}</span></td>
                              <td className="center"><span className="badge badge-ind">{a.ticket_split?.INC ?? 0}</span></td>
      ...
      {/* ══════════════════════════════════════════
          ANALYST DETAIL VIEW — premium gradient header
      ══════════════════════════════════════════ */}
      {activeTab === 'team' && viewMode === 'detail' && (
        <div className="db-card db-detail-card" style={{ flex: 1 }}>

          {/* Gradient header */}
          <div className="db-detail-head">
            <button className="db-detail-back" onClick={() => setViewMode('list')}>
              <i className="bi bi-arrow-left"></i> Back
            </button>
            <div className="db-detail-center">
              <div className="db-detail-av">{selAgent?.agent?.charAt(0)?.toUpperCase()}</div>
              <div>
                <div className="db-detail-name">
                  {fmt(selAgent?.agent)} <span>Tickets</span>
                </div>
                <div className="db-detail-sub">Analyst · {detQa === 'pending' ? 'Pending QA' : 'Done QA'} View</div>
              </div>
            </div>
            <div className="db-detail-head-right">
              <div className="db-detail-hbadge">📋 {Math.ceil(detTotal / 10) || 1} Pages</div>
              <div className="db-detail-hcount">{detTotal} Records</div>
            </div>
          </div>
              ...
              <tbody>
                {tableLoading
                  ...
                  : agentTickets.length === 0
                    ? <tr><td colSpan={5}><div className="db-empty"><i className="bi bi-inbox db-empty-icon"></i><p className="db-empty-title">No tickets found</p><span className="db-empty-sub">Try adjusting the filters above.</span></div></td></tr>
                    : agentTickets.map((t, i) => (
                        <tr key={i} onClick={() => openModal(t)}>
                          <td className="center">
                            <span className="db-row-num">{(detPage - 1) * 10 + i + 1}</span>
                          </td>
                          <td>
                            <span className="db-inc-no">
                              <span className="db-inc-hash">#</span>
                              {t.incident_number}
                            </span>
                          </td>
                          <td className="desc-cell">{t.short_description}</td>
                          <td className="center"><StatusPill status={t.status} /></td>
                          <td className="center"><PhasePill phase={detQa} /></td>
                        </tr>
                      ))
                }
              </tbody>
