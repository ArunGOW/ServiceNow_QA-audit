...
/* ─── Date helpers ──────────────────────────────────
   Maps the header "Data Range" buttons → from/to dates
   that go to the backend.
...
const getRangeForGlobal = (range) => {
  const today = new Date();
  const todayStr = toDateStr(today);
  if (range === '1m') {
    const s = new Date(today); s.setMonth(s.getMonth() - 1);
    return { from_date: toDateStr(s), to_date: todayStr, timeframe: 'monthly' };
  }
  if (range === '3m') {
  ...
  // 'all'
  return { from_date: undefined, to_date: undefined, timeframe: 'overall' };
};

/* For the chart-only timeframe toggle (inside the chart card) */
const getChartDateRange = (tf) => {
  const today = new Date();
  const todayStr = toDateStr(today);
  if (tf === 'today')   return { from_date: todayStr, to_date: todayStr };
  if (tf === 'weekly')  { const s = new Date(today); s.setDate(today.getDate() - today.getDay()); return { from_date: toDateStr(s), to_date: todayStr }; }
  if (tf === 'monthly') return { from_date: toDateStr(new Date(today.getFullYear(), today.getMonth(), 1)), to_date: todayStr };
  return { from_date: undefined, to_date: undefined };
};
...
const StatusPill = ({ status }) => {
  ...
  if (s === 'escalated')   cls = 'sp-escalated';
  if (s === 'in progress') cls = 'sp-progress';
  return <span className={`sp ${cls}`}><span className="sp-dot" />{status || '—'}</span>;
};

/* ── Phase pill ── */
const PhasePill = ({ phase }) => (
  <span className={`pp ${phase === 'done' ? 'pp-done' : 'pp-pending'}`}>{phase?.toUpperCase()}</span>
);
...
const LoadBar = ({ visible }) => !visible ? null : (
  <div className="db-load-bar"><div className="db-load-bar-fill" /></div>
);
...
const AllusersDashboard = () => {
  ...
  /* ══════════════════════════════════════════════
     globalTimeframe — header "Data Range" buttons
     Controls:  banner stats  +  chart  +  analyst table
  ══════════════════════════════════════════════ */
  const [globalTimeframe, setGlobalTimeframe] = useState('1m');

  /* bannerData = filtered by globalTimeframe */
  const [bannerData, setBannerData] = useState(null);

  /* allTimeData = always all-time, for fallback labels */
  const [allTimeData, setAllTimeData] = useState(null);

  /* chart-card inner toggle (within the global range) */
  const [chartTimeframe, setChartTimeframe] = useState('overall');
  const [chartLoading, setChartLoading]     = useState(false);
  ...
  /* ══════════════════════════════════════════════
     DERIVED — what the banner & analyst table show
     Always comes from bannerData (filtered).
     Falls back to allTimeData if bannerData is null.
  ══════════════════════════════════════════════ */
  const activeData  = bannerData || allTimeData;
  const summary     = activeData?.filtered?.summary
                   || activeData?.all_time?.summary
                   || {};
  const qaAdmins    = allTimeData?.all_time?.qa_admin_performance || [];

  /* chart agents — use filtered from bannerData */
  const chartAgents = (() => {
    if (!bannerData) return allTimeData?.all_time?.agent_performance || [];
    return bannerData?.filtered?.agent_performance
        || bannerData?.all_time?.agent_performance
        || [];
  })();
  const dirAgents = chartAgents;

  /* ── banner computed values ── */
  const totalTix      = summary.total_tickets      || 0;
  const incCount      = summary.ticket_split?.INC  ?? 0;
  const ritCount      = summary.ticket_split?.RIT  ?? 0;
  const incRatio      = totalTix ? Math.round((incCount / totalTix) * 100) : 0;
  const unassigned    = summary.unassigned_qa_tickets || 0;
  const unassignedPct = totalTix ? Math.min(Math.round((unassigned / totalTix) * 100), 100) : 0;
  const pendingCount  = summary.pending            || 0;
  const pendingPct    = totalTix ? Math.min(Math.round((pendingCount / totalTix) * 100), 100) : 0;
  const qcRemaining   = summary.qc_remaining       ?? 0;
  const qcDone        = summary.qc_done            || 0;
  const passedCount   = summary.passed             || 0;
  const failedCount   = summary.failed             || 0;
  const coveragePct   = totalTix ? +((qcDone / totalTix) * 100).toFixed(1) : 0;
  const failRate      = qcDone ? Math.round((failedCount / qcDone) * 100) : 0;
  const passPct       = summary.pass_percentage    || 0;
  const slaBreached   = summary.sla_breached       ?? 0;
  const slaPct        = totalTix ? Math.round((slaBreached / totalTix) * 100) : 0;

  const periodLabel = { '1m': 'Last 1 Month', '3m': 'Last 3 Months', '6m': 'Last 6 Months', 'all': 'All Time' }[globalTimeframe];
  const chartTfLabel = { overall: 'All Time', today: 'Today', weekly: 'This Week', monthly: 'This Month' }[chartTimeframe];

  /* ═══════════════════════════════════════════════════
     FETCH 1 — on mount: load all-time base data
  ═══════════════════════════════════════════════════ */
  useEffect(() => {
    if (activeTab !== 'team' || viewMode === 'detail') return;
    (async () => {
      try {
        const r = await api.get('/users/admin/dashboard', { headers: { Authorization: `Bearer ${tok()}` } });
        setAllTimeData(r.data);
      } catch (e) { console.error(e); }
    })();
  }, [activeTab, viewMode, user]);

  /* ═══════════════════════════════════════════════════
     FETCH 2 — whenever globalTimeframe changes
     This updates BOTH the banner stats AND the analyst
     table (dirAgents comes from this response).
  ═══════════════════════════════════════════════════ */
  useEffect(() => {
    if (activeTab !== 'team' || viewMode === 'detail') return;
    setBannerLoading(true);

    const { from_date, to_date, timeframe } = getRangeForGlobal(globalTimeframe);
  ...
  }, [globalTimeframe, activeTab, viewMode, user]);

  /* ═══════════════════════════════════════════════════
     FETCH 3 — chart inner timeframe toggle
     Only re-fetches when the chart toggle changes AND
     it differs from the global range.
  ═══════════════════════════════════════════════════ */
  useEffect(() => {
    if (activeTab !== 'team' || viewMode === 'detail') return;
    if (chartTimeframe === 'overall') return; // already have data from fetch 2
    setChartLoading(true);
    const { from_date, to_date } = getChartDateRange(chartTimeframe);
    (async () => {
      try {
        const r = await api.get('/users/admin/dashboard', {
          headers: { Authorization: `Bearer ${tok()}` },
        ...
        /* only update chart agents, not the banner */
        setBannerData(prev => prev
          ? { ...prev, _chartOverride: r.data?.filtered?.agent_performance || r.data?.all_time?.agent_performance }
          : prev
        );
      } catch (e) { console.error(e); }
      finally { setChartLoading(false); }
  ...
  /* chart agents with inner toggle override */
  const finalChartAgents = (() => {
    if (bannerData?._chartOverride && chartTimeframe !== 'overall') return bannerData._chartOverride;
    return chartAgents;
  })();
  ...
  const fetchSla = async (page, isPaging = false) => {
    isPaging ? setPageLoading(true) : setTableLoading(true);
    try {
      const r = await api.post('/users/get/sla-breached/incidents',
        { user_sid: sid(), page, per_page: 10 },
        { headers: { Authorization: `Bearer ${tok()}` } }
      );
      setSlaData(r.data.response || []); setSlaTotal(r.data.total || 0);
  ...
  const openDetail = (a) => { setSelAgent(a); setViewMode('detail'); prevDetPage.current = 1; setDetPage(1); fetchDetail(a, 1, false); };
  const openModal  = (t) => { setSelInc({ ...t, display_name: selAgent?.agent || t.done_by || 'System' }); setModal(true); };
  ...
  const DD = ({ open, setOpen, selected, opts, onSelect, label }) => (
    <div className="db-flt" style={{ position: 'relative' }}>
      ...
      <div className="db-fbtn" onClick={() => setOpen(o => !o)}>
        <div className="db-dot" style={{ background: selected.color }} />
        <span>{selected.label}</span>
        <i className="bi bi-chevron-down db-chevron" />
      </div>
      {open && (
        <div className="db-dropdown" style={{ zIndex: 9999 }}>
          {opts.map(o => (
            <div key={o.id} className="db-menu-item" onClick={() => { onSelect(o.id); setOpen(false); }}>
              <i className={`bi ${o.icon}`} style={{ color: o.color, fontSize: 13 }} /> {o.label}
            </div>
          ))}
        </div>
      )}
  ...
  const Pager = ({ page, total, perP = 10, onChange }) => {
    ...
    return (
      <div className="db-pag">
        <button className="db-pag-btn db-pag-ico" disabled={page === 1} onClick={() => onChange(1)}><i className="bi bi-chevron-double-left" /></button>
        <button className="db-pag-btn db-pag-ico" disabled={page === 1} onClick={() => onChange(page - 1)}><i className="bi bi-chevron-left" /></button>
        <div className="db-pag-nums">
          {getPages().map((p, i) =>
            p === '…' ? <span key={`e${i}`} className="db-pag-ellipsis">…</span>
              : <button key={p} className={`db-pag-num-btn ${p === page ? 'active' : ''}`} onClick={() => onChange(p)}>{p}</button>
          )}
        </div>
        <button className="db-pag-btn db-pag-ico" disabled={page >= totalPages} onClick={() => onChange(page + 1)}><i className="bi bi-chevron-right" /></button>
        <button className="db-pag-btn db-pag-ico" disabled={page >= totalPages} onClick={() => onChange(totalPages)}><i className="bi bi-chevron-double-right" /></button>
        <div className="db-pag-jump">
          <span className="db-pag-jump-lbl">Go to</span>
          <input type="number" min={1} max={totalPages} className="db-pag-jump-input" placeholder="—"
            onKeyDown={e => { if (e.key === 'Enter') { const v = parseInt(e.target.value); if (v >= 1 && v <= totalPages) { onChange(v); e.target.value = ''; } } }} />
          <span className="db-pag-jump-lbl">/ {totalPages}</span>
        </div>
      </div>
    );
    ...
    <div className="db-root">
      ...
      <header className="db-header">
        <div className="db-header-top">
          ...
          {/* Brand */}
          <div className="db-brand">
            <div className="db-brand-icon"><i className="bi bi-layers-fill" /></div>
            <div className="db-brand-text">
              <div className="db-brand-name">Audit<span>Pulse</span></div>
              <div className="db-brand-tagline">Quality Intelligence Platform</div>
            </div>
          ...
          {/* ── DATA RANGE — centred ──
               These 4 buttons drive the ENTIRE dashboard:
               banner stats, analyst table, bar chart.
               On click → getRangeForGlobal() computes from/to
               and the useEffect fires the API call.             */}
          <div className="db-tf-center">
            <div className="db-tf-range-label">Data Range</div>
            <div className="db-tf-tabs">
              {[
                ...
                <button key={opt.id}
                  className={`db-tf-range-btn ${globalTimeframe === opt.id ? 'active' : ''}`}
                  onClick={() => {
                    setGlobalTimeframe(opt.id);
                    setChartTimeframe('overall'); // reset inner toggle
                  }}>
                  {opt.label}
                </button>
              ))}
          ...
          {/* Right */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div className="db-live-pill"><div className="db-meta-dot" /> Live</div>
            <div className="db-tabs">
              <button className={`db-tab ${activeTab === 'team' ? 'active' : ''}`}
                onClick={() => { setActiveTab('team'); setViewMode('list'); setShowSla(false); }}>
                <i className="bi bi-people-fill" style={{ marginRight: 5 }} />Team View
              </button>
              <button className={`db-tab ${activeTab === 'pending' ? 'active' : ''}`}
                onClick={() => setActiveTab('pending')}>
                <i className="bi bi-hourglass-split" style={{ marginRight: 5 }} />My Pending
              </button>
            </div>
          </div>
        </div>

        {/* Period pill — shows which range is active */}
        {activeTab === 'team' && (
          <div className="db-header-period">
            <div className="db-period-pill">
              <div className="db-period-dot" />
              Showing data for: <strong style={{ marginLeft: 4 }}>{periodLabel}</strong>
              {bannerLoading && <i className="bi bi-arrow-repeat db-spin-sm" style={{ marginLeft: 6 }} />}
            </div>
          </div>
        )}

        {/* Filters for detail / pending views */}
        {((activeTab === 'team' && viewMode === 'detail') || activeTab === 'pending') && (
          <div className="db-header-bottom">
            <div style={{ fontSize: 10, fontWeight: 600, color: '#7b8099', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
              {activeTab === 'pending' ? 'Pending Queue Filters' : `Analyst: ${fmt(selAgent?.agent)}`}
            ...
            <div className="db-filters">
              {activeTab === 'team' && viewMode === 'detail' && (
                <>
                  ...
                  <div className="db-flt"><label className="db-flbl">From</label><input type="date" className="db-date" value={detFrom} onChange={e => setDetFrom(e.target.value)} /></div>
                  <div className="db-flt"><label className="db-flbl">To</label><input type="date" className="db-date" value={detTo} onChange={e => setDetTo(e.target.value)} /></div>
                  <div className="db-flt"><label className="db-flbl">Reset</label><button className="db-reset" onClick={() => { setDetFrom(''); setDetTo(''); setDetPage(1); }}><i className="bi bi-arrow-counterclockwise" /></button></div>
                </>
              )}
              {activeTab === 'pending' && (
                <>
                  ...
                  <div className="db-flt"><label className="db-flbl">From</label><input type="date" className="db-date" value={pendFrom} onChange={e => setPendFrom(e.target.value)} /></div>
                  <div className="db-flt"><label className="db-flbl">To</label><input type="date" className="db-date" value={pendTo} onChange={e => setPendTo(e.target.value)} /></div>
                  <div className="db-flt"><label className="db-flbl">Reset</label><button className="db-reset" onClick={() => { setPendFrom(''); setPendTo(''); setPendPage(1); }}><i className="bi bi-arrow-counterclockwise" /></button></div>
                </>
              )}
            </div>
          </div>
      ...
      {/* ══════════ STATS BANNER ══════════
          Every value here comes from `summary` which is
          derived from bannerData (filtered by globalTimeframe).
          Clicking a Data Range button → new API fetch →
          setBannerData → summary re-derives → cards update.
      ══════════════════════════════════════════════════ */}
      {activeTab === 'team' && viewMode !== 'detail' && (
        <div className={`dbs-banner${bannerLoading ? ' dbs-loading' : ''}`}>

          {/* ── ROW LABEL ── */}
          <div className="dbs-row-label">
            <i className="bi bi-stack" style={{ marginRight: 5 }} />Ticket Volume
            <span className="dbs-range-tag">{periodLabel}</span>
          </div>

          {/* ── ROW 1 : 4 volume cards ── */}
          <div className="dbs-row dbs-row-4">
            ...
            <div className="dbs-card v">
              <div className="dbs-card-top">
                <div>
                  <div className="dbs-lbl">Total Tickets</div>
                  <div className="dbs-val v">{fmtNum(totalTix)}</div>
                </div>
                <div className="dbs-ico v"><i className="bi bi-ticket-perforated-fill" /></div>
              </div>
              <div className="dbs-chips">
                <span className="dbs-chip inc">INC {fmtNum(incCount)}</span>
                <span className="dbs-chip rit">RIT {fmtNum(ritCount)}</span>
              </div>
              <div className="dbs-bar-track"><div className="dbs-bar-fill" style={{ width: `${incRatio}%`, background: '#6366f1' }} /></div>
              <div className="dbs-divider" />
              <div className="dbs-foot">
                <span className="dbs-foot-lbl">INC ratio</span>
                <span className="dbs-foot-val v">{incRatio}%</span>
              </div>
            </div>
            ...
            <div className="dbs-card s">
              <div className="dbs-card-top">
                <div>
                  <div className="dbs-lbl">Unassigned QA</div>
                  <div className="dbs-val s">{fmtNum(unassigned)}</div>
                </div>
                <div className="dbs-ico s"><i className="bi bi-inbox-fill" /></div>
              </div>
              <div className="dbs-chips">
              ...
              <div className="dbs-divider" />
              <div className="dbs-foot">
                <span className="dbs-foot-lbl">of total tickets</span>
                <span className="dbs-foot-val s">{unassignedPct}%</span>
              </div>
            </div>

            {/* 3 — Pending Assigned / QC Remaining */}
            <div className="dbs-card a">
              <div className="dbs-card-top">
                <div>
                  <div className="dbs-lbl">Pending Assigned</div>
                  <div className="dbs-val a">{fmtNum(pendingCount)}</div>
                </div>
                <div className="dbs-ico a"><i className="bi bi-hourglass-split" /></div>
              </div>
              <div className="dbs-chips">
                <span className="dbs-chip amb">QC remaining {fmtNum(qcRemaining)}</span>
              </div>
              <div className="dbs-bar-track"><div className="dbs-bar-fill" style={{ width: `${pendingPct}%`, background: '#f59e0b' }} /></div>
              <div className="dbs-divider" />
              <div className="dbs-foot">
                <span className="dbs-foot-lbl">QC backlog</span>
                <span className="dbs-foot-val a">{fmtNum(qcRemaining)} tickets</span>
              </div>
            </div>

            {/* 4 — SLA Breached (clickable) */}
            <div className={`dbs-card or click${showSla ? ' dbs-sla-active' : ''}`} onClick={() => setShowSla(true)}>
              <div className="dbs-card-top">
                <div>
                  <div className="dbs-lbl">SLA Breached</div>
                  <div className="dbs-val or">{fmtNum(slaBreached)}</div>
                </div>
                <div className="dbs-ico or"><i className="bi bi-clock-history" /></div>
              </div>
              <div className="dbs-chips">
                {slaBreached === 0
                  ? <span className="dbs-chip grn">All within SLA</span>
                  : <span className="dbs-chip or">{fmtNum(slaBreached)} breached</span>}
              </div>
              <div className="dbs-bar-track"><div className="dbs-bar-fill" style={{ width: `${Math.max(slaPct, 1)}%`, background: '#f97316' }} /></div>
              <div className="dbs-divider" />
              <div className="dbs-foot">
                <span className="dbs-foot-lbl">Breach rate</span>
                <span className={`dbs-foot-val ${slaBreached === 0 ? 'g' : 'or'}`}>{slaPct}%</span>
              </div>
            </div>
          </div>

          {/* ── ROW LABEL 2 ── */}
          <div className="dbs-row-label" style={{ marginTop: 6 }}>
            <i className="bi bi-patch-check-fill" style={{ marginRight: 5 }} />QA Outcomes
            <span className="dbs-range-tag">{periodLabel}</span>
          </div>

          {/* ── ROW 2 : 4 outcome cards ── */}
          <div className="dbs-row dbs-row-4">

            {/* 5 — QA Passed */}
            <div className={`dbs-card g click${!showSla ? ' dbs-red-active' : ''}`} onClick={() => setShowSla(false)}>
              <div className="dbs-card-top">
                <div>
                  <div className="dbs-lbl">QA Passed</div>
                  <div className="dbs-val g">{fmtNum(passedCount)}</div>
                </div>
                <div className="dbs-ico g"><i className="bi bi-shield-check" /></div>
              </div>
              <div className="dbs-chips">
                <span className="dbs-chip grn">of {fmtNum(qcDone)} reviewed</span>
              </div>
              <div className="dbs-bar-track"><div className="dbs-bar-fill" style={{ width: `${passPct}%`, background: '#10b981' }} /></div>
              <div className="dbs-divider" />
              <div className="dbs-foot">
                <span className="dbs-foot-lbl">Pass rate</span>
                <span className="dbs-foot-val g">{passPct}%</span>
              </div>
            </div>

            {/* 6 — QA Failed */}
            <div className="dbs-card r">
              <div className="dbs-card-top">
                <div>
                  <div className="dbs-lbl">QA Failed</div>
                  <div className="dbs-val r">{fmtNum(failedCount)}</div>
                </div>
                <div className="dbs-ico r"><i className="bi bi-exclamation-octagon-fill" /></div>
              </div>
              <div className="dbs-chips">
                {failedCount === 0
                  ? <span className="dbs-chip grn">All clear</span>
                  : <span className="dbs-chip red">{fmtNum(failedCount)} failed</span>}
              </div>
              <div className="dbs-bar-track"><div className="dbs-bar-fill" style={{ width: `${Math.max(failRate, 1)}%`, background: '#ef4444' }} /></div>
              <div className="dbs-divider" />
              <div className="dbs-foot">
                <span className="dbs-foot-lbl">Fail rate</span>
                <span className={`dbs-foot-val ${failedCount === 0 ? 'g' : 'r'}`}>{failRate}%</span>
              </div>
            </div>
            ...
            <div className="dbs-card t">
              <div className="dbs-card-top">
                <div>
                  <div className="dbs-lbl">QA Completed</div>
                  <div className="dbs-val t">{fmtNum(qcDone)}</div>
                </div>
                <div className="dbs-ico t"><i className="bi bi-check2-all" /></div>
              </div>
              <div className="dbs-chips">
              ...
              <div className="dbs-divider" />
              <div className="dbs-foot">
                <span className="dbs-foot-lbl">of {fmtNum(totalTix)} total</span>
                <span className="dbs-foot-val t">QC done</span>
              </div>
            </div>
            ...
            <div className="dbs-card p">
              <div className="dbs-card-top">
                <div>
                  <div className="dbs-lbl">Total QA Score</div>
                  <div className="dbs-val p">{passPct}%</div>
                </div>
                <div className="dbs-ico p"><i className="bi bi-graph-up-arrow" /></div>
              </div>
              <div className="dbs-chips">
              ...
              <div className="dbs-bar-track"><div className="dbs-bar-fill" style={{ width: `${passPct}%`, background: '#8b5cf6' }} /></div>
              <div className="dbs-divider" />
              <div className="dbs-foot">
                <span className="dbs-foot-lbl">Performance</span>
                <span className={`dbs-foot-val ${passPct >= 80 ? 'g' : passPct >= 50 ? 'a' : 'r'}`}>
                  {passPct >= 80 ? 'Excellent' : passPct >= 50 ? 'Average' : 'Needs work'}
                </span>
              </div>
            </div>
      ...
      {activeTab === 'team' && viewMode === 'list' && showSla && (
        <div className="db-card" style={{ flex: 1 }}>
          <div className="db-card-head-ind">
            <span className="db-card-title"><i className="bi bi-exclamation-octagon-fill" style={{ marginRight: 7 }} />Global SLA Breaches</span>
            <button className="db-back" style={{ background: 'rgba(255,255,255,0.18)', color: '#fff', border: '0.5px solid rgba(255,255,255,0.3)' }} onClick={() => setShowSla(false)}>
            ...
            <table className="db-table">
              ...
              <tbody>
                {tableLoading
                  ? <SkeletonRows cols={4} rows={6} />
                  : slaData.length === 0
                    ? <tr><td colSpan={4}><div className="db-empty"><i className="bi bi-patch-check db-empty-icon" /><p className="db-empty-title">No SLA breaches</p><span className="db-empty-sub">All incidents are within service level targets.</span></div></td></tr>
                    : slaData.map((it, i) => (
                        <tr key={i}>
                          <td className="inc-link">{it.incident_number}</td>
                          <td>{fmt(it.done_by)}</td>
      ...
      {activeTab === 'team' && viewMode === 'list' && !showSla && (
        <div className="db-full">
          <div className="db-grid-2">
            ...
            <div className="db-card">
              <div className="db-card-head" style={{ flexWrap: 'wrap', gap: 8 }}>
                <div>
                  <span className="db-card-title">
                    <i className="bi bi-bar-chart-line-fill" style={{ marginRight: 7, color: '#6366f1' }} />
                    Tickets by Analyst
                  </span>
                  <div style={{ fontSize: 10, color: '#9aa0b4', fontWeight: 600, marginTop: 2, letterSpacing: '0.04em' }}>
                    {chartTfLabel.toUpperCase()} · within {periodLabel}
                  </div>
                </div>
                <div className="db-tf-switch">
                  {[
                    ...
                    <button key={opt.id} onClick={() => setChartTimeframe(opt.id)} className={`db-tf-btn ${chartTimeframe === opt.id ? 'active' : ''}`}>
              ...
              <div style={{ padding: '4px 14px 14px', position: 'relative', flex: 1, minHeight: 260 }}>
                {chartLoading && <div className="db-chart-loader"><div className="db-spinner" /></div>}
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart
                    data={[...finalChartAgents].sort((a, b) => (b.total_tickets || 0) - (a.total_tickets || 0))}
                    margin={{ top: 22, right: 14, left: -14, bottom: 30 }}
                    barCategoryGap="28%"
                  >
                    <defs>
                    ...
                    <CartesianGrid strokeDasharray="2 6" vertical={false} stroke="#e4e7ef" />
                    <XAxis dataKey="agent" axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 700, fill: '#7b8099' }} interval={0} angle={-22} textAnchor="end" tickFormatter={n => fmt(n.split(' ')[0])} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 700, fill: '#9aa0b4' }} allowDecimals={false} />
                    <Tooltip cursor={{ fill: 'rgba(99,102,241,0.08)' }} content={<BarTip />} />
                    <Bar dataKey="total_tickets" radius={[8, 8, 0, 0]} maxBarSize={42} label={{ position: 'top', fontSize: 10, fontWeight: 800, fill: '#1a1d2e' }}>
                      {[...finalChartAgents].sort((a, b) => (b.total_tickets || 0) - (a.total_tickets || 0)).map((a, i) => (
                        <Cell key={i} fill="url(#grad-bar)" cursor="pointer" onClick={() => openDetail(a)}
            ...
            {/* ── Analyst directory — driven by globalTimeframe ── */}
            <div className="db-card" style={{ minHeight: 0 }}>
              <div className="db-card-head">
                <div>
                  <span className="db-card-title">Analyst performance directory</span>
              ...
              <div className="db-table-wrap">
                {!bannerLoading && dirAgents.length === 0 && (
                  <div className="db-empty" style={{ padding: '40px 20px' }}>
                    <i className="bi bi-calendar-x db-empty-icon" />
                    <p className="db-empty-title">No data for {periodLabel}</p>
                    <span className="db-empty-sub">No analyst activity recorded for this period. Try a wider range.</span>
                  </div>
                )}
                {(bannerLoading || dirAgents.length > 0) && (
                  <table className="db-table" style={{ minWidth: 600 }}>
                            ...
                            <tr key={i} onClick={() => openDetail(a)}>
                              ...
                              <td>
                                <div className="db-prog">
                                  ...
                                  <span className="db-prog-lbl">{a.pass_percentage != null ? `${a.pass_percentage}%` : '—'}</span>
                                </div>
                              </td>
                            </tr>
                          ))
          ...
          <div className="db-card">
            <div className="db-card-head-ind">
              <span className="db-card-title"><i className="bi bi-person-badge-fill" style={{ marginRight: 7 }} />QA Admin Performance</span>
              <span className="db-pill-ghost">{qaAdmins.length} Auditors</span>
            </div>
            {qaAdmins.length === 0
              ? <div className="db-empty" style={{ padding: '32px' }}><p className="db-empty-title">No QA admin data</p></div>
                  ...
                  {qaAdmins.map((qa, idx) => {
                    ...
                    return (
                      <div key={idx} className="db-qa-col">
                        ...
                        <div>
                          <div className="db-qa-cov-lbl">Coverage</div>
                          <div className="db-qa-cov-track"><div className="db-qa-cov-fill" style={{ width: `${pct}%`, background: bc }} /></div>
                        </div>
                      </div>
                    );
                  })}
      ...
      {activeTab === 'team' && viewMode === 'detail' && (
        <div className="db-card" style={{ flex: 1 }}>
          <div className="db-detail-header">
            <button className="db-back" onClick={() => setViewMode('list')}><i className="bi bi-arrow-left" /> Back</button>
            <h3 className="db-detail-title">{fmt(selAgent?.agent)} <span>Tickets</span></h3>
          </div>
          <div className="db-table-wrap">
            <table className="db-table">
            ...
            <table className="db-table">
              ...
              <tbody>
                {tableLoading
                  ? <SkeletonRows cols={4} rows={8} />
                  : pending.length === 0
                    ? <tr><td colSpan={4}><div className="db-empty"><i className="bi bi-inbox db-empty-icon" /><p className="db-empty-title">No records found</p><span className="db-empty-sub">No incidents matching "{selQaPend?.label}" for the selected criteria.</span></div></td></tr>
                    : pending.map((inc, i) => (
                        <tr key={i}>
                          <td className="inc-link" onClick={() => navigate('/dashboard/update-incident', { state: { incidentData: inc, fromPending: true } })}>
                            # {inc.incident_number}
                          </td>
                          <td className="desc-cell">{inc.short_description || 'No description'}</td>
                          <td><StatusPill status={inc.status} /></td>
      ...
      {modal && selInc && (
        <div className="db-modal-overlay" onClick={() => setModal(false)}>
          <div className="db-modal" onClick={e => e.stopPropagation()}>
            <div className="db-modal-head">
              <h2 className="db-modal-title">{selInc.incident_number}</h2>
              <button className="db-modal-close" onClick={() => setModal(false)}><i className="bi bi-x-lg" /></button>
            </div>
            <div className="db-modal-body">
              <div className="db-modal-agent">
                <div className="db-modal-av">{selInc.display_name?.charAt(0)?.toUpperCase()}</div>
              ...
              <div className="db-modal-grid">
                <div className="db-modal-full"><div className="db-modal-lbl">Description</div><div className="db-modal-val">{selInc.short_description}</div></div>
                <div><div className="db-modal-lbl">Status</div><StatusPill status={selInc.status} /></div>
                <div className="db-modal-full"><div className="db-modal-lbl">Resolution Notes</div><div className="db-modal-res">{selInc.resolution_shared || 'No details provided.'}</div></div>
              </div>
            </div>
            <div className="db-modal-footer">
              <button className="db-done-btn" onClick={() => setModal(false)}>Close</button>
