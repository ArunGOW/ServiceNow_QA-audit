...
const [bannerLoading, setBannerLoading] = useState(false);
  ...
  // const summary  = data?.all_time?.summary              || {};
  const summary = bannerData?.summary || data?.all_time?.summary || {};
  const qaAdmins = data?.all_time?.qa_admin_performance || [];
...
const isBottom = (a) => dirAgents.length > 1 && a.agent === bottomAgentName && a.agent !== topAgentName;
  ...
  const [globalTimeframe, setGlobalTimeframe] = useState('all');
...
const periodLabel = {
  ...
  if (tf === '1m') {
    ...
    return { from_date: toDateStr(start), to_date: toDateStr(end) };
  }

  if (tf === '3m') {
    // 3 months back: start of that month to end of current month
  ...
  if (tf === '6m') {
    // 6 months back: start of that month to end of current month
  ...
  (async () => {
    try {
      ...
      // Use 'filtered' if the API returns it, fall back to 'all_time'
      const payload = r.data;
      const resolved = (globalTimeframe === 'all')
        ? payload?.all_time
        : (payload?.filtered || payload?.all_time);
      setBannerData(resolved);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  <div className="db-brand-text">
    <div className="db-brand-name">
      QA<span> Application</span>
    </div>

    <div className="db-brand-tagline">
      Smart Quality Assurance for SDO Team
    </div>
  </div>
</div>
  ...
  {activeTab === 'team' && viewMode !== 'detail' && (
  <div className="dbs-banner">
    {bannerLoading && <LoadBar visible={true} />}
