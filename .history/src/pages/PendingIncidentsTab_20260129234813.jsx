    ...
    <div style={styles.fullView}>
      ...
      <div style={styles.premiumBanner}>
        <div style={styles.bannerLeft}>
          ...
          <div>
            <h5 style={styles.bannerTitle}>My Pending Queues</h5>
            <p style={styles.bannerSub}>Showing {pendingIncidents.length} of {totalPending} assignments</p>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <div style={styles.paginationArea}>
                  ...
                  <span style={styles.pageIndicator}>Page {currentPage} / {Math.ceil(totalPending / perPage) || 1}</span>
            ...
            <div style={styles.emptyState}>
                <div style={styles.emptyIconBox}>
                    <i className={`bi ${selectedOption.icon}`} style={{ color: selectedOption.id ? selectedOption.color : '#cbd5e1' }}></i>
                </div>
                <h6 style={styles.emptyTitle}>No {selectedOption.label} Incidents Found</h6>
                <p style={styles.emptySub}>We couldn't find any records matching your current filter.</p>
                <button onClick={() => setStatusFilter('')} style={styles.resetLink}>Clear all filters</button>
            </div>
          )}
        </div>
      </section>
...
const styles = {
  fullView: { display: 'flex', flexDirection: 'column', flex: 1, height: '100%', minHeight: '100vh', gap: '12px', padding: '10px', backgroundColor: '#f8fafc' },
  premiumBanner: {
    background: 'linear-gradient(135deg, #1e1b4b 0%, #4338ca 100%)',
    borderRadius: '16px', padding: '12px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
    boxShadow: '0 10px 20px -5px rgba(67, 56, 202, 0.3)', color: 'white'
  },
  bannerLeft: { display: 'flex', alignItems: 'center', gap: '12px' },
  bannerRight: { display: 'flex', alignItems: 'center', gap: '20px' },
  bannerIconBox: { width: '36px', height: '36px', background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(10px)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' },
  bannerTitle: { margin: 0, fontWeight: '800', fontSize: '15px' },
  bannerSub: { margin: 0, fontSize: '10px', opacity: 0.7 },
  
  filterGroup: { display: 'flex', flexDirection: 'column', gap: '2px', minWidth: '150px' },
  bannerLabel: { fontSize: '8px', fontWeight: '800', opacity: 0.6, textTransform: 'uppercase' },
  bannerDropdown: { 
    background: 'rgba(255,255,255,0.1)', padding: '5px 10px', borderRadius: '8px', 
    display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer',
    position: 'relative'
  },
  dropdownText: { fontSize: '11px', fontWeight: '700' },
  divider: { width: '1px', height: '25px', background: 'rgba(255,255,255,0.2)' },
  statValue: { fontSize: '18px', fontWeight: '900', display: 'block' },
  
  customMenu: { 
    position: 'absolute', top: '115%', right: 0, width: '160px', background: '#fff', 
    borderRadius: '10px', boxShadow: '0 10px 25px rgba(0,0,0,0.15)', zIndex: 100, padding: '4px', border: '1px solid #f1f5f9' 
  },
  menuItem: { padding: '7px 10px', fontSize: '11px', fontWeight: '600', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' },
  
  card: { background: '#fff', borderRadius: '16px', border: '1px solid #eef2f6', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' },
  tableScroll: { flex: 1, overflowY: 'auto', padding: '0 15px', display: 'flex', flexDirection: 'column' },
  ...
  td: { padding: '10px 8px', fontSize: '11.5px', background: '#fcfdfe', borderTop: '1px solid #f1f5f9', borderBottom: '1px solid #f1f5f9', verticalAlign: 'middle' },
  idBadge: { fontWeight: '800', color: '#4338ca', fontSize: '11px' },
  textTruncate: { maxWidth: '280px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#334155', fontWeight: '500' },
  ...
  paginationArea: { display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '15px', padding: '15px', marginTop: 'auto' },
  pagBtn: { border: 'none', background: '#f0f4ff', color: '#4338ca', padding: '5px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontWeight: '700' },
  pagBtnDisabled: { background: '#f8fafc', color: '#cbd5e1', padding: '5px 12px', borderRadius: '6px', cursor: 'not-allowed' },
  ...
  emptyIconBox: { width: '60px', height: '60px', background: '#f8fafc', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', marginBottom: '15px', border: '1px solid #f1f5f9' },
  emptyTitle: { fontSize: '16px', fontWeight: '800', color: '#1e293b', margin: '0 0 5px 0' },
  emptySub: { fontSize: '12px', color: '#94a3b8', margin: 0, maxWidth: '250px' },
  resetLink: { background: 'none', border: 'none', color: '#4338ca', fontWeight: '700', fontSize: '12px', cursor: 'pointer', marginTop: '15px', textDecoration: 'underline' },
  
  skeletonRow: { height: '38px', background: '#f8fafc', margin: '8px 0', borderRadius: '8px' },
