...
const PendingIncidentsTab = () => {
  ...
  return (
    <div style={styles.fullView}>
      ...
      {/* 🌤️ LITE GRADIENT BANNER */}
      <div style={styles.premiumBanner}>
        <div style={styles.bannerLeft}>
          <div style={styles.bannerIconBox}><i className="bi bi-intersect"></i></div>
          <div>
            <h5 style={styles.bannerTitle}>My Pending Queues</h5>
            <p style={styles.bannerSub}>Overview of your active workload</p>
          </div>
        </div>
        ...
        <div style={styles.bannerRight}>
          <div style={styles.filterGroup} ref={dropdownRef}>
            <label style={styles.bannerLabel}>Filter Status</label>
            <div 
              style={{...styles.bannerDropdown, border: isOpen ? '1px solid #4f46e5' : '1px solid #e2e8f0'}}
              onClick={() => setIsOpen(!isOpen)}
            >
              <div className="d-flex align-items-center gap-2">
              ...
              <i className="bi bi-chevron-down" style={{fontSize: '10px', color: '#64748b'}}></i>
          ...
          <div style={styles.divider}></div>
          <div style={styles.statMini}>
              <span style={styles.bannerLabel}>TOTAL</span>
              <span style={styles.statValue}>{totalPending}</span>
          </div>
        </div>
      </div>
        ...
        <div style={styles.tableScroll}>
          {loading ? (
                ...
                <tbody>
                  {pendingIncidents.map((incident, i) => (
                    <tr key={i}>
                      ...
                      <td style={styles.td}><div style={styles.resText}>{incident.resolution_shared || "Pending check..." }</div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div style={styles.emptyState}>
                ...
                <h6 style={styles.emptyTitle}>Queue Empty</h6>
                <p style={styles.emptySub}>No records found for "{selectedOption.label}" status.</p>
                <button onClick={() => setStatusFilter('')} style={styles.resetLink}>View all incidents</button>
            </div>
          )}
        </div>
      </section>
...
const styles = {
  fullView: { display: 'flex', flexDirection: 'column', flex: 1, height: '100%', gap: '12px', padding: '10px', backgroundColor: '#fdfeff' },
  premiumBanner: {
    // Light Lavender to Soft Blue Gradient
    background: 'linear-gradient(135deg, #f5f7ff 0%, #e0e7ff 100%)',
    borderRadius: '16px', padding: '15px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
    border: '1px solid #dee2f2', boxShadow: '0 4px 12px rgba(0,0,0,0.03)', color: '#1e293b'
  },
  bannerLeft: { display: 'flex', alignItems: 'center', gap: '12px' },
  bannerRight: { display: 'flex', alignItems: 'center', gap: '20px' },
  bannerIconBox: { width: '38px', height: '38px', background: '#fff', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', color: '#4f46e5', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' },
  bannerTitle: { margin: 0, fontWeight: '800', fontSize: '16px', color: '#1e1b4b' },
  bannerSub: { margin: 0, fontSize: '11px', color: '#64748b', fontWeight: '500' },
  
  filterGroup: { display: 'flex', flexDirection: 'column', gap: '3px', minWidth: '150px' },
  bannerLabel: { fontSize: '8px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase' },
  bannerDropdown: { 
    background: '#fff', padding: '5px 10px', borderRadius: '8px', 
    display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer',
    position: 'relative'
  },
  dropdownText: { fontSize: '11px', fontWeight: '700', color: '#334155' },
  divider: { width: '1px', height: '30px', background: '#cbd5e1' },
  statValue: { fontSize: '20px', fontWeight: '900', color: '#4f46e5', display: 'block' },
  
  customMenu: { 
    position: 'absolute', top: '115%', right: 0, width: '165px', background: '#fff', 
    borderRadius: '10px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', zIndex: 100, padding: '4px', border: '1px solid #f1f5f9' 
  },
  menuItem: { padding: '8px 10px', fontSize: '11px', fontWeight: '600', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' },
  
  card: { background: '#fff', borderRadius: '16px', border: '1px solid #f1f5f9', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' },
  ...
  td: { padding: '11px 8px', fontSize: '11.5px', background: '#fcfdfe', borderTop: '1px solid #f8f9fb', borderBottom: '1px solid #f8f9fb', verticalAlign: 'middle' },
  idBadge: { fontWeight: '800', color: '#4f46e5', fontSize: '11px' },
  textTruncate: { maxWidth: '300px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#334155', fontWeight: '600' },
  statusBadge: { padding: '3px 8px', borderRadius: '5px', fontSize: '9px', fontWeight: '800', textTransform: 'uppercase' },
  ...
  pagBtn: { border: '1px solid #e2e8f0', background: '#fff', color: '#4f46e5', padding: '5px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontWeight: '700' },
  pagBtnDisabled: { background: '#f8fafc', color: '#cbd5e1', border: '1px solid #f1f5f9', padding: '5px 12px', borderRadius: '6px', cursor: 'not-allowed' },
  pageIndicator: { fontSize: '11px', fontWeight: '700', color: '#94a3b8' },

  emptyState: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '50px 0' },
  emptyIconBox: { width: '50px', height: '50px', background: '#f8fafc', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', marginBottom: '12px' },
  emptyTitle: { fontSize: '15px', fontWeight: '800', color: '#1e293b', margin: '0' },
  emptySub: { fontSize: '11px', color: '#94a3b8', marginTop: '4px' },
  resetLink: { background: 'none', border: 'none', color: '#4f46e5', fontWeight: '700', fontSize: '11px', cursor: 'pointer', marginTop: '12px', textDecoration: 'underline' },
  
  skeletonRow: { height: '40px', background: '#f8fafc', margin: '6px 0', borderRadius: '8px' },
