    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const data = await apiRequest(
          "get-processed/incidents",   // ✅ endpoint
          "POST",                      // ✅ method
          { user_sid: user.user_sid }, // ✅ payload (body)
          { skip, limit }              // ✅ query params
        );

        console.log("📌 API Response:", data);

        setProcessedIncidents(data?.items || []); // adapt to backend structure
        if (data?.totalCount) {
          const pages = Math.ceil(data.totalCount / limit);
          setTotalPages(pages);
          console.log("📄 Total Pages:", pages);
        } else {
          setTotalPages(currentPage + 1); // fallback
        }
      } catch (error) {
        console.error("❌ Error fetching processed incidents:", error);
      } finally {
