...
const UpdateIncident = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    status: 'resolved',
    resolution_shared: 'No resolution steps provided yet...',
    updates_link: 'https://example.com',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFeedback({ type: 'danger', message: err.response?.data?.message || "Sync Failed" });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={styles.pageWrapper}>
      ...
      <div style={styles.mainGrid}>
        ...
        <div style={styles.formSection}>
          <h4 style={styles.sectionTitle}>Incident <span style={{ color: '#4f46e5' }}>Editor</span></h4>
          <Form onSubmit={handleSubmit}>
            <Row className="g-3">
              <Col md={6}>
                <Form.Label style={styles.label}>Incident ID</Form.Label>
                <Form.Control name="incident_number" placeholder="INC0012345" style={styles.input} onChange={handleChange} required />
              </Col>
              <Col md={6}>
                <Form.Label style={styles.label}>Analyst</Form.Label>
                <Form.Control name="user" value={formData.user} style={styles.input} onChange={handleChange} />
              </Col>
              <Col md={12}>
                <Form.Label style={styles.label}>Short Description</Form.Label>
                <Form.Control name="short_description" style={styles.input} onChange={handleChange} required />
              </Col>
              <Col md={12}>
                <Form.Label style={styles.label}>Resolution Shared</Form.Label>
                <Form.Control as="textarea" rows={4} name="resolution_shared" style={styles.textarea} onChange={handleChange} />
              </Col>
              <Col md={7}>
                <Form.Label style={styles.label}>Documentation Link</Form.Label>
                <Form.Control name="updates_link" style={styles.input} onChange={handleChange} />
              </Col>
              <Col md={5}>
                <Form.Label style={styles.label}>Lifecycle Status</Form.Label>
                <Form.Select name="status" style={styles.select} onChange={handleChange}>
            ...
            <Button type="submit" style={styles.submitBtn} className="w-100 mt-4" disabled={loading}>
              {loading ? <Spinner size="sm" /> : 'Commit Changes to Database'}
            </Button>
          </Form>
        </div>

        {/* RIGHT: LIVE PREVIEW (The "Digital Twin") */}
        <div style={styles.previewSection}>
          <div style={styles.previewHeader}>
            <span style={styles.liveDot}></span> LIVE DATA PREVIEW
          </div>
          
          <div style={styles.previewCard}>
            <div className="d-flex justify-content-between align-items-start mb-4">
              <div>
                <div style={styles.previewTag}>OFFICIAL RECORD</div>
                <h3 style={styles.previewId}>{formData.incident_number || '---'}</h3>
              </div>
              <div style={{...styles.statusPill, backgroundColor: formData.status === 'resolved' ? '#dcfce7' : '#fef3c7', color: formData.status === 'resolved' ? '#166534' : '#92400e'}}>
                {formData.status.toUpperCase()}
              </div>
            </div>

            <div style={styles.previewItem}>
              <label style={styles.previewLabel}>Subject</label>
              <div style={styles.previewValue}>{formData.short_description}</div>
            </div>

            <div style={styles.previewItem}>
              <label style={styles.previewLabel}>Resolution Intelligence</label>
              <div style={styles.previewBox}>{formData.resolution_shared}</div>
            </div>

            <Row className="mt-auto">
              <Col xs={6}>
                <label style={styles.previewLabel}>Author</label>
                <div style={styles.previewSmallValue}><i className="bi bi-person-circle me-1"></i> {formData.user}</div>
              </Col>
              <Col xs={6} className="text-end">
                <label style={styles.previewLabel}>Log Link</label>
                <div style={styles.previewSmallValue}><i className="bi bi-link-45deg"></i> External Reference</div>
              </Col>
            </Row>
          </div>
        </div>
...
const styles = {
  ...
  previewSection: { backgroundColor: '#0f172a', borderRadius: '20px', padding: '25px', display: 'flex', flexDirection: 'column', color: '#fff' },
