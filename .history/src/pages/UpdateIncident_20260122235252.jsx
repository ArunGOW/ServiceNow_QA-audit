...
const UpdateIncident = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // 422 Fix: Ensure we don't send empty strings for the link if not provided
    const payload = [{
      ...formData,
      updates_link: formData.updates_link || " ", // Some APIs prefer a space or null over ""
      resolution_shared: formData.resolution_shared || "No resolution provided"
    }];
    ...
    try {
      ...
      if (res.data.status === "success" || res.status === 200) {
        setFeedback({ type: 'success', message: res.data.message || "Sync Successful" });
        const newEntry = { ...formData, timestamp: new Date().toLocaleTimeString() };
        setRecentIncidents([newEntry, ...recentIncidents.filter(i => i.incident_number !== formData.incident_number)].slice(0, 5)); 
        setFormData(initialForm);
      }
    } catch (err) {
      // Detailed error logging for 422 debugging
      console.error("Server 422 Error Detail:", err.response?.data);
      setFeedback({ 
        type: 'danger', 
        message: err.response?.data?.detail?.[0]?.msg || "Validation Error (422): Please check all fields." 
      });
    } finally {
      setLoading(false);
    }
        ...
        <div style={styles.formSection}>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <div className="d-flex gap-2 mt-4">
              ...
              <Button onClick={resetForm} variant="light" style={{ borderRadius: '12px', fontWeight: '700', color: '#64748b', border: '1px solid #e2e8f0' }}>
            ...
            {recentIncidents.length === 0 ? (
              ...
              recentIncidents.map((inc, index) => (
                <div 
                  ...
                  style={{...styles.previewItem, cursor: 'pointer', borderBottom: '1px solid #1e293b', paddingBottom: '15px', transition: '0.2s'}}
                ...
                >
                  ...
                  <div style={{...styles.previewValue, fontSize: '18px', color: '#fff'}}>{inc.incident_number}</div>
                  <div style={{...styles.previewSmallValue, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}}>
...
const styles = {
  ...
  previewId: { fontSize: '28px', fontWeight: '900', margin: 0, color: '#f8fafc' },
  statusPill: { padding: '4px 12px', borderRadius: '20px', fontSize: '10px', fontWeight: '900' },
  previewItem: { marginBottom: '10px', padding: '10px', borderRadius: '8px' },
  previewLabel: { display: 'block', fontSize: '10px', color: '#475569', fontWeight: '800', textTransform: 'uppercase', marginBottom: '4px' },
  previewValue: { fontSize: '15px', fontWeight: '600', color: '#cbd5e1' },
  previewSmallValue: { fontSize: '12px', fontWeight: '600', color: '#94a3b8' }
};
