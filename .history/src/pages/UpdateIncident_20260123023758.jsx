...
import toast, { Toaster } from 'react-hot-toast'; // Premium Toast Library
import api from "../api/axois";
import { useAuth } from "../context/AuthContext";
...
const UpdateIncident = () => {
  ...
  const handleEdit = (incident) => {
    setFormData({ ...incident });
    toast('Record loaded for editing', { icon: '📝' });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Sync Failed. Please check fields.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={styles.pageWrapper}>
      {/* Toaster Container - Must be present in the JSX */}
      ...
      <div style={styles.mainGrid}>
        <div style={styles.formSection}>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Button type="submit" style={styles.submitBtn} className="w-100 mt-4 text-white" disabled={loading}>
              {loading ? <Spinner size="sm" /> : 'SAVE & SYNC INCIDENT'}
            </Button>
          </Form>
        </div>
        ...
        <div style={styles.previewSection}>
          <div style={styles.previewHeader}>
            <div style={styles.liveDot}></div> PERSISTENT ACTIVITY LOG
          </div>
          <div style={styles.previewCard}>
            {recentIncidents.length === 0 ? (
              <div style={{ textAlign: 'center', marginTop: '50px', color: '#475569' }}>
