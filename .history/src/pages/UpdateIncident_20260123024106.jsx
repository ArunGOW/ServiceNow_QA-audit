...
import { toast } from 'react-toastify'; // Using your existing library
import api from "../api/axois";
import { useAuth } from "../context/AuthContext";
...
const UpdateIncident = () => {
  ...
  // Defining the blank state to reuse for clearing the form
  ...
  const handleEdit = (incident) => {
    setFormData({ ...incident });
    toast.info(`Loaded ${incident.incident_number} for editing`);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.status === "success" || res.status === 200) {
        // 1. Success Toast
        toast.success(`Incident ${formData.incident_number} updated successfully!`);

        // 2. Update local history list
        const newEntry = { ...formData, timestamp: new Date().toLocaleString() };
        const updatedList = [newEntry, ...recentIncidents.filter(i => i.incident_number !== formData.incident_number)].slice(0, 10);
  ...
  return (
    <div style={styles.pageWrapper}>
      ...
      <div style={styles.mainGrid}>
        ...
        <div style={styles.previewSection}>
          <div style={styles.previewHeader}>
            <div style={styles.liveDot}></div> INCIDENT HISTORY
          </div>
          <div style={styles.previewCard}>
            {recentIncidents.length === 0 ? (
              <div style={{ textAlign: 'center', marginTop: '50px', color: '#475569' }}>
                <i className="bi bi-clock-history mb-2" style={{ fontSize: '2rem' }}></i>
                <p style={styles.previewLabel}>No recent updates</p>
              </div>
            ) : (
              recentIncidents.map((inc, index) => (
                <div key={index} onClick={() => handleEdit(inc)} style={styles.previewItem}>
                  ...
                  <div style={{...styles.previewValue, fontSize: '18px', color: '#fff'}}>{inc.incident_number}</div>
                  <div style={{...styles.previewSmallValue, textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap'}}>{inc.short_description}</div>
                </div>
              ))
            )}
          </div>
...
const styles = {
  ...
  previewId: { fontSize: '28px', fontWeight: '900', margin: 0, color: '#f8fafc' },
  statusPill: { padding: '4px 12px', borderRadius: '20px', fontSize: '10px', fontWeight: '900' },
  previewItem: { marginBottom: '15px', padding: '10px', borderRadius: '10px', cursor: 'pointer', transition: '0.2s', borderBottom: '1px solid #1e293b' },
  previewLabel: { display: 'block', fontSize: '10px', color: '#475569', fontWeight: '800', textTransform: 'uppercase', marginBottom: '4px' },
  previewValue: { fontSize: '15px', fontWeight: '600', color: '#cbd5e1' },
  previewSmallValue: { fontSize: '12px', fontWeight: '600', color: '#94a3b8' }
};
