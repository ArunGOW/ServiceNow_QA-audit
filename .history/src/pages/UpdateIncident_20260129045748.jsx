  ...
  <div style={styles.dropdownContainer}>
    ...
    <div 
      style={{
        ...
        boxShadow: isOpenAnalyst ? '0 0 0 3px rgba(79, 70, 229, 0.1)' : 'none'
      }} 
      onClick={() => {
        setIsOpenAnalyst(!isOpenAnalyst);
        setIsOpenStatus(false); // Close other dropdowns
      }}
    >
      <span style={styles.triggerText}>
        {formData.done_by || "Select Analyst..."}
      </span>
      <i className={`bi bi-chevron-${isOpenAnalyst ? 'up' : 'down'} text-muted`} style={{ fontSize: '10px' }}></i>
    </div>

    {/* The Floating Menu */}
    {isOpenAnalyst && (
      <div style={styles.customMenu} className="custom-scrollbar">
        {/* Added Search Bar inside Dropdown for Premium feel */}
        <div style={{ padding: '8px', borderBottom: '1px solid #f1f5f9', position: 'sticky', top: 0, background: '#fff', zIndex: 2 }}>
          <input 
            autoFocus
            placeholder="Search analyst..." 
            style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '12px', padding: '5px 10px', outline: 'none' }}
            onClick={(e) => e.stopPropagation()} // Prevent menu from closing when clicking input
            onChange={(e) => {
              const term = e.target.value.toLowerCase();
              // Optional: You can filter the local userList here if you want
            }}
          />
        </div>
        ...
        {userList.length === 0 ? (
          <div style={{ padding: '20px', textAlign: 'center', fontSize: '12px', color: '#94a3b8' }}>
            <Spinner size="sm" animation="border" className="me-2" /> Loading Users...
          </div>
        ) : (
          userList.map(u => (
            <div 
              ...
              style={{
                ...styles.menuItem,
                background: formData.done_by === u.full_name ? '#f0f4ff' : 'transparent',
                color: formData.done_by === u.full_name ? '#4f46e5' : '#475569'
              }} 
              onClick={(e) => {
                e.stopPropagation();
                setFormData({...formData, done_by: u.full_name});
            ...
            >
              <div style={{
                ...
                fontSize: '10px', fontWeight: 'bold', marginRight: '10px'
              }}>
