...
const UpdateIncident = () => {
  ...
  // Refs to fix the ReferenceError and handle clicking outside
  ...
  return (
    <div style={styles.pageWrapper}>
      <style>{`
        ...
        .menu-item:hover { background-color: #f1f5f9 !important; transform: translateX(5px); }
                  ...
                  {isOpenAnalyst && (
                    <div style={styles.customMenu} className="custom-scrollbar">
                      {userList.length > 0 ? (
                        ...
                        <div className="text-center p-3 small text-muted">Loading analysts...</div>
                      )}
                    </div>
                  )}
                </div>
...
const styles = {
  ...
  premiumTrigger: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 15px', borderRadius: '12px', border: '1px solid #e2e8f0', backgroundColor: '#f8fafc', cursor: 'pointer' },
  triggerText: { fontSize: '14px', fontWeight: '600', color: '#1e293b' },
  statusDot: { width: '8px', height: '8px', borderRadius: '50%', marginRight: '10px' },
  customMenu: { position: 'absolute', top: '110%', left: 0, right: 0, backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 15px 30px rgba(0,0,0,0.1)', zIndex: 1000, border: '1px solid #e2e8f0', padding: '6px', maxHeight: '200px', overflowY: 'auto', animation: 'fadeInUp 0.2s ease-out' },
  ...
  avatarMini: { width: '22px', height: '22px', background: '#e0e7ff', color: '#4f46e5', borderRadius: '6px', marginRight: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: 'bold' },
