...
const UpdateIncident = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFeedback({ type: 'danger', message: "Sync Failed. Please check connection." });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={styles.pageWrapper}>
      ...
      <div className="premium-card">
        ...
        <div style={styles.banner}>
          <div className="d-flex align-items-center gap-3">
            <div style={styles.iconCircle}><i className="bi bi-shield-check"></i></div>
            <div>
              <h4 className="m-0 fw-bold" style={{ color: '#fff' }}>Update <span style={{ opacity: 0.8 }}>Incident</span></h4>
              <p className="m-0 small text-white-50">Operational Management Portal</p>
            </div>
          </div>
          <div className="text-end">
            <div style={styles.liveBadge}><span style={styles.liveDot}></span> System Live</div>
          </div>
        </div>
        ...
        <Form onSubmit={handleSubmit}>
          <Row className="g-4">
              ...
              <div style={styles.dropdownContainer} ref={analystRef}>
                <div style={styles.premiumTrigger} onClick={() => setIsOpenAnalyst(!isOpenAnalyst)}>
                  <div className="d-flex align-items-center overflow-hidden">
                    ...
                    <span style={styles.triggerText} className="text-truncate">{formData.done_by || "Select Analyst"}</span>
                  </div>
            ...
            <Col md={6}>
              <Form.Label style={styles.label}>Short Description</Form.Label>
              <Form.Control value={formData.short_description} placeholder="Main issue..." style={styles.input} onChange={(e) => setFormData({...formData, short_description: e.target.value})} required />
            </Col>
            ...
            <Col md={12}>
              ...
              <Form.Control value={formData.resolution_shared} placeholder="Describe the fix..." style={styles.input} onChange={(e) => setFormData({...formData, resolution_shared: e.target.value})} />
            </Col>
          </Row>

          {feedback && <Alert variant={feedback.type} className="mt-4 border-0 shadow-sm py-2 small">{feedback.message}</Alert>}

          <Button type="submit" style={styles.submitBtn} className="w-100 mt-4 text-white" disabled={loading}>
            {loading ? <Spinner size="sm" className="me-2"/> : <i className="bi bi-cloud-arrow-up-fill me-2"></i>}
            {loading ? 'Processing...' : 'Sync with Central Dashboard'}
          </Button>
        </Form>

        {/* Horizontal Session History Section */}
        <div className="mt-5">
          <div className="d-flex justify-content-between align-items-center mb-3">
            <h6 style={{ fontSize: '12px', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '1px' }}>
              <i className="bi bi-clock-history me-2"></i> Recent Sessions
            </h6>
            {recentIncidents.length > 0 && (
              <span onClick={() => setShowClearModal(true)} style={{ cursor: 'pointer', fontSize: '11px', color: '#f43f5e', fontWeight: 700 }}>
                Clear All
              </span>
            )}
          </div>
          <div className="d-flex overflow-auto pb-2 custom-scrollbar">
            {recentIncidents.length === 0 ? (
              <div className="text-muted small italic">No recent history in this session.</div>
            ) : (
              recentIncidents.map((inc, i) => (
                <div key={i} className="history-item">
                  <div className="d-flex justify-content-between mb-1">
                    {/* ✅ White Incident Number */}
                    <span style={{ fontSize: '11px', fontWeight: 800, color: '#fff' }}>{inc.incident_number}</span>
                    <span style={{ fontSize: '9px', color: inc.status === 'resolved' ? '#10b981' : '#f59e0b' }}>● {inc.status}</span>
                  </div>
                  <div style={{ fontSize: '10px', color: '#94a3b8', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{inc.short_description}</div>
                </div>
              ))
            )}
          </div>
      ...
      <Modal show={showClearModal} onHide={() => setShowClearModal(false)} centered size="sm">
        <Modal.Body className="text-center p-4">
          <h6 className="fw-bold">Clear history?</h6>
          <div className="d-flex gap-2 mt-4">
            <Button variant="light" className="flex-grow-1" onClick={() => setShowClearModal(false)}>No</Button>
            <Button variant="danger" className="flex-grow-1" onClick={() => { setRecentIncidents([]); setShowClearModal(false); }}>Yes</Button>
          </div>
        </Modal.Body>
      </Modal>
    </div>
...
const styles = {
  ...
  label: { fontSize: '10px', fontWeight: '800', color: '#64748b', textTransform: 'uppercase', marginBottom: '8px', letterSpacing: '0.5px' },
  input: { 
    borderRadius: '12px', 
    padding: '10px 15px', 
    border: '1px solid #e2e8f0', 
  ...
  menuItem: { padding: '10px 15px', fontSize: '13px', fontWeight: '600', color: '#475569', cursor: 'pointer' },
  submitBtn: { 
    background: '#4f46e5', 
    border: 'none', 
    borderRadius: '12px', 
