  ...
  return (
    <div style={styles.pageWrapper}>
      ...
      <style>{`
        ...
        .menu-item:hover { background-color: #f8fafc !important; transform: translateX(5px); transition: 0.2s; }
      `}</style>
      ...
      <div style={styles.mainContainer}>
        ...
        <div style={styles.leftSection}>
          <div className="d-flex justify-content-between align-items-start mb-4">
            <div>
              <h2 className="fw-bold m-0" style={{ color: '#0f172a' }}>Update <span style={{ color: '#4f46e5' }}>Incident</span></h2>
              <p className="text-muted small">Standard operating procedure for dashboard sync</p>
            </div>
            <div style={styles.badge}><span style={styles.dot}></span> System Online</div>
          </div>
          ...
          <Form onSubmit={handleSubmit}>
            <Row className="g-3">
              ...
              <Col md={6}>
                <Form.Label style={styles.label}>Analyst</Form.Label>
                <div style={styles.dropdownContainer} ref={analystRef}>
                  <div style={styles.premiumTrigger} onClick={() => setIsOpenAnalyst(!isOpenAnalyst)}>
                    <span style={styles.triggerText}>{formData.done_by || "Select Name"}</span>
                    <i className="bi bi-chevron-expand text-muted"></i>
                  </div>
                  {isOpenAnalyst && (
                    <div style={styles.customMenu} className="custom-scrollbar">
              ...
              <Col md={6}>
                <Form.Label style={styles.label}>Client User</Form.Label>
              ...
              <Col md={6}>
                <Form.Label style={styles.label}>Ticket Status</Form.Label>
                <div style={styles.dropdownContainer} ref={statusRef}>
                  <div style={styles.premiumTrigger} onClick={() => setIsOpenStatus(!isOpenStatus)}>
                    <div className="d-flex align-items-center">
                      <i className={`bi ${currentStatus.icon} me-2`} style={{ color: currentStatus.color }}></i>
                      <span style={styles.triggerText}>{currentStatus.label}</span>
                    </div>
                    <i className="bi bi-chevron-expand text-muted"></i>
                  </div>
              ...
              {formData.status === 'escalated' && (
                <Col md={12}>
                  <Form.Label style={{...styles.label, color: '#f43f5e'}}>Escalation Reason</Form.Label>
                  <Form.Control as="textarea" rows={1} style={{...styles.input, borderColor: '#fecaca'}} value={formData.escalated_notes} onChange={(e) => setFormData({...formData, escalated_notes: e.target.value})} required />
                </Col>
              )}

              <Col md={12}><Form.Label style={styles.label}>Resolution Summary</Form.Label><Form.Control as="textarea" rows={2} value={formData.resolution_shared} style={styles.input} onChange={(e) => setFormData({...formData, resolution_shared: e.target.value})} /></Col>
            </Row>
            
            {feedback && <Alert variant={feedback.type} className="mt-3 py-2 small border-0 shadow-sm">{feedback.message}</Alert>}
            
            <Button type="submit" style={styles.submitBtn} className="w-100 mt-4 text-white" disabled={loading}>
              {loading ? <Spinner size="sm" /> : 'Sync with Central Dashboard'}
            </Button>
          </Form>
        </div>

        {/* RIGHT SECTION: SESSION HISTORY (SIDEBAR) */}
        <div style={styles.rightSection} className="custom-scrollbar">
          <div className="d-flex justify-content-between align-items-center mb-4">
            <h6 className="m-0 text-white-50 fw-bold" style={{ letterSpacing: '1px', fontSize: '11px' }}>RECENT ACTIVITY</h6>
            {recentIncidents.length > 0 && <i className="bi bi-trash3 text-danger" style={{cursor: 'pointer'}} onClick={() => setShowClearModal(true)}></i>}
          </div>
          ...
          {recentIncidents.map((inc, i) => (
            <div key={i} style={styles.historyCard}>
              <div className="d-flex justify-content-between align-items-center mb-2">
                ...
                <span style={{ fontSize: '12px', fontWeight: 800, color: '#ffffff' }}>{inc.incident_number}</span>
                <span style={{ fontSize: '8px', background: inc.status === 'resolved' ? '#064e3b' : '#451a03', color: inc.status === 'resolved' ? '#10b981' : '#fbbf24', padding: '2px 6px', borderRadius: '4px' }}>
                  {inc.status.toUpperCase()}
                </span>
              </div>
              <div style={styles.cardDesc}>{inc.short_description}</div>
              <div style={styles.cardTime}>{inc.timestamp}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <Modal show={showClearModal} onHide={() => setShowClearModal(false)} centered size="sm">
        <Modal.Body className="text-center p-4">
          <h6 className="fw-bold">Clear local log?</h6>
          <div className="d-flex gap-2 mt-4">
...
const styles = {
  ...
  leftSection: { 
    ...
    padding: '40px', 
    overflowY: 'auto',
    minWidth: '60%' // Ensures form stays prominent
  },
  rightSection: { 
    width: '320px', 
    flexShrink: 0, // CRITICAL: Prevents sidebar from disappearing
    backgroundColor: '#0f172a', 
    padding: '30px', 
    overflowY: 'auto' 
  },
  badge: { background: '#f1f5f9', padding: '5px 12px', borderRadius: '20px', fontSize: '10px', color: '#64748b', fontWeight: 'bold' },
  dot: { display: 'inline-block', width: '6px', height: '6px', background: '#10b981', borderRadius: '50%', marginRight: '5px' },
  label: { fontSize: '10px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', marginBottom: '5px' },
  input: { borderRadius: '10px', padding: '10px', border: '1px solid #e2e8f0', fontSize: '13px', backgroundColor: '#f8fafc' },
  dropdownContainer: { position: 'relative' },
  premiumTrigger: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px', borderRadius: '10px', border: '1px solid #e2e8f0', backgroundColor: '#f8fafc', cursor: 'pointer' },
  triggerText: { fontSize: '13px', fontWeight: '600', color: '#1e293b' },
  customMenu: { position: 'absolute', top: '110%', left: 0, right: 0, backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 10px 15px rgba(0,0,0,0.1)', zIndex: 1000, border: '1px solid #e2e8f0', maxHeight: '200px', overflowY: 'auto' },
  menuItem: { padding: '10px 15px', fontSize: '12px', fontWeight: '600', color: '#475569', cursor: 'pointer' },
  submitBtn: { background: 'linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)', border: 'none', borderRadius: '12px', padding: '15px', fontWeight: '800', boxShadow: '0 10px 15px -3px rgba(79, 70, 229, 0.4)' },
  historyCard: { background: '#1e293b', padding: '15px', borderRadius: '12px', marginBottom: '12px', borderLeft: '3px solid #4f46e5' },
  cardDesc: { fontSize: '11px', color: '#94a3b8', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  cardTime: { fontSize: '9px', color: '#475569', marginTop: '8px', textAlign: 'right' }
};
