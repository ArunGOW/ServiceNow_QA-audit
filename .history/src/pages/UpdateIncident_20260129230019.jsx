...
const UpdateIncident = () => {
  ...
  const handleEdit = (incident) => {
    ...
    setFeedback({ type: 'info', message: `Editing ${incident.incident_number}` });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.status || !formData.done_by) {
      setFeedback({ type: 'danger', message: "Analyst and Status selection is mandatory." });
      return;
    }
    ...
    try {
      ...
      setFeedback({ type: 'success', message: "Dashboard Updated Successfully!" });
      setFormData(initialFormState);
    } catch (err) {
      setFeedback({ type: 'danger', message: "Sync Failed - API Error" });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div style={styles.formSection}>
          <div className="premium-banner">
            <div className="d-flex align-items-center gap-3">
              ...
              <div>
                <h5 className="m-0 fw-bold">Update Controls</h5>
                <p className="m-0 small opacity-75">Mandatory fields are marked with *</p>
              </div>
            </div>
          </div>

          <div className="d-flex justify-content-between align-items-center mb-3">
            <h4 className="m-0 fw-bold">Incident <span style={{ color: '#4f46e5' }}>Entry</span></h4>
            {recentIncidents.length > 0 && (
              <Button variant="link" className="text-danger p-0 text-decoration-none small fw-bold" onClick={() => setShowClearModal(true)}>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <div className="d-flex gap-2 mt-4">
               <Button type="button" variant="light" onClick={handleReset} style={{ borderRadius: '12px', fontWeight: '700', flex: 1 }}>Reset</Button>
            ...
            <div key={i} style={styles.historyCard} className="history-card">
              <div className="d-flex justify-content-between align-items-center">
                ...
                <div className="d-flex align-items-center gap-2">
                   <Button variant="link" className="edit-btn p-0 text-info opacity-0" style={{visibility: 'hidden', transition: '0.2s'}} onClick={() => handleEdit(inc)}>
