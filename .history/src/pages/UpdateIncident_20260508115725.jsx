...
//     incident_number: '',
//     short_description: '',
//     status: '', // Initialized empty for validation
//     resolution_shared: '',
//     user: '', 
//     done_by: currentUser?.full_name || '', 
//     escalated_notes: '' 
...
//       resolution_shared: incident.resolution_shared,
//       user: incident.user,
//       done_by: incident.done_by,
//       escalated_notes: incident.escalated_notes || ''
//     });
...
//     if (!formData.status || !formData.done_by) {
//       setFeedback({ type: 'danger', message: "Please select both an Analyst and a Status." });
//       return;
//     }
...
//       // Update history: If ID exists, replace it, else add new
...
//       setFormData({ incident_number: '', short_description: '', resolution_shared: '', user: '', status: '', done_by: currentUser?.full_name || '', escalated_notes: '' });
//     } catch (err) {
//       setFeedback({ type: 'danger', message: "Sync Failed" });
//     } finally {
//       setLoading(false);
...
//         .custom-scrollbar::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; }
//         .menu-item:hover { background-color: #f8fafc !important; transform: translateX(5px); }
//         .history-card:hover .edit-btn { opacity: 1 !important; }
//         .premium-banner {
//           background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%);
//           border-radius: 16px;
//           padding: 20px;
...
//           <div className="premium-banner">
//             <div className="d-flex align-items-center gap-3">
//               <div style={styles.bannerIconBox}><i className="bi bi-lightning-charge-fill"></i></div>
//               <div>
//                 <h5 className="m-0 fw-bold">Incident Portal</h5>
//                 <p className="m-0 small opacity-75">All fields marked with * are mandatory</p>
//               </div>
//             </div>
//           </div>
...
//                         <span style={styles.triggerText}>{currentStatus.label}</span></>
//                       ) : (
//                         <span style={{...styles.triggerText, color: '#94a3b8'}}>Select Status</span>
//                       )}
//                     </div>
//                     <i className="bi bi-chevron-expand text-muted"></i>
//                   </div>
...
//               {(formData.status === 'escalated' || formData.status === '') && (
//                 <Col md={12} style={{ animation: 'fadeInUp 0.3s ease' }}>
//                   <Form.Label style={{...styles.label, color: formData.status === 'escalated' ? '#f43f5e' : '#94a3b8'}}>Escalation Reason {formData.status === 'escalated' && '*'}</Form.Label>
...
//                    <Button variant="link" className="edit-btn p-0 text-white opacity-50" style={{transition: '0.2s'}} onClick={() => handleEdit(inc)}>
...
const UpdateIncident = () => {
  ...
  const statusOptions = [
    { id: 'resolved', label: 'Resolved', color: '#10b981', icon: 'bi-collection-fill' },
    { id: 'escalated', label: 'Escalated', color: '#f43f5e', icon: 'bi-patch-check-fill' },
    { id: 'in progress', label: 'In Progress', color: '#3b82f6', icon: 'bi-arrow-up-circle-fill' },
    { id: 'on hold', label: 'On Hold', color: '#f59e0b', icon: 'bi-arrow-repeat' },
  ];
  ...
  <div style={{
    ...
    <span style={{ fontWeight: '600', fontSize: '13px' }}>{feedback.message}</span>
            ...
            <div className="d-flex gap-2 mt-4">
               ...
               <Button type="submit" style={{...styles.submitBtn, flex: 3}} className="text-white" disabled={loading}>
                 {loading ? <Spinner size="sm" /> : 'Sync with Dashboard'}
               </Button>
            </div>
          </Form>
...
const styles = {
  ...
  premiumTrigger: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderRadius: '10px', border: '1px solid #e2e8f0', backgroundColor: '#f8fafc', cursor: 'pointer', transition: 'all 0.2s' },
  triggerText: { fontSize: '13px', fontWeight: '600', color: '#1e293b' },
  customMenu: { position: 'absolute', top: '110%', left: 0, right: 0, backgroundColor: '#fff', borderRadius: '14px', boxShadow: '0 12px 30px rgba(0,0,0,0.1)', zIndex: 1000, border: '1px solid #e2e8f0', padding: '5px', maxHeight: '180px', overflowY: 'auto' },
  menuItem: { padding: '8px 12px', borderRadius: '8px', fontSize: '12px', fontWeight: '600', color: '#475569', cursor: 'pointer', display: 'flex', alignItems: 'center', transition: '0.2s', marginBottom: '2px' },
  avatarMini: { width: '20px', height: '20px', background: '#e0e7ff', color: '#4f46e5', borderRadius: '5px', marginRight: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: 'bold' },
  submitBtn: { background: '#4f46e5', border: 'none', borderRadius: '12px', padding: '12px', fontWeight: '800', boxShadow: '0 4px 12px rgba(79, 70, 229, 0.3)' },
  previewSection: { backgroundColor: '#0f172a', padding: '25px', overflowY: 'auto' },
